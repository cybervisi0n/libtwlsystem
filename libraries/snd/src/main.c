...
#else
static SNDDriverInfo sDriverInfo[2];
#endif
...
void NNS_SndStopChannelAll(void) { SND_StopUnlockedChannel(0xffff, 0); }
...
BOOL NNSi_SndReadDriverTrackInfo(int playerNo, int trackNo,
                                 SNDTrackInfo *info) {
  const SNDDriverInfo *driverInfo;
...
static void EndSleep(void *) { NNSi_SndCaptureEndSleep(); }
