...
BOOL NNS_SndWaveOutStart(NNSSndWaveOutHandle handle, NNSSndWaveFormat format,
                         const void *dataaddr, BOOL loopFlag,
                         int loopStartSample, int samples, int sampleRate,
                         int volume, int speed, int pan) {
  int loopStart;
  int loopLen;
  u64 timer;
  ...
  NNS_MINMAX_ASSERT(volume, NNS_SND_WAVEOUT_VOLUME_MIN,
                    NNS_SND_WAVEOUT_VOLUME_MAX);
  NNS_MINMAX_ASSERT(pan, NNS_SND_WAVEOUT_PAN_MIN, NNS_SND_WAVEOUT_PAN_MAX);

  switch (format) {
  case NNS_SND_WAVE_FORMAT_PCM8:
    NNS_ASSERTMSG((loopStartSample & 0x03) == 0,
                  "loopStartSample must be a multiple of 4");
  ...
  case NNS_SND_WAVE_FORMAT_PCM16:
    NNS_ASSERTMSG((loopStartSample & 0x01) == 0,
                  "loopStartSample must be a multiple of 2");
  ...
  case NNS_SND_WAVE_FORMAT_ADPCM:
    NNS_ASSERTMSG((loopStartSample & 0x07) == 0,
                  "loopStartSample must be a multiple of 8");
  ...
  SND_SetupChannelPcm(
      handle->chNo, (SNDWaveFormat)format, dataaddr,
      loopFlag ? SND_CHANNEL_LOOP_REPEAT : SND_CHANNEL_LOOP_1SHOT, loopStart,
      loopLen, volume, SND_CHANNEL_DATASHIFT_NONE, (int)timer, pan);
  SND_StartTimer((u32)(1 << handle->chNo), 0, 0, 0);
...
void NNS_SndWaveOutStop(NNSSndWaveOutHandle handle) {
  ...
  SND_StopTimer((u32)(1 << handle->chNo), 0, 0, 0);
...
void NNS_SndWaveOutSetVolume(NNSSndWaveOutHandle handle, int volume) {
  ...
  SND_SetChannelVolume((u32)(1 << handle->chNo), volume,
...
void NNS_SndWaveOutSetSpeed(NNSSndWaveOutHandle handle, int speed) {
  ...
  SND_SetChannelTimer((u32)(1 << handle->chNo), (int)timer);
}
...
void NNS_SndWaveOutSetPan(NNSSndWaveOutHandle handle, int pan) {
  ...
  SND_SetChannelPan((u32)(1 << handle->chNo), pan);
}
