...
static NNS_SND_INLINE void MI_CpuCopy32(register const void *srcp,
                                        register void *destp,
                                        register u32 size) {
...
static const char *GetSymbol(const NNSSndArcOffsetTable *table, int index,
                             const void *base);
static void *GetPtr(void *base, u32 offset);
static const void *GetPtrConst(const void *base, u32 offset);
static const NNSSndArcOffsetTable *GetOffsetTable(const NNSSndArcInfo *info,
                                                  u32 offset);
...
static NNS_SND_INLINE const NNSSndArcOffsetTable *
GetOffsetTable(const NNSSndArcInfo *info, u32 offset) {
...
void NNS_SndArcInit(NNSSndArc *arc, const char *filePath, NNSSndHeapHandle heap,
                    BOOL symbolLoadFlag) {
  BOOL result;
...
BOOL NNS_SndArcInitWithResult(NNSSndArc *arc, const char *filePath,
                              NNSSndHeapHandle heap, BOOL symbolLoadFlag) {
  BOOL result;
...
BOOL NNS_SndArcSetup(NNSSndArc *arc, NNSSndHeapHandle heap,
                     BOOL symbolLoadFlag) {
  BOOL result;
  s32 readSize;
  ...
  if (readSize != sizeof(arc->header))
    return FALSE;
  NNS_ASSERT(arc->header.fileHeader.signature[0] == 'S' &&
             arc->header.fileHeader.signature[1] == 'D' &&
             arc->header.fileHeader.signature[2] == 'A' &&
             arc->header.fileHeader.signature[3] == 'T');
  NNS_ASSERT(arc->header.fileHeader.version >=
             NNS_SND_ARC_SUPPORTED_FILE_VERSION);

  if (heap != NNS_SND_HEAP_INVALID_HANDLE) {

    arc->info = (NNSSndArcInfo *)NNS_SndHeapAlloc(
        heap, arc->header.infoSize, InfoDisposeCallback, (u32)arc, 0);
    if (arc->info == NULL)
      return FALSE;
    result =
        FS_SeekFile(&arc->file, (s32)(arc->header.infoOffset), FS_SEEK_SET);
    if (!result)
      return FALSE;
    ...
    arc->fat = (NNSSndArcFat *)NNS_SndHeapAlloc(
        heap, arc->header.fatSize, FatDisposeCallback, (u32)arc, 0);
    if (arc->fat == NULL)
      return FALSE;
    ...
    if (symbolLoadFlag && arc->header.symbolDataSize > 0) {
      arc->symbol = (NNSSndArcSymbol *)NNS_SndHeapAlloc(
          heap, arc->header.symbolDataSize, SymbolDisposeCallback, (u32)arc, 0);
      if (arc->symbol == NULL)
        return FALSE;
      result = FS_SeekFile(&arc->file, (s32)(arc->header.symbolDataOffset),
                           FS_SEEK_SET);
      if (!result)
        return FALSE;

      readSize = FS_ReadFile(&arc->file, arc->symbol,
                             (s32)(arc->header.symbolDataSize));
...
void NNS_SndArcInitOnMemory(NNSSndArc *arc, void *data) {
  ...
  NNS_ASSERT(arc->info->blockHeader.kind == NNS_SND_ARC_SIGNATURE_INFO);
  NNS_ASSERT(arc->fat->blockHeader.kind == NNS_SND_ARC_SIGNATURE_FAT);
  NNS_ASSERT(arc->symbol == NULL ||
             arc->symbol->blockHeader.kind == NNS_SND_ARC_SIGNATURE_SYMB);
...
NNSSndArc *NNS_SndArcGetCurrent(void) { return sCurrent; }
...
const NNSSndArcSeqArcInfo *NNS_SndArcGetSeqArcInfo(int seqArcNo) {
  ...
  return (const NNSSndArcSeqArcInfo *)GetPtrConst(arc->info,
                                                  table->offset[seqArcNo]);
}
...
const NNSSndArcBankInfo *NNS_SndArcGetBankInfo(int bankNo) {
  ...
  return (const NNSSndArcBankInfo *)GetPtrConst(arc->info,
                                                table->offset[bankNo]);
}
...
const NNSSndArcWaveArcInfo *NNS_SndArcGetWaveArcInfo(int waveArcNo) {
  ...
  return (const NNSSndArcWaveArcInfo *)GetPtrConst(arc->info,
                                                   table->offset[waveArcNo]);
}
...
const NNSSndArcStrmInfo *NNS_SndArcGetStrmInfo(int strmNo) {
  ...
  return (const NNSSndArcStrmInfo *)GetPtrConst(arc->info,
                                                table->offset[strmNo]);
}
...
const NNSSndArcPlayerInfo *NNS_SndArcGetPlayerInfo(int playerNo) {
  ...
  return (const NNSSndArcPlayerInfo *)GetPtrConst(arc->info,
                                                  table->offset[playerNo]);
}
...
const NNSSndArcStrmPlayerInfo *NNS_SndArcGetStrmPlayerInfo(int playerNo) {
  ...
  return (const NNSSndArcStrmPlayerInfo *)GetPtrConst(arc->info,
                                                      table->offset[playerNo]);
}
...
const NNSSndArcGroupInfo *NNS_SndArcGetGroupInfo(int groupNo) {
  ...
  return (const NNSSndArcGroupInfo *)GetPtrConst(arc->info,
                                                 table->offset[groupNo]);
}
...
s32 NNS_SndArcReadFile(u32 fileId, void *buffer, s32 size, s32 offset) {
  ...
  NNS_ASSERTMSG(arc->file_open,
                "Cannot use this function for the Sound Archive initialized by "
  ...
  while (totalReadSize < size) {
    ...
    if (!FS_SeekFile(&arc->file, (s32)(file->offset + currentOffset),
                     FS_SEEK_SET)) {
      return -1;
    }
...
const char *NNS_SndArcGetSeqSymbol(int seqNo) {
  ...
  table = (const NNSSndArcOffsetTable *)GetPtrConst(arc->symbol,
                                                    arc->symbol->seqOffset);
  if (table == NULL)
    return &null_string;
...
const char *NNS_SndArcGetSeqArcSymbol(int seqArcNo) {
  ...
  table = (const NNSSndArcSeqArcOffsetTable *)GetPtrConst(
      arc->symbol, arc->symbol->seqArcOffset);
  if (table == NULL)
    return &null_string;
...
const char *NNS_SndArcGetSeqArcIdxSymbol(int seqArcNo, int index) {
  ...
  symbolTable = (const NNSSndArcOffsetTable *)GetPtrConst(
      arc->symbol, table->offset[seqArcNo].table);
  if (symbolTable == NULL)
    return &null_string;
