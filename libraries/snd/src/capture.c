...
BOOL NNS_SndCaptureStartReverb(void *buffer_p, u32 bufSize,
                               NNSSndCaptureFormat format, int sampleRate,
                               int volume) {
  void *bufL_p;
  void *bufR_p;

  NNS_NULL_ASSERT(buffer_p);
  NNS_ASSERTMSG(((u32)buffer_p & 31) == 0,
                "buffer address must be aligned to 32 bytes boundary.");
  NNS_ASSERTMSG(((u32)bufSize & 63) == 0,
                "buffer size must be aligned to 64 bytes boundary.");
  ...
  return NNSi_SndCaptureStart(NNS_SND_CAPTURE_TYPE_REVERB, bufL_p, bufR_p,
                              bufSize / 2, format, SND_CAPTURE_IN_MIXER,
...
BOOL NNS_SndCaptureStartEffect(void *buffer_p, u32 bufSize,
                               NNSSndCaptureFormat format, int sampleRate,
                               int interval,
  ...
  return NNSi_SndCaptureStart(NNS_SND_CAPTURE_TYPE_EFFECT, bufL_p, bufR_p,
                              bufSize / 2, format, SND_CAPTURE_IN_MIXER,
                              SND_CAPTURE_OUT_NORMAL,
                              TRUE, // loop
                              sampleRate, 127,
...
BOOL NNS_SndCaptureStartSampling(void *buffer_p, u32 bufSize,
                                 NNSSndCaptureFormat format, int sampleRate,
  ...
  return NNSi_SndCaptureStart(NNS_SND_CAPTURE_TYPE_SAMPLING, bufL_p, bufR_p,
                              bufSize / 2, format, SND_CAPTURE_IN_MIXER,
                              SND_CAPTURE_OUT_NORMAL,
                              TRUE, // loop
                              sampleRate, 127,
...
void NNS_SndCaptureCreateThread(u32 threadPrio) {
  ...
  OS_CreateThread(&sThread, CaptureThread, NULL,
                  sThreadStack + THREAD_STACK_SIZE / sizeof(u64),
                  THREAD_STACK_SIZE, threadPrio);
...
BOOL NNS_SndCaptureIsActive(void) { return sCapParam.activeFlag; }
...
void NNSi_SndCaptureMain(void) {
  ...
  if (cap->activeFlag && cap->type == NNS_SND_CAPTURE_TYPE_REVERB) {
    ...
    if (volume != cap->volume) {
      SND_SetChannelVolume(cap->playChBitMask, volume,
...
BOOL NNSi_SndCaptureStart(NNSSndCaptureType type, void *buffer0, void *buffer1,
                          u32 bufLen, NNSSndCaptureFormat format,
                          SNDCaptureIn input, SNDCaptureOut output,
                          BOOL loopFlag, int sampleRate, int volume, int pan0,
                          int pan1, int interval,
                          NNSSndCaptureCallback callback, void *arg) {
  ...
  NNS_ASSERTMSG(((u32)buffer0 & 0x1f) == 0,
                "buffer address must be aligned to 32 bytes boundary.");
  NNS_ASSERTMSG(((u32)buffer1 & 0x1f) == 0,
                "buffer address must be aligned to 32 bytes boundary.");
  NNS_ASSERTMSG((bufLen & 0x1f) == 0,
                "buffer size must be aligned to 32 bytes boundary.");
  ...
  if (callback != NULL) {
    NNS_ASSERTMSG(((bufLen / interval) & 0x1f) == 0,
                  "buffer size divided by inverval must be aligned to 32 bytes "
                  "boundary.");

    samples = bufLen;
    if (!is8bit)
      samples >>= 1;
    NNS_ASSERTMSG((samples % interval) == 0,
                  "interval must be a multiple of sample count.");
  ...
  chBitMask |=
      (1 << SND_CAPTURE_0_OUT_CHANNEL) | (1 << SND_CAPTURE_1_OUT_CHANNEL);
  ...
  SND_SetupChannelPcm(
      SND_CAPTURE_0_OUT_CHANNEL, wave_format, buffer0,
      loopFlag ? SND_CHANNEL_LOOP_REPEAT : SND_CHANNEL_LOOP_1SHOT, 0,
      (int)(bufLen >> 2), volume, SND_CHANNEL_DATASHIFT_NONE, timer, pan0);
  SND_SetupCapture(SND_CAPTURE_0, capture_format, buffer0, bufLen >> 2,
                   loopFlag, input, output);
  SND_SetupChannelPcm(
      SND_CAPTURE_1_OUT_CHANNEL, wave_format, buffer1,
      loopFlag ? SND_CHANNEL_LOOP_REPEAT : SND_CHANNEL_LOOP_1SHOT, 0,
      (int)(bufLen >> 2), volume, SND_CHANNEL_DATASHIFT_NONE, timer, pan1);
  SND_SetupCapture(SND_CAPTURE_1, capture_format, buffer1, bufLen >> 2,
                   loopFlag, input, output);

  if (alarmNo >= 0) {
    SND_SetupAlarm(alarmNo, alarmTimer + alarmOffset, alarmTimer, AlarmCallback,
                   cap);
  }

  if (type == NNS_SND_CAPTURE_TYPE_EFFECT) {
    SND_SetOutputSelector(SND_OUTPUT_CHANNEL1, SND_OUTPUT_CHANNEL3,
                          SND_CHANNEL_OUT_BYPASS, SND_CHANNEL_OUT_BYPASS);
  }

  SND_StartTimer(playChBitMask, capBitMask,
                 alarmNo >= 0 ? (u32)(1 << alarmNo) : 0, 0);
...
void NNSi_SndCaptureStop(void) {
  ...
  SND_StopTimer(cap->playChBitMask, cap->capBitMask,
                useAlarm ? (u32)(1 << cap->alarmNo) : 0, 0);
...
void NNSi_SndCaptureBeginSleep(void) {
  ...
  SND_StopTimer(cap->playChBitMask, cap->capBitMask,
                cap->alarmNo >= 0 ? (u32)(1 << cap->alarmNo) : 0, 0);
...
void NNSi_SndCaptureEndSleep(void) {
  ...
  SND_StartTimer(cap->playChBitMask, cap->capBitMask,
                 cap->alarmNo >= 0 ? (u32)(1 << cap->alarmNo) : 0, 0);
}
...
static void CaptureThread(void * /*arg*/) {
  ...
  while (1) {
    ...
    info->cap->callback(info->bufL_p, info->bufR_p, info->blockSize,
                        info->cap->format, info->cap->callbackArg);
  }
}
