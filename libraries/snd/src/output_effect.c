...
typedef void (*EffectFunc)(void *bufferL_p, void *bufferR_p, u32 len,
...
static CallbackInfo sCallbackInfo = {
    (NNSSndCaptureOutputEffectType)-1, NULL, NULL, NULL, NULL, NULL};

static void EffectCallback(void *bufferL_p, void *bufferR_p, u32 len,
                           NNSSndCaptureFormat format, void *arg);
static void SurroundProc(void *bufferL_p, void *bufferR_p, u32 len,
                         CallbackInfo *info);
static void NothingProc(void *bufferL_p, void *bufferR_p, u32 len,
                        CallbackInfo *info);
#ifndef SDK_SMALL_BUILD
static void HeadphoneProc(void *bufferL_p, void *bufferR_p, u32 len,
                          CallbackInfo *info);
static void MonoProc(void *bufferL_p, void *bufferR_p, u32 len,
                     CallbackInfo *info);
...
BOOL NNS_SndCaptureStartOutputEffect(void *buffer, u32 bufSize,
                                     NNSSndCaptureOutputEffectType type) {
  NNS_ASSERTMSG(((u32)buffer & 0x1f) == 0,
                "buffer address must be aligned to 32 bytes boundary.");
  NNS_ASSERTMSG((bufSize & 0x7f) == 0,
                "buffer size must be aligned to 128 bytes boundary.");

  NNS_SndCaptureChangeOutputEffect(type);

  return NNS_SndCaptureStartEffect(buffer, bufSize,
                                   NNS_SND_CAPTURE_FORMAT_PCM16, SAMPLING_RATE,
                                   2, EffectCallback, &sCallbackInfo);
}
...
void NNS_SndCaptureSetOutputEffectCallback(NNSSndCaptureCallback func,
                                           void *arg) {
...
void NNS_SndCaptureSetPostOutputEffectCallback(NNSSndCaptureCallback func,
                                               void *arg) {
  OSIntrMode enabled = OS_DisableInterrupts();
  ;
...
static void SurroundProc(void *bufferL_p, void *bufferR_p, u32 len,
  ...
  for (i = 0; i < SURROUND_MIX_DELAY; i++) {
    s32 temp = lp[i + samples - SURROUND_MIX_DELAY] -
               rp[i + samples - SURROUND_MIX_DELAY];
