...
static NNS_MCS_INLINE BOOL IsInitialized() { return spFileIOWork != NULL; }
...
static void OnDefaltOperation(const void *pRecv, u32 /*recvSize*/
) {
...
static void OnOpen(const void *pRecv, u32 /*recvSize*/
) {
...
static void OnRead(const void *pRecv, u32 recvSize) {
  ...
  if (cmd->result == NNSi_MCS_FILEIO_RESULT_READING ||
    ...
    (void)memcpy(cmd->pBuffer, (char *)pRecv + sizeof(NNSiMcsFileIOResultRead),
                 size);
  }
...
static void OnSeekFile(const void *pRecv, u32 /*recvSize*/
) {
...
static void OnFind(const void *pRecv, u32 /*recvSize*/, u16 type) {
...
static void RecvCallback(const void *pRecv, u32 recvSize, u32 /*userData*/,
...
static void DoLoop(NNSMcsFile *pFile) {
  do {
    if (!CheckConnect()) {
...
void NNS_McsInitFileIO(void *workMem) {
  ...
  NNS_McsRegisterRecvCallback(&spFileIOWork->cbInfo, NNSi_MCS_FILEIO_CHANNEL,
                              RecvCallback, 0);
}

void NNS_McsFinalizeFileIO() {
  if (!IsInitialized()) {
    return;
  }
...
u32 NNS_McsOpenFile(NNSMcsFile *pFile, const char *fileName, u32 openFlag) {
  NNSiMcsFileIOCmdOpen *const pCmd =
      (NNSiMcsFileIOCmdOpen *)spFileIOWork->sendBuffer;
...
u32 NNS_McsCloseFile(NNSMcsFile *pFile) {
  NNSiMcsFileIOCmdClose *const pCmd =
      (NNSiMcsFileIOCmdClose *)spFileIOWork->sendBuffer;
...
u32 NNS_McsReadFile(NNSMcsFile *pFile, void *buffer, u32 length,
                    u32 *pReadBytes) {
  NNSiMcsFileIOCmdRead *const pCmd =
      (NNSiMcsFileIOCmdRead *)spFileIOWork->sendBuffer;
...
u32 NNS_McsWriteFile(NNSMcsFile *pFile, const void *buffer, u32 length) {
  NNSiMcsFileIOCmdWrite *const pCmd =
      (NNSiMcsFileIOCmdWrite *)spFileIOWork->sendBuffer;
  const u8 *fromBuf = buffer;
  u8 *sendBuf = spFileIOWork->sendBuffer + sizeof(NNSiMcsFileIOCmdWrite);
  const u32 maxsize =
      sizeof(spFileIOWork->sendBuffer) - sizeof(NNSiMcsFileIOCmdWrite);
  ...
  while (length > 0) {
    ...
    if (!WriteStream(pFile, spFileIOWork->sendBuffer,
                     sendSize + sizeof(NNSiMcsFileIOCmdWrite))) {
      return pFile->errCode = NNS_MCS_FILEIO_ERROR_COMERROR;
    }
...
u32 NNS_McsSeekFile(NNSMcsFile *pFile, s32 distanceToMove, u32 moveMethod,
                    u32 *pNewFilePointer) {
  NNSiMcsFileIOCmdFileSeek *const pCmd =
      (NNSiMcsFileIOCmdFileSeek *)spFileIOWork->sendBuffer;
...
u32 NNS_McsFindFirstFile(NNSMcsFile *pFile, NNSMcsFileFindData *pFindData,
                         const char *pattern) {
  NNSiMcsFileIOCmdFindFirst *const pCmd =
      (NNSiMcsFileIOCmdFindFirst *)spFileIOWork->sendBuffer;
...
u32 NNS_McsFindNextFile(NNSMcsFile *pFile, NNSMcsFileFindData *pFindData) {
  NNSiMcsFileIOCmdFindNext *const pCmd =
      (NNSiMcsFileIOCmdFindNext *)spFileIOWork->sendBuffer;
