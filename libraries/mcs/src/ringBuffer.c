...
#if defined(_WIN32)
#define GetVarFromRBMem(app, rb)                                               \
  NNSi_McsReadFromRingBufferMem(&(app), (uint32_t)&(rb), sizeof(app))
#define SetVarToRBMem(rb, app)                                                 \
  NNSi_McsWriteToRingBufferMem((uint32_t)&(rb), &(app), sizeof(rb))

#define ReadFromRBMem(appAddr, rbAddr, bytes)                                  \
  NNSi_McsReadFromRingBufferMem(appAddr, rbAddr, bytes)
#define WriteToRBMem(rbAddr, appAddr, bytes)                                   \
  NNSi_McsWriteToRingBufferMem(rbAddr, appAddr, bytes)
#else
#define GetVarFromRBMem(app, rb) ((app) = (rb))
#define SetVarToRBMem(rb, app) ((rb) = (app))

#define ReadFromRBMem(appAddr, rbAddr, bytes)                                  \
  MI_CpuCopy8((const void *)rbAddr, appAddr, bytes)
#define WriteToRBMem(rbAddr, appAddr, bytes)                                   \
  MI_CpuCopy8(appAddr, (void *)rbAddr, bytes)
#endif
...
static NNS_MCS_INLINE NNSiMcsUIntPtr GetRingBufferPtrInc(
    NNSiMcsUIntPtr ptr, uint32_t val, const NNSiMcsBufRgn *pBRgn) {
  ptr += val;
...
static NNS_MCS_INLINE NNSiMcsUIntPtr GetRingBufferPtrDec(
    NNSiMcsUIntPtr ptr, uint32_t val, const NNSiMcsBufRgn *pBRgn) {
  ptr -= val;
...
static uint32_t GetRingBufferWritableBytes(const NNSiMcsMsgRange *pMRng,
                                           uint32_t bufSize) {
...
static void GetRingBufferHeader(NNSiMcsBufRgn *pBrgn, NNSiMcsMsgRange *pMrng,
                                NNSMcsRingBuffer rb) {
  const NNSiMcsRingBufferHeader *pRbh =
      (NNSiMcsRingBufferHeader *)rb; // target address

  DISABLE_IRQ_BLOCK((GetVarFromRBMem(pBrgn->buf, pRbh->brgn.buf),
...
static NNSiMcsUIntPtr WriteBuffer(NNSiMcsUIntPtr pDst, const uint8_t *pSrc,
                                  uint32_t size, const NNSiMcsBufRgn *pBrgn) {
  while (size > 0) {
    const uint32_t writeByte =
        GetMin(GetRingBufferBufferEnd(pBrgn) - pDst, size);
...
static void SetState(NNSMcsRingBuffer rb, uint32_t addState) {
  NNSiMcsRingBufferHeader *pRbh =
      (NNSiMcsRingBufferHeader *)rb; // target address
  uint32_t state;

  DISABLE_IRQ_BLOCK((GetVarFromRBMem(state, pRbh->state), state |= addState,
...
NNSMcsRingBuffer NNS_McsInitRingBuffer(NNSiMcsBufPtr buf, uint32_t bufSize) {
  ...
  NNSiMcsUIntPtr startBuf =
      (NNSiMcsUIntPtr)buf + sizeof(NNSiMcsRingBufferHeader);
  ...
  SetVarToRBMem(pRbh->signature,
                signature); // Lastly, set signature (completed initialization).
...
uint32_t NNS_McsGetRingBufferReadableBytes(NNSMcsRingBuffer rb) {
  ...
  DISABLE_IRQ_BLOCK((GetVarFromRBMem(mrng.start, pRbh->mrng.start),
...
uint32_t NNS_McsGetRingBufferTotalReadableBytes(NNSMcsRingBuffer rb) {
  ...
  DISABLE_IRQ_BLOCK((GetVarFromRBMem(brgn.buf, pRgh->brgn.buf),
...

  {
    ...
    while (msgPtr != mrng.end) {
      ...
      total += size;
      msgPtr = GetRingBufferPtrInc(msgPtr, sizeof(uint32_t) + RoundUp(size, 4),
                                   &brgn);
    }
...
uint32_t NNS_McsGetRingBufferWritableBytes(NNSMcsRingBuffer rb) {
  ...
  DISABLE_IRQ_BLOCK((GetVarFromRBMem(bufSize, pRbh->brgn.bufSize),
                     GetVarFromRBMem(mrng.end, pRbh->mrng.end),
...
BOOL NNS_McsReadRingBuffer(NNSMcsRingBuffer rb, void *buf, uint32_t size,
                           uint32_t *pReadBytes) {
  NNSiMcsRingBufferHeader *pRbh =
      (NNSiMcsRingBufferHeader *)rb; // target address
  NNSiMcsBufRgn brgn;
  ...
  DISABLE_IRQ_BLOCK((GetVarFromRBMem(brgn.buf, pRbh->brgn.buf),
...

  {
    ...
    NNSiMcsUIntPtr pSrc =
        GetRingBufferPtrInc(mrng.start, sizeof(uint32_t), &brgn);
    uint32_t restSize;
    ...
    *pReadBytes = restSize;
    ...
    while (restSize > 0) {
      const uint32_t readByte =
          GetMin(GetRingBufferBufferEnd(&brgn) - pSrc, restSize);
    ...
    if (*pReadBytes != readableBytes) {
      ...
      pSrc = GetRingBufferPtrDec(pSrc, sizeof(uint32_t),
                                 &brgn);    // go back by uint32_t
...
BOOL NNS_McsWriteRingBuffer(NNSMcsRingBuffer rb, const void *buf,
                            uint32_t size) {
  NNSiMcsBufRgn brgn;
  NNSiMcsMsgRange mrng;
...

  {
    NNSiMcsRingBufferHeader *pRbh =
        (NNSiMcsRingBufferHeader *)rb; // target address
    NNSiMcsUIntPtr pDst =
        GetRingBufferPtrInc(mrng.end, sizeof(uint32_t), &brgn);
...
BOOL NNS_McsWriteRingBufferEx(NNSMcsRingBuffer rb, const void *buf,
  ...
  {
    NNSiMcsUIntPtr pDst =
        GetRingBufferPtrInc(mrng.end, sizeof(uint32_t) + offset, &brgn);
    pDst = WriteBuffer(pDst, (const uint8_t *)buf, size, &brgn);
...
void NNS_McsClearRingBuffer(NNSMcsRingBuffer rb) {
  ...
  DISABLE_IRQ_BLOCK((GetVarFromRBMem(endPtr, pRbh->mrng.end),
