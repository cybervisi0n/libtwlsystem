...
void NNS_GfdDumpLnkPlttVramManagerEx(NNSGfdLnkDumpCallBack pFunc,
                                     void *pUserData) {
...
void NNS_GfdInitLnkPlttVramManager(u32 szByte, void *pManagementWork,
                                   u32 szByteManagementWork,
                                   BOOL useAsDefault) {
...
#pragma unused(opt)
  ...
  if (b4Pltt) {
    result = NNSi_GfdAllocLnkVramAligned(&mgr_.mgr, &mgr_.pBlockPoolList, &addr,
                                         szByte, 0x08);
...
int NNS_GfdFreeLnkPlttVram(NNSGfdPlttKey plttKey) {
  ...
  const BOOL result =
      NNSi_GfdFreeLnkVram(&mgr_.mgr, &mgr_.pBlockPoolList, addr, szByte);
...
void NNS_GfdResetLnkPlttVramState(void) {
...

  {
    ...
    result = NNSi_GfdAddNewFreeBlock(&mgr_.mgr, &mgr_.pBlockPoolList, 0,
                                     mgr_.szByte);
