...
static NNS_GFD_INLINE BOOL AllocFromLo_(u32 szByte, BOOL b4Pltt,
                                        u32 *pResAddr) {
  const u32 addr = s_managerState_.loAddr;
  const u32 alignment = GetUpperSideAlignmentSize_(
      addr, b4Pltt); // Perform alignment best suited to the format.
...
static NNS_GFD_INLINE BOOL AllocFromHi_(u32 szByte, BOOL b4Pltt,
                                        u32 *pResAddr) {
  NNS_GFD_ASSERT(szByte != 0);
  NNS_GFD_ASSERT(IsVramManagerValid_());
...
static void FrmPlttVramDebugDumpCallBack_(u32 loAddr, u32 hiAddr, u32 szFree,
                                          u32 szTotal) {
#ifdef SDK_FINALROM
...
#pragma unused(szTotal)
  ...
  OS_Printf("    %08d / %08d bytes (%6.2f%%) used \n", szTotal - szFree,
            szTotal, 100.0f * (szTotal - szFree) / szTotal);
  OS_Printf("==========================================\n");
}
...
void NNS_GfdDumpFrmPlttVramManagerEx(NNSGfdFrmPlttVramDebugDumpCallBack pFunc) {
  ...
  (*pFunc)(s_managerState_.loAddr, s_managerState_.hiAddr,
...
NNSGfdPlttKey NNS_GfdAllocFrmPlttVram(u32 szByte, BOOL bPltt4, u32 allocFrom) {
...

  {
    ...
    if (szByte >= NNS_GFD_PLTTSIZE_MAX) {
      NNS_GFD_WARNING(
          "Allocation size is too big. : NNS_GfdAllocLnkPlttVram()");
