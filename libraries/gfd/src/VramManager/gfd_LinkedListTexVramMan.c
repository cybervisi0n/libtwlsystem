...
static NNS_GFD_INLINE BOOL InitSlotFreeBlock_(NNSiGfdLnkVramMan *pMgr,
...
void NNS_GfdDumpLnkTexVramManager() {
  ...
  if (mgr_.szByteFor4x4 != 0) {
    NNSi_GfdDumpLnkVramManFreeListInfo(mgr_.mgr4x4.pFreeList,
                                       mgr_.szByteFor4x4);
  }
  OS_Printf("=======================================\n");
}

void NNS_GfdDumpLnkTexVramManagerEx(NNSGfdLnkDumpCallBack pFuncForNrm,
                                    NNSGfdLnkDumpCallBack pFuncFor4x4,
                                    void *pUserData) {
  NNS_GFD_ASSERT(pFuncForNrm != NULL || pFuncFor4x4 != NULL);

  if (pFuncForNrm != NULL) {
    NNSi_GfdDumpLnkVramManFreeListInfoEx(mgr_.mgrNrm.pFreeList, pFuncForNrm,
                                         pUserData);
  }

  if (mgr_.szByteFor4x4 != 0 && pFuncFor4x4 != NULL) {
    NNSi_GfdDumpLnkVramManFreeListInfoEx(mgr_.mgr4x4.pFreeList, pFuncFor4x4,
                                         pUserData);
  }
}
...
void NNS_GfdInitLnkTexVramManager(u32 szByte, u32 szByteFor4x4,
                                  void *pManagementWork,
                                  u32 szByteManagementWork, BOOL useAsDefault) {
...
#pragma unused(opt)
  ...
  if (is4x4comp) {
    result =
        NNSi_GfdAllocLnkVram(&mgr_.mgr4x4, &mgr_.pBlockPoolList, &addr, szByte);
  } else {
    result =
        NNSi_GfdAllocLnkVram(&mgr_.mgrNrm, &mgr_.pBlockPoolList, &addr, szByte);
  }
...
int NNS_GfdFreeLnkTexVram(NNSGfdTexKey memKey) {
  ...
  if (szByte != 0) {
    if (b4x4) {
      result =
          NNSi_GfdFreeLnkVram(&mgr_.mgr4x4, &mgr_.pBlockPoolList, addr, szByte);
    } else {
      result =
          NNSi_GfdFreeLnkVram(&mgr_.mgrNrm, &mgr_.pBlockPoolList, addr, szByte);
    }
...
void NNS_GfdResetLnkTexVramState(void) {
  SlotData sd[4] = {
      ...
      {0x20000, 0, 0}, {0x20000, 0, 0}, {0x20000, 0, 0}, {0x20000, 0, 0}};
...

  {
...

    {
      ...
      result &= InitSlotFreeBlock_(&mgr_.mgr4x4, &mgr_.pBlockPoolList,
                                   GFD_SLOT0_BASEADDR, sd[0].sz4x4);
      result &=
          InitSlotFreeBlock_(&mgr_.mgrNrm, &mgr_.pBlockPoolList,
                             GFD_SLOT0_BASEADDR + sd[0].sz4x4, sd[0].szNrm);

      result &= InitSlotFreeBlock_(&mgr_.mgr4x4, &mgr_.pBlockPoolList,
                                   GFD_SLOT2_BASEADDR, sd[2].sz4x4);
      result &=
          InitSlotFreeBlock_(&mgr_.mgrNrm, &mgr_.pBlockPoolList,
                             GFD_SLOT2_BASEADDR + sd[2].sz4x4, sd[2].szNrm);

      NNS_GFD_ASSERT(sd[3].sz4x4 == 0);
      result &= InitSlotFreeBlock_(&mgr_.mgrNrm, &mgr_.pBlockPoolList,
                                   GFD_SLOT3_BASEADDR, sd[3].szNrm);

      result &=
          InitSlotFreeBlock_(&mgr_.mgrNrm, &mgr_.pBlockPoolList,
                             GFD_SLOT1_BASEADDR + szIndexTbl, sd[1].szNrm);
    }
