...
static NNSGfdFrmTexRegionState vramRegions_[NNS_GFD_NUM_TEX_VRAM_REGION] = {
    ...
    {NNS_GFD_INVALID_ADDR, NNS_GFD_INVALID_ADDR, FALSE, TRUE, 1, 0xFFFF,
     (u32)NNS_GFD_TEX_SLOT_SIZE},
    {NNS_GFD_INVALID_ADDR, NNS_GFD_INVALID_ADDR, FALSE, TRUE, 2, 0xFFFF,
     (u32)(NNS_GFD_TEX_SLOT_SIZE + NNS_GFD_TEX_SLOT_SIZE / 2)},
    {NNS_GFD_INVALID_ADDR, NNS_GFD_INVALID_ADDR, FALSE, FALSE, 3, 0xFFFF,
     (u32)(NNS_GFD_TEX_SLOT_SIZE * 2)},
    {NNS_GFD_INVALID_ADDR, NNS_GFD_INVALID_ADDR, FALSE, FALSE, 4, 0xFFFF,
     (u32)(NNS_GFD_TEX_SLOT_SIZE * 3)}};

static NNSGfdFrmTexRegionState
    *texNrmSearchArray_[NNS_GFD_NUM_TEX_VRAM_REGION] = {
        &vramRegions_[4], &vramRegions_[3], &vramRegions_[0], &vramRegions_[2],
        &vramRegions_[1]};

static NNSGfdFrmTexRegionState
    *tex4x4SearchArray_[NNS_GFD_NUM_TEX_VRAM_REGION_4x4] = {&vramRegions_[0],
                                                            &vramRegions_[3]};
...
ResetRegionNormal_(NNSGfdFrmTexRegionState *pRegion) {
...
static NNS_GFD_INLINE BOOL
IsRegionStateValid_(const NNSGfdFrmTexRegionState *pRegion) {
  NNS_GFD_NULL_ASSERT(pRegion);
...
static NNS_GFD_INLINE u32
GetReagionCapacity_(const NNSGfdFrmTexRegionState *pRegion) {
  NNS_GFD_NULL_ASSERT(pRegion);
...
static NNS_GFD_INLINE u32
AllocFromReagionHead_(NNSGfdFrmTexRegionState *pRegion, u32 szByte) {
  NNS_GFD_NULL_ASSERT(pRegion);
  NNS_GFD_ASSERT(IsRegionStateValid_(pRegion));
...
static NNS_GFD_INLINE u32
AllocFromReagionTail_(NNSGfdFrmTexRegionState *pRegion, u32 szByte) {
  NNS_GFD_NULL_ASSERT(pRegion);
  NNS_GFD_ASSERT(IsRegionStateValid_(pRegion));
  NNS_GFD_ASSERT(GetReagionCapacity_(pRegion) >= szByte);
...
static NNS_GFD_INLINE NNSGfdFrmTexRegionState *
...
static NNS_GFD_INLINE BOOL DoAlloc4x4_(u32 szByte, u32 *addr) {
  ...
  NNS_GFD_ASSERTMSG(IsVramManagerValid_(),
                    "Make sure to Initialize the manager.");

  {
    ...
    for (i = 0; i < NNS_GFD_NUM_TEX_VRAM_REGION_4x4; i++) {
      ...
      if (pRegion->bActive && GetReagionCapacity_(pRegion) >= szByte) {
...
static void DefaultDebugDumpFunc_(int index, u32 startAddr, u32 endAddr,
                                  u32 blockMax, BOOL bActive,
                                  void *pUserContext) {
...
#pragma unused(index)
  ...
  NNSiGfdDefaultDebugContext *pContext =
      (NNSiGfdDefaultDebugContext *)pUserContext;
...
void NNSi_GfdSetTexNrmSearchArray(int idx1st, int idx2nd, int idx3rd,
                                  int idx4th, int idx5th) {
...
void NNS_GfdDumpFrmTexVramManager() {
  ...
  OS_Printf("    %08d / %08d bytes (%6.2f%%) used \n",
            ...
            100.0f * (context.szTotalReserved - context.szTotalFree) /
                context.szTotalReserved);

  OS_Printf("==========================================\n");
}

void NNS_GfdDumpFrmTexVramManagerEx(NNSGfdFrmTexVramDebugDumpCallBack pFunc,
                                    void *pUserContext) {
  int i;
  const NNSGfdFrmTexRegionState *pState = NULL;
  ...
  for (i = 0; i < NNS_GFD_NUM_TEX_VRAM_REGION; i++) {
    ...
    (*pFunc)(i, pState->head + pState->baseAddress,
             pState->tail + pState->baseAddress,
             (pState->bHalfSize) ? (u32)(NNS_GFD_TEX_SLOT_SIZE / 2)
                                 : NNS_GFD_TEX_SLOT_SIZE,
             pState->bActive, pUserContext);
  }
}
...
void NNS_GfdResetFrmTexVramState() {
  ...
  const
      ...
      int
#endif
          numRegion = (numSlot > 1) ? numSlot + 1 : numSlot + 0;
