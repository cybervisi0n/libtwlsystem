...
BOOL NNS_FndMountArchive(NNSFndArchive *archive, const char *arcName,
  ...
  {
    int count;
    NNSiFndArchiveBlockHeader *block =
        (NNSiFndArchiveBlockHeader *)((u32)arc + arc->headerSize);
    ...
    if (!FS_RegisterArchiveName(&archive->fsArchive, arcName,
                                strlen(arcName))) {
      OS_Printf("archive cannot register to file system.\n");
      return FALSE;
    }
    if (!FS_LoadArchive(&archive->fsArchive, (u32)(img + 1),
                        (u32)(fat + 1) - (u32)(img + 1),
                        fat->blockSize - sizeof(NNSiFndArchiveFatBlockHeader),
                        (u32)(fnt + 1) - (u32)(img + 1),
                        fnt->blockSize - sizeof(NNSiFndArchiveDirBlockHeader),
                        NULL, NULL)) {
...
void *NNS_FndGetArchiveFileByName(const char *path) {
  ...
  if (FS_ConvertPathToFileID(&fileID, path)) {
    ...
    addr = (void *)(archive->fileImage +
                    archive->fatData->fatEntries[fileID.file_id].fileTop);
  }
#endif
...
void *NNS_FndGetArchiveFileByIndex(NNSFndArchive *archive, u32 index) {
  ...
  if (index < archive->fatData->numFiles) {
    addr = (void *)(archive->fileImage +
                    archive->fatData->fatEntries[index].fileTop);
  }
  return addr;
}
