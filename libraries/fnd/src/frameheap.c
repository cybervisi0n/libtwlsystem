...
static NNSiFndHeapHead *InitFrameHeap(void *startAddress, void *endAddress,
                                      u16 optFlag) {
...
static void *AllocFromHead(NNSiFndFrmHeapHead *pFrmHeapHd, u32 size,
                           int alignment) {
  ...
  FillAllocMemory( // fill memory
      GetHeapHeadPtrFromFrmHeapHead(pFrmHeapHd), pFrmHeapHd->headAllocator,
...
static void *AllocFromTail(NNSiFndFrmHeapHead *pFrmHeapHd, u32 size,
                           int alignment) {
  void *newBlock = NNSi_FndRoundDownPtr(
      SubU32ToPtr(pFrmHeapHd->tailAllocator, size), alignment);
  ...
  FillAllocMemory( // fill memory
      GetHeapHeadPtrFromFrmHeapHead(pFrmHeapHd), newBlock,
...
static void FreeTail(NNSiFndHeapHead *pHeapHd) {
  ...
  FillFreeMemory(pHeapHd, pFrmHeapHd->tailAllocator,
...
void NNSi_FndDumpFrmHeap(NNSFndHeapHandle heap) {
...

  {
    ...
    NNSiFndFrmHeapHead *const pFrmHeapHd =
        GetFrmHeapHeadPtrFromHeapHead(pHeapHd);
    const u32 heapSize = GetOffsetFromPtr(pHeapHd->heapStart, pHeapHd->heapEnd);

    NNSi_FndDumpHeapHead(pHeapHd);

    OS_Printf("     head [%p - %p) ", pHeapHd->heapStart,
              pFrmHeapHd->headAllocator);
    PrintSize(GetOffsetFromPtr(pHeapHd->heapStart, pFrmHeapHd->headAllocator),
              heapSize);
    OS_Printf("\n     free                           ");
    PrintSize(
        GetOffsetFromPtr(pFrmHeapHd->headAllocator, pFrmHeapHd->tailAllocator),
        heapSize);
    OS_Printf("\n     tail [%p - %p) ", pFrmHeapHd->tailAllocator,
              pHeapHd->heapEnd);
    PrintSize(GetOffsetFromPtr(pFrmHeapHd->tailAllocator, pHeapHd->heapEnd),
              heapSize);
    OS_Printf("\n");
    ...
    if (pFrmHeapHd->pState) {
      ...
      for (pState = pFrmHeapHd->pState; pState; pState = pState->pPrevState) {
        OS_Printf("            '%c%c%c%c' : %p %p\n", pState->tagName >> 24,
                  (pState->tagName >> 16) & 0xFF, (pState->tagName >> 8) & 0xFF,
                  pState->tagName & 0xFF, pState->headAllocator,
                  pState->tailAllocator);
      }
    }
...
NNSFndHeapHandle NNS_FndCreateFrmHeapEx(void *startAddress, u32 size,
                                        u16 optFlag) {
  void *endAddress;
  ...
  if (NNSiGetUIntPtr(startAddress) > NNSiGetUIntPtr(endAddress) ||
      GetOffsetFromPtr(startAddress, endAddress) <
          sizeof(NNSiFndHeapHead) + sizeof(NNSiFndFrmHeapHead)) {
...
void *NNS_FndAllocFromFrmHeapEx(NNSFndHeapHandle heap, u32 size,
                                int alignment) {
  void *memory = NULL;
...
u32 NNS_FndGetAllocatableSizeForFrmHeapEx(NNSFndHeapHandle heap,
                                          int alignment) {
  NNS_ASSERT(IsValidFrmHeapHandle(heap));
...

  {
    ...
    const void *block =
        NNSi_FndRoundUpPtr(pFrmHeapHd->headAllocator, alignment);
...
BOOL NNS_FndRecordStateForFrmHeap(NNSFndHeapHandle heap, u32 tagName) {
...

  {
    ...
    NNSiFndFrmHeapState *pState =
        AllocFromHead(pFrmHeapHd, sizeof(NNSiFndFrmHeapState), MIN_ALIGNMENT);
    if (!pState) {
      return FALSE;
    }
...
u32 NNS_FndResizeForMBlockFrmHeap(NNSFndHeapHandle heap, void *memBlock,
                                  u32 newSize) {
  ...
  NNS_ASSERT(IsValidFrmHeapHandle(heap));
  NNS_ASSERT(memBlock ==
             NNSi_FndRoundDownPtr(
                 memBlock, MIN_ALIGNMENT)); // check if at the minimum alignment

  pHeapHd = heap;
  pFrmHeapHd = GetFrmHeapHeadPtrFromHeapHead(pHeapHd);

  NNS_ASSERT(ComparePtr(pHeapHd->heapStart, memBlock) <= 0 &&
             ComparePtr(pFrmHeapHd->headAllocator, memBlock) >
                 0); // be sure that memory blocks exist at the front
  NNS_ASSERT(
      pFrmHeapHd->pState == NULL ||
      ComparePtr(pFrmHeapHd->pState, memBlock) <
          0); // be sure that there is no status saved after the memory block
...

  {
    ...
    {
      if (ComparePtr(endAddress, pFrmHeapHd->tailAllocator) >
          0) // when the size is insufficient
      {
        return 0;
      }
