...
static NNS_FND_INLINE void PushMBlock(NNSiFndUntMBlockList *list,
...
NNSFndHeapHandle NNS_FndCreateUnitHeapEx(void *startAddress, u32 heapSize,
  ...
  heapEnd =
      NNSi_FndRoundDownPtr(AddU32ToPtr(startAddress, heapSize), MIN_ALIGNMENT);
...

  {
    ...
    void *heapStart = NNSi_FndRoundUpPtr(
        AddU32ToPtr(pUntHeapHd, sizeof(NNSiFndUntHeapHead)), alignment);
    u32 elementNum;
    ...
    NNSi_FndInitHeapHead( // common heap initializations
        pHeapHd, NNSI_UNTHEAP_SIGNATURE, heapStart, heapEnd, optFlag);
