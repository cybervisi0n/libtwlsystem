...
#define HEAP_WARNING(exp, ...) (void)((exp) && (OS_Printf(__VA_ARGS__), 0))
...
static NNS_FND_INLINE void
SetAlignmentForMBlock(NNSiFndExpHeapMBlockHead *pMBlkHd, u16 alignment) {
...
static NNS_FND_INLINE void
SetGroupIDForMBlock(NNSiFndExpHeapMBlockHead *pMBHead, u16 id) {
...
static NNS_FND_INLINE void
SetAllocDirForMBlock(NNSiFndExpHeapMBlockHead *pMBHead, u16 mode) {
...
static void GetRegionOfMBlock(NNSiMemRegion *region,
...
static NNSiFndExpHeapMBlockHead *RemoveMBlock(NNSiFndExpMBlockList *list,
                                              NNSiFndExpHeapMBlockHead *block) {
...
static NNSiFndExpHeapMBlockHead *InsertMBlock(NNSiFndExpMBlockList *list,
...
static NNS_FND_INLINE void AppendMBlock(NNSiFndExpMBlockList *list,
                                        NNSiFndExpHeapMBlockHead *block) {
  (void)InsertMBlock(list, block, list->tail);
}

static NNSiFndExpHeapMBlockHead *InitMBlock(const NNSiMemRegion *pRegion,
                                            u16 signature) {
...
static NNS_FND_INLINE NNSiFndExpHeapMBlockHead *
InitFreeMBlock(const NNSiMemRegion *pRegion) {
  return InitMBlock(pRegion, MBLOCK_FREE_SIGNATURE);
}

static NNSiFndHeapHead *InitExpHeap(void *startAddress, void *endAddress,
                                    u16 optFlag) {
  ...
  NNSi_FndInitHeapHead( // common heap initializations
      pHeapHd, NNSI_EXPHEAP_SIGNATURE,
...
static void *AllocUsedBlockFromFreeBlock(NNSiFndExpHeapHead *pEHHead,
                                         NNSiFndExpHeapMBlockHead *pMBHeadFree,
                                         void *mblock, u32 size,
                                         u16 direction) {
  ...
  pMBHeadFreePrev =
      RemoveMBlock(&pEHHead->mbFreeList,
                   pMBHeadFree); // delete the free block for the time being

  if (GetOffsetFromPtr(freeRgnT.start, freeRgnT.end) <
      sizeof(NNSiFndExpHeapMBlockHead) + MIN_FREE_BLOCK_SIZE) {
    freeRgnT.end =
        freeRgnT.start; // include the alignment value for the block being used
  } else {
    pMBHeadFreePrev = InsertMBlock(&pEHHead->mbFreeList,
                                   InitFreeMBlock(&freeRgnT), pMBHeadFreePrev);
  }

  if (GetOffsetFromPtr(freeRgnB.start, freeRgnB.end) <
      sizeof(NNSiFndExpHeapMBlockHead) + MIN_FREE_BLOCK_SIZE) {
    freeRgnB.start = freeRgnB.end; // include the block being used
  } else {
    (void)InsertMBlock(&pEHHead->mbFreeList, InitFreeMBlock(&freeRgnB),
                       pMBHeadFreePrev);
  }

  FillAllocMemory(GetHeapHeadPtrFromExpHeapHead(pEHHead), freeRgnT.end,
...

  {
    ...
    SetAlignmentForMBlock(pMBHeadNewUsed,
                          (u16)GetOffsetFromPtr(freeRgnT.end, pMBHeadNewUsed));
...
static void *AllocFromHead(NNSiFndHeapHead *pHeapHd, u32 size, int alignment) {
  ...
  const BOOL bAllocFirst =
      GetAllocMode(pExpHeapHd) == NNS_FND_EXPHEAP_ALLOC_MODE_FIRST;
  ...
  for (pMBlkHd = pExpHeapHd->mbFreeList.head; pMBlkHd;
       pMBlkHd = pMBlkHd->pMBHeadNext) {
  ...
  return AllocUsedBlockFromFreeBlock( // allocate a region from the free block
                                      // that was found
      pExpHeapHd, pMBlkHdFound, foundMBlock, size,
...
static void *AllocFromTail(NNSiFndHeapHead *pHeapHd, u32 size, int alignment) {
  ...
  for (pMBlkHd = pExpHeapHd->mbFreeList.tail; pMBlkHd;
       pMBlkHd = pMBlkHd->pMBHeadPrev) {
    void *const mblock = GetMemPtrForMBlock(pMBlkHd);
    void *const mblockEnd = AddU32ToPtr(mblock, pMBlkHd->blockSize);
    void *const reqMBlock = NNSi_FndRoundDownPtr(SubU32ToPtr(mblockEnd, size),
                                                 alignment); // aligned address
...
static BOOL RecycleRegion(NNSiFndExpHeapHead *pEHHead,
                          const NNSiMemRegion *pRegion) {
  NNSiFndExpHeapMBlockHead *pBlkPrFree =
      NULL; // immediately preceding free block
...

  {
    ...
    for (pBlk = pEHHead->mbFreeList.head; pBlk; pBlk = pBlk->pMBHeadNext) {
      ...
      if (pBlk == pRegion->end) // Is the block adjacent to the end?
      {
        ...
        FillNoUseMemory(GetHeapHeadPtrFromExpHeapHead(pEHHead), pBlk,
  ...
  if (GetOffsetFromPtr(freeRgn.start, freeRgn.end) <
      sizeof(
          NNSiFndExpHeapMBlockHead)) // size is not suitable for use as a block
  {
    return FALSE; // arrive here when attempting to shrink to a small size using
                  // the NNS_FndResizeForMBlockExpHeap function and when there
                  // are no free blocks at the end
  }

  FillFreeMemory(GetHeapHeadPtrFromExpHeapHead(pEHHead), pRegion->start,
                 GetOffsetFromPtr(pRegion->start, pRegion->end));
...
static BOOL CheckMBlock(const NNSiFndExpHeapMBlockHead *pMBHead,
                        NNSiFndHeapHead *pHeapHd, u16 signature,
                        const char *heapType, u32 flag) {
  ...
  if (pHeapHd) {
    if (NNSiGetUIntPtr(pMBHead) < NNSiGetUIntPtr(pHeapHd->heapStart) ||
      ...
      HEAP_WARNING(bPrint,
                   ...
                   "Exp"
                   " Heap]"
                   " Bad %s memory block address. - address %08X, heap area "
                   "[%08X - %08X)\n",
    ...
    if (NNSiGetUIntPtr(pMBHead) >= 0x11000000) {
      HEAP_WARNING(bPrint,
                   ...
                   "Exp"
                   " Heap]"
                   " Bad %s memory block address. - address %08X\n",
                   heapType, memBlock);
      return FALSE;
    }
  }
  ...
  if (pMBHead->signature != signature) // Is the signature different?
  {
    HEAP_WARNING(
        ...
        "Exp"
        " Heap]"
        " Bad %s memory block signature. - address %08X, signature %04X\n",
  ...
  if (pMBHead->blockSize >= 0x01000000) // Size too big?
  {
    HEAP_WARNING(
        ...
        "Exp"
        " Heap]"
        " Too large %s memory block. - address %08X, block size %08X\n",
        heapType, memBlock, pMBHead->blockSize);
    return FALSE;
  }

  if (pHeapHd) {
    if (NNSiGetUIntPtr(memBlock) + pMBHead->blockSize >
        NNSiGetUIntPtr(pHeapHd->heapEnd)) {
      HEAP_WARNING(
          bPrint,
          "[NNS Foundation "
          "Exp"
          " Heap]"
          " wrong size %s memory block. - address %08X, block size %08X\n",
          heapType, memBlock, pMBHead->blockSize);
      return FALSE;
    }
  }
...
static NNS_FND_INLINE
    BOOL CheckUsedMBlock(const NNSiFndExpHeapMBlockHead *pMBHead,
                         NNSiFndHeapHead *pHeapHd, u32 flag) {
...
static NNS_FND_INLINE
    BOOL CheckFreeMBlock(const NNSiFndExpHeapMBlockHead *pMBHead,
                         NNSiFndHeapHead *pHeapHd, u32 flag) {
...
static BOOL CheckMBlockPrevPtr(const NNSiFndExpHeapMBlockHead *pMBHead,
  ...
  if (pMBHead->pMBHeadPrev != pMBHeadPrev) {
    HEAP_WARNING(bPrint,
                 ...
                 "Exp"
                 " Heap]"
                 " Wrong link memory block. - address %08X, previous address "
                 "%08X != %08X\n",
                 GetMemCPtrForMBlock(pMBHead), pMBHead->pMBHeadPrev,
                 pMBHeadPrev);
    return FALSE;
  }
...
static BOOL CheckMBlockNextPtr(const NNSiFndExpHeapMBlockHead *pMBHead,
  ...
  if (pMBHead->pMBHeadNext != pMBHeadNext) {
    HEAP_WARNING(
        ...
        "Exp"
        " Heap]"
        " Wrong link memory block. - address %08X, next address %08X != %08X\n",
...
static BOOL CheckMBlockLinkTail(const NNSiFndExpHeapMBlockHead *pMBHead,
  ...
  if (pMBHead != pMBHeadTail) {
    HEAP_WARNING(bPrint,
                 ...
                 "Exp"
                 " Heap]"
                 " Wrong memory brock list %s pointer. - address %08X, %s "
                 "address %08X != %08X\n",
                 heapType, GetMemCPtrForMBlock(pMBHead), heapType, pMBHead,
                 pMBHeadTail);
    return FALSE;
  }
...
void NNSi_FndDumpExpHeap(NNSFndHeapHandle heap) {
...

  {
    ...
    OS_Printf(
        "     attr  address:   size    gid aln   prev_ptr next_ptr\n"); // header
                                                                        // line
    ...
    if (pExpHeapHd->mbUsedList.head == NULL) {
      ...
      for (pMBHead = pExpHeapHd->mbUsedList.head; pMBHead;
           pMBHead = pMBHead->pMBHeadNext) {
        ...
        OS_Printf(
            ...
            GetAllocDirForMBlock(pMBHead) == NNS_FND_EXPHEAP_ALLOC_DIR_REAR
                ? " rear"
                : "front",
            GetMemPtrForMBlock(pMBHead), pMBHead->blockSize,
            GetGroupIDForMBlock(pMBHead), GetAlignmentForMBlock(pMBHead),
            pMBHead->pMBHeadPrev ? GetMemPtrForMBlock(pMBHead->pMBHeadPrev)
                                 : NULL,
            pMBHead->pMBHeadNext ? GetMemPtrForMBlock(pMBHead->pMBHeadNext)
                                 : NULL);

        usedSize += sizeof(NNSiFndExpHeapMBlockHead) + pMBHead->blockSize +
    ...
    if (pExpHeapHd->mbFreeList.head == NULL) {
      ...
      for (pMBHead = pExpHeapHd->mbFreeList.head; pMBHead;
           pMBHead = pMBHead->pMBHeadNext) {
    ...
    {
      u32 heapSize = GetOffsetFromPtr(
          pHeapHd->heapStart, pHeapHd->heapEnd); // heap size (data region size)
      OS_Printf("    %d / %d bytes (%6.2f%%) used (U:%d F:%d)\n", usedSize,
                heapSize, 100.0 * usedSize / heapSize, usedCnt, freeCnt);
    }
...
NNSFndHeapHandle NNS_FndCreateExpHeapEx(void *startAddress, u32 size,
                                        u16 optFlag) {
  void *endAddress;
  ...
  endAddress =
      NNSi_FndRoundDownPtr(AddU32ToPtr(startAddress, size), MIN_ALIGNMENT);
  ...
  if (NNSiGetUIntPtr(startAddress) > NNSiGetUIntPtr(endAddress) ||
      GetOffsetFromPtr(startAddress, endAddress) <
          sizeof(NNSiFndHeapHead) + sizeof(NNSiFndExpHeapHead) +
  ...
  { // initialization of the expanded heap
    ...
    return pHeapHd; // the pointer to the heap header is used as the handle
                    // value
  }
}
...
u32 NNS_FndResizeForMBlockExpHeap(NNSFndHeapHandle heap, void *memBlock,
                                  u32 size) {
  ...
  if (size > pMBHead->blockSize) {
    void *crUsedEnd =
        GetMBlockEndAddr(pMBHead); // end address for the used block
    NNSiFndExpHeapMBlockHead *block;
    ...
    if (!block || size > pMBHead->blockSize + sizeof(NNSiFndExpHeapMBlockHead) +
                             block->blockSize) {
      return 0;
    }

    {
      ...
      rgnNewFree.start =
          AddU32ToPtr(memBlock, size); // region position to be newly freed

      if (GetOffsetFromPtr(rgnNewFree.start, rgnNewFree.end) <
          sizeof(NNSiFndExpHeapMBlockHead)) {
        rgnNewFree.start = rgnNewFree.end; // absorbed into the used block
      }

      pMBHead->blockSize = GetOffsetFromPtr(
          memBlock, rgnNewFree.start); // change the target block size

      if (GetOffsetFromPtr(rgnNewFree.start, rgnNewFree.end) >=
          sizeof(NNSiFndExpHeapMBlockHead)) {
        (void)InsertMBlock(&pEHHead->mbFreeList, InitFreeMBlock(&rgnNewFree),
                           nextBlockPrev); // create a new free block
      }

      FillAllocMemory( // expanded portion fill
          heap, oldFreeStart, GetOffsetFromPtr(oldFreeStart, rgnNewFree.start));
    }
  }
...
void NNS_FndFreeToExpHeap(NNSFndHeapHandle heap, void *memBlock) {
...

  {
    ...
    (void)RemoveMBlock(&pExpHeapHd->mbUsedList,
                       pMBHead); // remove from the list being used
    (void)RecycleRegion(pExpHeapHd,
                        &region); // add the specified size from the specified
                                  // address to the free region
  }
}
...
u32 NNS_FndGetTotalFreeSizeForExpHeap(NNSFndHeapHandle heap) {
...

  {
    ...
    for (pMBHead = pEHHead->mbFreeList.head; pMBHead;
         pMBHead = pMBHead->pMBHeadNext) {
...
u32 NNS_FndGetAllocatableSizeForExpHeapEx(NNSFndHeapHandle heap,
                                          int alignment) {
  NNS_ASSERT(IsValidExpHeapHandle(heap));
...

  {
    ...
    for (pMBlkHd = pEHHead->mbFreeList.head; pMBlkHd;
         pMBlkHd = pMBlkHd->pMBHeadNext) {

      void *baseAddress =
          NNSi_FndRoundUpPtr(GetMemPtrForMBlock(pMBlkHd), alignment);

      if (NNSiGetUIntPtr(baseAddress) <
          NNSiGetUIntPtr(GetMBlockEndAddr(pMBlkHd))) {
        const u32 blockSize =
            GetOffsetFromPtr(baseAddress, GetMBlockEndAddr(pMBlkHd));

        const u32 offset =
            GetOffsetFromPtr(GetMemPtrForMBlock(pMBlkHd), baseAddress);
...
void NNS_FndVisitAllocatedForExpHeap(NNSFndHeapHandle heap,
                                     NNSFndHeapVisitor visitor, u32 userParam) {
  NNS_ASSERT(IsValidExpHeapHandle(heap));
  SDK_NULL_ASSERT(visitor);

  {
    NNSiFndExpHeapMBlockHead *pMBHead =
        GetExpHeapHeadPtrFromHandle(heap)->mbUsedList.head;
...
BOOL NNS_FndCheckExpHeap(NNSFndHeapHandle heap, u32 optFlag) {
...

  {
    ...
    NNSiFndExpHeapHead *const pExpHeapHd =
        GetExpHeapHeadPtrFromHeapHead(pHeapHd);
    NNSiFndExpHeapMBlockHead *pMBHead = NULL;
    NNSiFndExpHeapMBlockHead *pMBHeadPrev = NULL;

    for (pMBHead = pExpHeapHd->mbUsedList.head; pMBHead;
         pMBHeadPrev = pMBHead, pMBHead = pMBHead->pMBHeadNext) {
      if (!CheckUsedMBlock(pMBHead, pHeapHd, optFlag) ||
          !CheckMBlockPrevPtr(
              pMBHead, pMBHeadPrev,
              optFlag) // Is the pointer to the previous block the same as the
                       // pointer to the memory block in the previous loop?
      ) {
        return FALSE;
      }
    ...
    if (!CheckMBlockLinkTail(pMBHeadPrev, pExpHeapHd->mbUsedList.tail, "tail",
                             optFlag)) // Is the last block indicating the
    ...
    if (!CheckMBlockLinkTail(pMBHeadPrev, pExpHeapHd->mbFreeList.tail, "tail",
                             optFlag)) // Is the last block indicating the
                                       // pointer to the final block?
    {
      return FALSE;
    ...
    if (totalBytes != GetOffsetFromPtr(pHeapHd->heapStart, pHeapHd->heapEnd)) {
      HEAP_WARNING(bPrint,
                   ...
                   "Exp"
                   " Heap]"
                   " Incorrect total memory block size. - heap size %08X, sum "
                   "size %08X\n",
...
BOOL NNS_FndCheckForMBlockExpHeap(const void *memBlock, NNSFndHeapHandle heap,
                                  u32 optFlag) {
  ...
  if (pMBHead->pMBHeadPrev) {
    if (!CheckUsedMBlock(
            pMBHead->pMBHeadPrev, pHeapHd,
            optFlag) // check of signature and size of previous block
        ||
        !CheckMBlockNextPtr(pMBHead->pMBHeadPrev, pMBHead,
                            optFlag) // Is the previous block's pointer to the
                                     // next block indicating the current block?
    ) {
      return FALSE;
    }
  } else {
    if (pHeapHd) {
      ...
      if (!CheckMBlockLinkTail(
              pMBHead, GetExpHeapHeadPtrFromHeapHead(pHeapHd)->mbUsedList.head,
              "head", optFlag)) {
        return FALSE;
      }
    }
  ...
  if (pMBHead->pMBHeadNext) {
    if (!CheckUsedMBlock(pMBHead->pMBHeadNext, pHeapHd,
                         optFlag) // check of signature and size of next block
        || !CheckMBlockPrevPtr(
               pMBHead->pMBHeadNext, pMBHead,
               optFlag) // Is the next block's pointer to the previous block
    ...
    if (pHeapHd) {
      ...
      if (!CheckMBlockLinkTail(
              pMBHead, GetExpHeapHeadPtrFromHeapHead(pHeapHd)->mbUsedList.tail,
              "tail", optFlag)) {
        return FALSE;
      }
    }
