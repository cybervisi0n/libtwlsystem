...
typedef s32
    NNSiIntPtr; // signed integer type mutually convertible with void* pointer
typedef u32 NNSiUIntPtr; // unsigned integer type mutually convertible with
                         // void* pointer
...
NNS_FND_INLINE void FillAllocMemory(NNSiFndHeapHead *pHeapHd, void *address,
                                    u32 size) {
  ...
  if (GetOptForHeap(pHeapHd) & NNS_FND_HEAP_OPT_0_CLEAR) {
    ...
    if (GetOptForHeap(pHeapHd) & NNS_FND_HEAP_OPT_DEBUG_FILL) {
      MI_CpuFill32(address, NNS_FndGetFillValForHeap(NNS_FND_HEAP_FILL_ALLOC),
                   size);
    }
#endif
  }
...
#else
NNS_FND_INLINE void FillFreeMemory(NNSiFndHeapHead *pHeapHd, void *address,
                                   u32 size) {
  if (GetOptForHeap(pHeapHd) & NNS_FND_HEAP_OPT_DEBUG_FILL) {
    MI_CpuFill32(address, NNS_FndGetFillValForHeap(NNS_FND_HEAP_FILL_FREE),
                 size);
  }
}
#endif
...
#else
NNS_FND_INLINE void FillNoUseMemory(NNSiFndHeapHead *pHeapHd, void *address,
                                    u32 size) {
  if (GetOptForHeap(pHeapHd) & NNS_FND_HEAP_OPT_DEBUG_FILL) {
    MI_CpuFill32(address, NNS_FndGetFillValForHeap(NNS_FND_HEAP_FILL_NOUSE),
                 size);
  }
}
#endif

void NNSi_FndInitHeapHead(NNSiFndHeapHead *pHeapHd, u32 signature,
                          void *heapStart, void *heapEnd, u16 optFlag);
