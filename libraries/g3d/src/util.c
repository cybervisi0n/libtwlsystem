...
BOOL NNS_G3dGetResultMtx(const NNSG3dRenderObj *pRenderObj, MtxFx43 *pos,
                         MtxFx33 *nrm, u32 nodeID) {
...
BOOL NNS_G3dSetResultMtx(const NNSG3dRenderObj *pRenderObj, const MtxFx43 *pos,
                         const MtxFx33 *nrm, u32 nodeID) {
  const NNSG3dResNodeData *nd;
  u32 stackID;
  ...
  case NNS_G3D_SIGNATURE_NSBMD: // BMD0
  {
    ...
    if (tex) {
      ...
      if (szPltt > 0) {
        ...
        keyPltt = NNS_GfdAllocPlttVram(
            szPltt, tex->tex4x4Info.flag & NNS_G3D_RESPLTT_USEPLTT4, 0);
    ...
    if (*(u32 *)&binFile[0] == NNS_G3D_SIGNATURE_NSBMD) {
      ...
      NNSG3dResMdlSet *mdlSet =
          NNS_G3dGetMdlSet((NNSG3dResFileHeader *)pResData);
...
int NNS_G3dWorldPosToScrPos(const VecFx32 *pWorld, int *px, int *py) {
  ...
  w = (fx32)(((fx64)tmp.x * proj->_03 + (fx64)tmp.y * proj->_13 +
              (fx64)tmp.z * proj->_23) >>
             FX32_SHIFT);
  w += proj->_33;
  ...
  vec.x = (fx32)(((fx64)tmp.x * proj->_00 + (fx64)tmp.y * proj->_10 +
                  (fx64)tmp.z * proj->_20) >>
                 FX32_SHIFT);
  vec.x += proj->_30;

  vec.y = (fx32)(((fx64)tmp.x * proj->_01 + (fx64)tmp.y * proj->_11 +
                  (fx64)tmp.z * proj->_21) >>
                 FX32_SHIFT);
  vec.y += proj->_31;
...
int NNS_G3dScrPosToWorldLine(int px, int py, VecFx32 *pNear, VecFx32 *pFar) {
  ...
  vNear.x =
      m->_30 + (fx32)(((fx64)x * m->_00 + (fx64)y * m->_10) >> FX32_SHIFT);
  vNear.y =
      m->_31 + (fx32)(((fx64)x * m->_01 + (fx64)y * m->_11) >> FX32_SHIFT);
  vNear.z =
      m->_32 + (fx32)(((fx64)x * m->_02 + (fx64)y * m->_12) >> FX32_SHIFT);
