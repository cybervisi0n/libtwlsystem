...
static void G3dDrawInternal_(NNSG3dRS *pRS, NNSG3dRenderObj *pRenderObj) {
  ...
  pRS->funcJntScale =
      NNS_G3dGetJointScale_FuncArray[pRenderObj->resMdl->info.scalingRule];
  pRS->funcJntMtx =
      NNS_G3dSendJointSRT_FuncArray[pRenderObj->resMdl->info.scalingRule];
  pRS->funcTexMtx =
      NNS_G3dSendTexSRT_FuncArray[pRenderObj->resMdl->info.texMtxMode];
...
void NNS_G3dDraw(NNSG3dRenderObj *pRenderObj) {
  ...
  if (NNS_G3dRenderObjTestFlag(pRenderObj,
                               NNS_G3D_RENDEROBJ_FLAG_HINT_OBSOLETE)) {

    MI_CpuClear32(&pRenderObj->hintMatAnmExist[0],
                  sizeof(u32) * (NNS_G3D_SIZE_MAT_MAX / 32));
    MI_CpuClear32(&pRenderObj->hintJntAnmExist[0],
                  sizeof(u32) * (NNS_G3D_SIZE_JNT_MAX / 32));
    MI_CpuClear32(&pRenderObj->hintVisAnmExist[0],
                  sizeof(u32) * (NNS_G3D_SIZE_JNT_MAX / 32));
  ...
  if (!(rs->flag & NNS_G3D_RSFLAG_OPT_SKIP_SBCDRAW)) {
    ...
    if (!cbFlag) {
      ...
      if (rs->pRenderObj->anmVis &&
          ...
          (*rs->pRenderObj->funcBlendVis)(rs->pVisAnmResult,
                                          rs->pRenderObj->anmVis, curNode)) {
...
void NNSi_G3dFuncSbc_MAT_InternalDefault(NNSG3dRS *rs, u32 opt,
                                         const NNSG3dResMatData *mat,
                                         u32 idxMat) {
  static const u32 matColorMask_[8] = {0x00000000, 0x00007fff, 0x7fff0000,
                                       0x7fff7fff, 0x00008000, 0x0000ffff,
                                       0x7fff8000, 0x7fffffff};
  BOOL cbFlag;
  NNSG3dSbcCallBackTiming cbTiming;
  ...
  if (!cbFlag) {
    ...
    if (rs->pRenderObj->recMatAnm && !(rs->flag & NNS_G3D_RSFLAG_OPT_RECORD)) {
      ...
      if ((opt == NNS_G3D_SBCFLG_001 || opt == NNS_G3D_SBCFLG_010) &&
...

        {
          ...
          if (NNS_G3dGetMatDataByIdx(rs->pResMat, idxMat)->flag &
              NNS_G3D_MATFLAG_WIREFRAME) {
        ...
        }
        {
          ...
          pResult->prmMatColor0 =
              (NNS_G3dGlb.prmMatColor0 & ~mask) | (mat->diffAmb & mask);
        }

        {

          u32 mask = matColorMask_[(mat->flag >> 9) & 7];
          pResult->prmMatColor1 =
              (NNS_G3dGlb.prmMatColor1 & ~mask) | (mat->specEmi & mask);
        }

        pResult->prmPolygonAttr =
            (NNS_G3dGlb.prmPolygonAttr & ~mat->polyAttrMask) |
        ...
        if (rs->pRenderObj->anmMat &&
          ...
          (void)(*rs->pRenderObj->funcBlendMat)(pResult, rs->pRenderObj->anmMat,
                                                idxMat);
        }
  ...
  if (!cbFlag) {
    ...
    if (pResult->prmPolygonAttr & REG_G3_POLYGON_ATTR_ALPHA_MASK) {
      ...
      if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD)) {
        ...
        cmd[0] =
            GX_PACK_OP(G3OP_DIF_AMB, G3OP_SPE_EMI, G3OP_POLYGON_ATTR, G3OP_NOP);
        cmd[1] = pResult->prmMatColor0;
        cmd[2] = pResult->prmMatColor1;
        cmd[3] = pResult->prmPolygonAttr;
        cmd[4] = GX_PACK_OP(G3OP_TEXIMAGE_PARAM, G3OP_TEXPLTT_BASE, G3OP_NOP,
                            G3OP_NOP);
...
void NNSi_G3dFuncSbc_SHP_InternalDefault(NNSG3dRS *rs, u32,
                                         const NNSG3dResShpData *shp, u32) {
  BOOL cbFlag;
  NNSG3dSbcCallBackTiming cbTiming;
  NNS_G3D_NULL_ASSERT(rs);
...
void NNSi_G3dFuncSbc_NODEDESC(NNSG3dRS *rs, u32 opt) {
  u32 cmdLen = 4;
  static const u8 pivotUtil_[9][4] = {{4, 5, 7, 8}, {3, 5, 6, 8}, {3, 4, 6, 7},
  ...
  if (rs->flag & NNS_G3D_RSFLAG_OPT_SKIP_SBCMTXCALC) {
    ...
    if (opt == NNS_G3D_SBCFLG_001 || opt == NNS_G3D_SBCFLG_011) {
      ...
      if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD)) {
        ...
        NNS_G3dGeBufferOP_N(G3OP_MTX_RESTORE, &idxMtxDest,
...

  {
    if (opt == NNS_G3D_SBCFLG_010 || opt == NNS_G3D_SBCFLG_011) {
      ...
      if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD)) {
        NNS_G3dGeBufferOP_N(G3OP_MTX_RESTORE, &idxMtxSrc,
                            G3OP_MTX_RESTORE_NPARAMS);
      }
    }
  }
  ...
  if (!cbFlag) {
    ...
    if (!isUseRecordData) {
      ...
      if (rs->pRenderObj->anmJnt &&
          (*rs->pRenderObj->funcBlendJnt)(pAnmResult, rs->pRenderObj->anmJnt,
                                          idxNode)) {
        ...
        if (pNd->flag & NNS_G3D_SRTFLAG_ROT_ZERO) {
          ...
          if (pNd->flag & NNS_G3D_SRTFLAG_PIVOT_EXIST) {
            ...
            pAnmResult->rot.a[idxPivot] =
                (pNd->flag & NNS_G3D_SRTFLAG_PIVOT_MINUS) ? -FX32_ONE
                                                          : FX32_ONE;
  ...
  if (opt == NNS_G3D_SBCFLG_001 || opt == NNS_G3D_SBCFLG_011) {
    ...
    if (!cbFlag) {
      if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD)) {
        ...
        NNS_G3dGeBufferOP_N(G3OP_MTX_STORE, &idxMtxDest,
...
void NNSi_G3dFuncSbc_BB(NNSG3dRS *rs, u32 opt) {
  ...
  static u32 bbcmd1[] = {
      ...
      0,
      0,
      0, // This area is subject to change (Trans)
      0,
      0,
      0 // subject to change (Scale)
  };
  ...
  if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD) && (!cbFlag)) {

    NNS_G3dGeFlushBuffer();

    reg_G3X_GXFIFO =
        GX_PACK_OP(G3OP_MTX_MODE, G3OP_MTX_PUSH, G3OP_MTX_IDENTITY, G3OP_NOP);
    ...
    if (NNS_G3dGlb.flag & NNS_G3D_GLB_FLAG_FLUSH_WVP) {
      ...
      reg_G3X_GXFIFO =
          GX_PACK_OP(G3OP_MTX_POP, G3OP_MTX_MODE, G3OP_MTX_LOAD_4x3, G3OP_NOP);
      MI_CpuSend32(&bbcmd1[1], &reg_G3X_GXFIFO, 2 * sizeof(u32));
      MI_CpuSend32(NNS_G3dGlbGetInvSrtCameraMtx(), &reg_G3X_GXFIFO,
                   G3OP_MTX_LOAD_4x3_NPARAMS * sizeof(u32));

      reg_G3X_GXFIFO =
          GX_PACK_OP(G3OP_MTX_MULT_4x3, G3OP_MTX_SCALE, G3OP_NOP, G3OP_NOP);
      MI_CpuSend32(&bbcmd1[3], &reg_G3X_GXFIFO,
      ...
      reg_G3X_GXFIFO =
          GX_PACK_OP(G3OP_MTX_POP, G3OP_MTX_MODE, G3OP_MTX_LOAD_4x3, G3OP_NOP);
      MI_CpuSend32(&bbcmd1[1], &reg_G3X_GXFIFO, 2 * sizeof(u32));
      MI_CpuSend32(NNS_G3dGlbGetInvCameraMtx(), &reg_G3X_GXFIFO,
                   G3OP_MTX_LOAD_4x3_NPARAMS * sizeof(u32));
...
void NNSi_G3dFuncSbc_BBY(NNSG3dRS *rs, u32 opt) {
  ...
  static u32 bbcmd1[] = {
      ...
      FX32_ONE,
      0,
      0, // This area is subject to change (4x3Mtx)
      0,
      FX32_ONE,
      0,
      0,
...
void NNSi_G3dFuncSbc_NODEMIX(NNSG3dRS *rs, u32) {
...
#if (NNS_G3D_USE_EVPCACHE)
  {
    ...
    G3_LoadMtx43((const MtxFx43 *)&sum.N); // This gets overwritten, so it's OK
                                           // if the garbage data gets mixed up.
    G3_MtxMode(GX_MTXMODE_POSITION);
    G3_LoadMtx43(&sum.M);
    G3_MtxMode(GX_MTXMODE_PROJECTION);
    G3_RestoreMtx(1);
...
void NNSi_G3dFuncSbc_CALLDL(NNSG3dRS *rs, u32) {
  ...
  if (!(rs->flag & NNS_G3D_RSFLAG_OPT_NOGECMD) && (!cbFlag)) {
    u32 rel_addr;
    u32 size;
    rel_addr = (u32)((*(rs->c + 1) << 0) | (*(rs->c + 2) << 8) |
                     (*(rs->c + 3) << 16) | (*(rs->c + 4) << 24));

    size = (u32)((*(rs->c + 5) << 0) | (*(rs->c + 6) << 8) |
                 (*(rs->c + 7) << 16) | (*(rs->c + 8) << 24));
...
void NNSi_G3dFuncSbc_POSSCALE(NNSG3dRS *rs, u32 opt) {
  VecFx32 s;
  NNS_G3D_NULL_ASSERT(rs);
  NNS_G3D_ASSERT(opt == NNS_G3D_SBCFLG_000 || opt == NNS_G3D_SBCFLG_001);
...
void NNSi_G3dFuncSbc_ENVMAP(NNSG3dRS *rs, u32) {
  ...
  if (!(rs->flag & NNS_G3D_RSFLAG_OPT_SKIP_SBCDRAW) &&
    ...
    if ((rs->pMatAnmResult->prmTexImage & REG_G3_TEXIMAGE_PARAM_TGEN_MASK) !=
      ...
      static u32 cmd[] = {G3OP_TEXIMAGE_PARAM, 0};
      rs->pMatAnmResult->prmTexImage &= ~REG_G3_TEXIMAGE_PARAM_TGEN_MASK;
      rs->pMatAnmResult->prmTexImage |= GX_TEXGEN_NORMAL
                                        << REG_G3_TEXIMAGE_PARAM_TGEN_SHIFT;
    ...
    if (!cbFlag) {
...

      {
        ...
        NNS_G3dGeScale(width << (FX32_SHIFT + 3), -height << (FX32_SHIFT + 3),
                       FX32_ONE << 4);
        NNS_G3dGeTexCoord(width << (FX32_SHIFT - 1),
                          height << (FX32_SHIFT - 1));
      }
    }
    ...
    if (!cbFlag) {
      ...
      if (NNS_G3dGlb.flag & NNS_G3D_GLB_FLAG_FLUSH_WVP) {
        ...
        NNS_G3dGeMultMtx33(
            (const MtxFx33 *)
                NNS_G3dGlbGetCameraMtx()); // world coordinate system -> view
                                           // coordinate system
        NNS_G3dGeMultMtx33(
            NNS_G3dGlbGetBaseRot()); // local coordinate system -> world
                                     // coordinate system
        NNS_G3dGeMultMtx33(
            &n); // joint coordinate system -> local coordinate system
        ...
        NNS_G3dGeMultMtx33(
            (const MtxFx33 *)
                NNS_G3dGlbGetCameraMtx()); // world coordinate system -> view
                                           // coordinate system
        NNS_G3dGeMultMtx33(
            &n); // joint coordinate system -> world coordinate system
      } else {
        NNS_G3dGeMultMtx33(
            &n); // joint coordinate system -> view coordinate system
      }
    }
...
void NNSi_G3dFuncSbc_PRJMAP(NNSG3dRS *rs, u32) {
  ...
  if (!(rs->flag & NNS_G3D_RSFLAG_OPT_SKIP_SBCDRAW) &&
    ...
    if ((rs->pMatAnmResult->prmTexImage & REG_G3_TEXIMAGE_PARAM_TGEN_MASK) !=
      ...
      static u32 cmd[] = {G3OP_TEXIMAGE_PARAM, 0};
      rs->pMatAnmResult->prmTexImage &= ~REG_G3_TEXIMAGE_PARAM_TGEN_MASK;
      rs->pMatAnmResult->prmTexImage |= GX_TEXGEN_VERTEX
                                        << REG_G3_TEXIMAGE_PARAM_TGEN_SHIFT;
      ...
      if (NNS_G3dGlb.flag & NNS_G3D_GLB_FLAG_FLUSH_WVP) {
        ...
        NNS_G3dGeMultMtx33(
            NNS_G3dGlbGetBaseRot()); // local coordinate system -> world
                                     // coordinate system
        NNS_G3dGeMultMtx43(
            &m); // joint coordinate system -> local coordinate system

      } else if (NNS_G3dGlb.flag & NNS_G3D_GLB_FLAG_FLUSH_VP) {

        NNS_G3dGeMultMtx43(
            &m); // joint coordinate system -> world coordinate system
      } else {

        NNS_G3dGeMultMtx43(NNS_G3dGlbGetInvV()); // camera coordinate system ->
                                                 // world coordinate system
        NNS_G3dGeMultMtx43(
            &m); // joint coordinate system -> camera coordinate system
      }
...
NNSG3dFuncSbc NNS_G3dFuncSbcTable[NNS_G3D_SBC_COMMAND_NUM] = {
    &NNSi_G3dFuncSbc_NOP,      &NNSi_G3dFuncSbc_RET,
    &NNSi_G3dFuncSbc_NODE,     &NNSi_G3dFuncSbc_MTX,
    &NNSi_G3dFuncSbc_MAT,      &NNSi_G3dFuncSbc_SHP,
    &NNSi_G3dFuncSbc_NODEDESC, &NNSi_G3dFuncSbc_BB,

    &NNSi_G3dFuncSbc_BBY,      &NNSi_G3dFuncSbc_NODEMIX,
    &NNSi_G3dFuncSbc_CALLDL,   &NNSi_G3dFuncSbc_POSSCALE,
    &NNSi_G3dFuncSbc_ENVMAP,   &NNSi_G3dFuncSbc_PRJMAP};
