...
void NNS_G3dAnmObjInit(NNSG3dAnmObj *pAnmObj, void *pResAnm,
  ...
  pAnmObj->numMapData =
      0; // for supporting the animation disable macro (NNS_G3D_NS***_DISABLE)
  pAnmObj->funcAnm = NULL; // for supporting the animation disable macro
                           // (NNS_G3D_NS***_DISABLE)
...
void NNS_G3dRenderObjAddAnmObj(NNSG3dRenderObj *pRenderObj,
...
void NNS_G3dRenderObjRemoveAnmObj(NNSG3dRenderObj *pRenderObj,
                                  NNSG3dAnmObj *pAnmObj) {
  NNS_G3D_NULL_ASSERT(pRenderObj);
  NNS_G3D_NULL_ASSERT(pAnmObj);
  ...
  NNS_G3D_WARNING(0,
                  "An AnmObj was not removed in NNS_G3dRenderObjRemoveAnmObj");
}

void NNS_G3dRenderObjSetCallBack(
    NNSG3dRenderObj *pRenderObj, NNSG3dSbcCallBackFunc func,
    u8 *, // NOTICE: eliminated the address specification of callbacks
    u8 cmd, NNSG3dSbcCallBackTiming timing) {
  NNS_G3D_NULL_ASSERT(pRenderObj);
...
void NNS_G3dRenderObjSetInitFunc(NNSG3dRenderObj *pRenderObj,
...
void NNS_G3dTexSetTexKey(NNSG3dResTex *pTex, NNSG3dTexKey texKey,
  ...
  if (tex4x4Key > 0) {
    ...
    NNS_G3D_ASSERTMSG((pTex->tex4x4Info.sizeTex << 3) <=
                          NNS_GfdGetTexKeySize(tex4x4Key),
                      "NNS_G3dTexSetTexKey(4x4): texKey size not enough, "
                      "0x%05x bytes required.",
                      pTex->tex4x4Info.sizeTex << 3);
...
void NNS_G3dTexReleaseTexKey(NNSG3dResTex *pTex, NNSG3dTexKey *texKey,
                             NNSG3dTexKey *tex4x4Key) {
  NNS_G3D_NULL_ASSERT(pTex);
...
static void bindMdlTex_Internal_(NNSG3dResMat *pMat,
  ...
  for (j = 0; j < pBindData->numIdx; ++j) {
    ...
    w = (s32)(((pTexData->extraParam) & NNS_G3D_TEXIMAGE_PARAMEX_ORIGW_MASK) >>
              NNS_G3D_TEXIMAGE_PARAMEX_ORIGW_SHIFT);
    h = (s32)(((pTexData->extraParam) & NNS_G3D_TEXIMAGE_PARAMEX_ORIGH_MASK) >>
...
static void releaseMdlTex_Internal_(NNSG3dResMat *pMat,
  ...
  for (j = 0; j < pData->numIdx; ++j) {
    ...
    matData->texImageParam &=
        REG_G3_TEXIMAGE_PARAM_TGEN_MASK | REG_G3_TEXIMAGE_PARAM_FT_MASK |
        REG_G3_TEXIMAGE_PARAM_FS_MASK | REG_G3_TEXIMAGE_PARAM_RT_MASK |
...
BOOL NNS_G3dBindMdlTexEx(NNSG3dResMdl *pMdl, const NNSG3dResTex *pTex,
  ...
  if (texData) {
    ...
    data = (NNSG3dResDictTexToMatIdxData *)NNS_G3dGetResDataByName(dictTex,
                                                                   pResName);
...
BOOL NNS_G3dForceBindMdlTex(NNSG3dResMdl *pMdl, const NNSG3dResTex *pTex,
                            u32 texToMatListIdx, u32 texIdx) {
  NNSG3dResMat *mat;
  NNSG3dResDict *dictTex;
  const NNSG3dResDictTexData *texData;
  NNSG3dResDictTexToMatIdxData *data;
  ...
  data = (NNSG3dResDictTexToMatIdxData *)NNS_G3dGetResDataByIdx(
      dictTex, texToMatListIdx);
...
static void bindMdlPltt_Internal_(NNSG3dResMat *pMat,
  ...
  u16 vramOffset = (u16)(NNS_GfdGetTexKeyAddr(pTex->plttInfo.vramKey) >>
                         NNS_GFD_TEXKEY_ADDR_SHIFT);
  u32 j;
...
BOOL NNS_G3dBindMdlPltt(NNSG3dResMdl *pMdl, const NNSG3dResTex *pTex) {
  ...
  for (i = 0; i < dictPltt->numEntry; ++i) {
    ...
    const NNSG3dResDictPlttData *plttData =
        NNS_G3dGetPlttDataByName(pTex, name);
...
BOOL NNS_G3dBindMdlPlttEx(NNSG3dResMdl *pMdl, const NNSG3dResTex *pTex,
                          const NNSG3dResName *pResName) {
  NNSG3dResMat *mat;
  NNSG3dResDict *dictPltt;
  const NNSG3dResDictPlttData *plttData;
  ...
  if (plttData) {
    ...
    data = (NNSG3dResDictPlttToMatIdxData *)NNS_G3dGetResDataByName(dictPltt,
                                                                    pResName);
...
BOOL NNS_G3dForceBindMdlPltt(NNSG3dResMdl *pMdl, const NNSG3dResTex *pTex,
                             u32 plttToMatListIdx, u32 plttIdx) {
  NNSG3dResMat *mat;
  NNSG3dResDict *dictPltt;
  const NNSG3dResDictPlttData *plttData;
  NNSG3dResDictPlttToMatIdxData *data;
  ...
  data = (NNSG3dResDictPlttToMatIdxData *)NNS_G3dGetResDataByIdx(
      dictPltt, plttToMatListIdx);
...
BOOL NNS_G3dReleaseMdlPlttEx(NNSG3dResMdl *pMdl,
                             const NNSG3dResName *pResName) {
  NNSG3dResMat *mat;
  NNSG3dResDict *dictPltt;
  NNSG3dResDictPlttToMatIdxData *data;
