...
void NNSi_G3dAnmObjInitNsBtp(NNSG3dAnmObj *pAnmObj, void *pResAnm,
...
static void SetTexParamaters_(const NNSG3dResTex *pTex,
      ...
      "No texture(4x4) key assigned");
  {
    ...
    {
      const u32 vramOffset =
          ((pData->texImageParam & REG_G3_TEXIMAGE_PARAM_TEXFMT_MASK) !=
           (GX_TEXFMT_COMP4x4 << REG_G3_TEXIMAGE_PARAM_TEXFMT_SHIFT))
              ? NNS_GfdGetTexKeyAddr(pTex->texInfo.vramKey) >>
                    NNS_GFD_TEXKEY_ADDR_SHIFT
              : // Some texture other than 4x4
              NNS_GfdGetTexKeyAddr(pTex->tex4x4Info.vramKey) >>
                  NNS_GFD_TEXKEY_ADDR_SHIFT; // A 4x4 texture
      ...
      pResult->origWidth =
          (u16)((pData->extraParam & NNS_G3D_TEXIMAGE_PARAMEX_ORIGW_MASK) >>
                NNS_G3D_TEXIMAGE_PARAMEX_ORIGW_SHIFT);
      pResult->origHeight =
          (u16)((pData->extraParam & NNS_G3D_TEXIMAGE_PARAMEX_ORIGH_MASK) >>
                NNS_G3D_TEXIMAGE_PARAMEX_ORIGH_SHIFT);

      {
        const s32 w =
            (s32)(((pData->extraParam) & NNS_G3D_TEXIMAGE_PARAMEX_ORIGW_MASK) >>
                  NNS_G3D_TEXIMAGE_PARAMEX_ORIGW_SHIFT);
        const s32 h =
            (s32)(((pData->extraParam) & NNS_G3D_TEXIMAGE_PARAMEX_ORIGH_MASK) >>
                  NNS_G3D_TEXIMAGE_PARAMEX_ORIGH_SHIFT);
...
static void SetPlttParamaters_(const NNSG3dResTex *pTex,
...

  {
    ...
    const NNSG3dResDictPlttData *pPlttData =
        NNS_G3dGetPlttDataByName(pTex, pPlttName);
...
void NNSi_G3dAnmCalcNsBtp(NNSG3dMatAnmResult *pResult,
...

  {
    ...
    pTexFV =
        NNSi_G3dGetTexPatAnmFV(pPatAnm, (u16)dataIdx, (u16)FX_Whole(frame));
    ...
    if (pTexFV->idPltt != 255) {
      SetPlttParamaters_(
          pAnmObj->resTex,
          NNSi_G3dGetTexPatAnmPlttNameByIdx(pPatAnm, pTexFV->idPltt), pResult);
    }
  }
}
