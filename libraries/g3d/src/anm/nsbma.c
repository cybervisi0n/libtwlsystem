...
static u16 GetMatColAnmValue_(const NNSG3dResMatCAnm *pAnm, u32 info,
                              u32 frame) {
  const u16 *pDataHead;
  u32 last_interp;
  u32 idx;
  ...
  pDataHead = (const u16 *)((const u8 *)pAnm +
...
static u16 GetMatColAnmuAlphaValue_(const NNSG3dResMatCAnm *pAnm, u32 info,
                                    u32 frame) {
  const u8 *pDataHead;
  u32 last_interp;
  ...
  pDataHead =
      (const u8 *)pAnm + (info & NNS_G3D_MATCANM_ELEM_OFFSET_CONSTANT_MASK);
...
static NNS_G3D_INLINE void GetMatColAnm_(const NNSG3dResMatCAnm *pAnm, u16 idx,
                                         u32 frame,
  ...
  NNS_G3D_ASSERT((pAnm->anmHeader.category0 == 'M') &&
                 (pAnm->anmHeader.category1 == 'MA'));

  {
    u16 diffuse, ambient, emission, specular, polygon_alpha;
    const NNSG3dResDictMatCAnmData *pAnmData =
        (const NNSG3dResDictMatCAnmData *)NNS_G3dGetResDataByIdx(&pAnm->dict,
                                                                 idx);
    ...
    pResult->prmMatColor0 = GX_PACK_DIFFAMB_PARAM(
        diffuse, ambient, (pResult->prmMatColor0 & REG_G3_DIF_AMB_C_MASK));
    ...
    pResult->prmMatColor1 = GX_PACK_SPECEMI_PARAM(
        specular, emission, (pResult->prmMatColor1 & REG_G3_SPE_EMI_S_MASK));

    polygon_alpha =
        GetMatColAnmuAlphaValue_(pAnm, pAnmData->polygon_alpha, frame);
...
void NNSi_G3dAnmObjInitNsBma(NNSG3dAnmObj *pAnmObj, void *pResAnm,
