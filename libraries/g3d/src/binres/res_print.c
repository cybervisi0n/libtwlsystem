...
void NNS_G3dPrintFileHeader(const NNSG3dResFileHeader *header) {
  ...
  tabPrint_();
  RES_PRINTF("[FileHeader]\n");
  tabPlus_();
  {
    tabPrint_();
    RES_PRINTF("file size, %d bytes\n", header->fileSize);
    tabPrint_();
    RES_PRINTF("Signature, %c%c%c%c\n", header->signature[0],
               header->signature[1], header->signature[2],
               header->signature[3]);
    tabPrint_();
    RES_PRINTF("Version,   %d.%d\n", header->version >> 8,
               header->version & 0xff);
  }
  tabMinus_();
}
...
void NNS_G3dPrintDictEntryItem_TEX(u32 idx, const NNSG3dResName *name,
...
void NNS_G3dPrintDictEntryItem_PLTT(u32 idx, const NNSG3dResName *name,
...
void NNS_G3dPrintTex(const NNSG3dResTex *texImage) {
  ...
  {
    const NNSG3dResDict *dict =
        (const NNSG3dResDict *)((u8 *)texImage + texImage->plttInfo.ofsDict);
    u32 numEntry = dict->numEntry;

    tabPrint_();
    RES_PRINTF("# of palettes, %d\n", numEntry);
    tabPrint_();
    RES_PRINTF("Image size of palettes,  %d\n",
...
void NNS_G3dPrintMdlInfo(const NNSG3dResMdlInfo *info) {
  ...
  tabPlus_();
  {
    ...
    {
      tabPrint_();
      RES_PRINTF("Texture Matrix Mode, ");
    ...
    {
      tabPrint_();
      RES_PRINTF("# of vectices, ");
      RES_PRINTF("%d\n", info->numVertex);

      tabPrint_();
      RES_PRINTF("# of polygons, ");
      RES_PRINTF("%d\n", info->numPolygon);

      tabPrint_();
      RES_PRINTF("# of triangles, ");
      RES_PRINTF("%d\n", info->numTriangle);
    ...
    {
      tabPrint_();
      RES_PRINTF("Box(xyzwhd), ");
      printFx32_(info->boxX);
      RES_PRINTF(", ");
      printFx32_(info->boxY);
      RES_PRINTF(", ");
      printFx32_(info->boxZ);
      RES_PRINTF(", ");
      printFx32_(info->boxW);
      RES_PRINTF(", ");
      printFx32_(info->boxH);
      RES_PRINTF(", ");
      printFx32_(info->boxD);
      RES_PRINTF("\n");
    }
  }
  tabMinus_();
...
static u8 pivotUtil_[9][4] = {{4, 5, 7, 8}, {3, 5, 6, 8}, {3, 4, 6, 7},
...
void NNS_G3dPrintNodeSRTInfo(const NNSG3dResNodeInfo *info, u32 idx) {
  ...
  if ((srt->flag & NNS_G3D_SRTFLAG_IDENTITY) == NNS_G3D_SRTFLAG_IDENTITY) {
    RES_PRINTF(", Identity");
    {
      u32 mtxStackID = ((u32)srt->flag & NNS_G3D_SRTFLAG_IDXMTXSTACK_MASK) >>
  ...
  tabPlus_();
  {
    ...
    if (!(srt->flag & NNS_G3D_SRTFLAG_ROT_ZERO)) {
      ...
      if (srt->flag & NNS_G3D_SRTFLAG_PIVOT_EXIST) {
        ...
        fx16 *p = (fx16 *)ptr;
        u32 idxPivot = ((u32)srt->flag & NNS_G3D_SRTFLAG_IDXPIVOT_MASK) >>
...
static void printOpNodeDescFlag(u32 flag) { RES_PRINTF(", 0x%02x", flag); }
...
void NNS_G3dPrintSbc(const u8 *pCode, u32 sizeByte, const NNSG3dResMdl *mdl) {
  ...
  idx = 0;
  tabPrint_();
  RES_PRINTF("size of SBC(bytes), %d\n", sizeByte);
  ...
  while (idx < sizeByte) {
    ...
    case NNS_G3D_SBC_NODEMIX: {
      ...
      idx += 3;
      for (i = 0; i < *(p + 2); ++i) {
        RES_PRINTF("<Sid=%d, Mid=%d, %f> ", *(pp), *(pp + 1),
                   *(pp + 2) / 256.f);
        pp += 3;
        idx += 3;
      }
...
#define NNS_G3D_DLPARAM_DEFAULT                                                \
  default:                                                                     \
    RES_PRINTF("Unknown\n");                                                   \
    break
#define NNS_G3D_DLPARAM_FX(val) RES_PRINTF("%6f", (fx32)(val) / 4096.f)
...
#define NNS_G3D_DLPARAM_COLOR(rgb)                                             \
  RES_PRINTF("(%d, %d, %d) ", (rgb) & 0x1f, ((rgb) >> 5) & 0x1f,               \
             ((rgb) >> 10) & 0x1f)

#define NNS_G3D_DLPARAM_44(p)                                                  \
  NNS_G3D_DLPARAM_FX(*(p + 0));                                                \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 1));                                                \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 2));                                                \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 3));                                                \
  RES_PRINTF("\n             ");                                               \
  NNS_G3D_DLPARAM_FX(*(p + 4));                                                \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 5));                                                \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 6));                                                \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 7));                                                \
  RES_PRINTF("\n             ");                                               \
  NNS_G3D_DLPARAM_FX(*(p + 8));                                                \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 9));                                                \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 10));                                               \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 11));                                               \
  RES_PRINTF("\n             ");                                               \
  NNS_G3D_DLPARAM_FX(*(p + 12));                                               \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 13));                                               \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 14));                                               \
  RES_PRINTF(", ");                                                            \
  NNS_G3D_DLPARAM_FX(*(p + 15));                                               \
  RES_PRINTF("\n");
  ...
  while (cur < dl + (szByte >> 2)) {
    ...
    for (i = 0; i < 4; ++i) {
      ...
      case G3OP_SWAP_BUFFERS:
        ...
        break;
      case G3OP_VIEWPORT:
        RES_PRINTF("ViewPort (%d, %d)-(%d, %d)", *param & 0xff,
                   (*param >> 8) & 0xff, (*param >> 16) & 0xff,
                   (*param >> 24) & 0xff);
...
static const NNSG3dResName *getTexNameForMatID_(const NNSG3dResMat *mat,
                                                u32 idx) {
  u32 i, j;
  ...
  for (i = 0; i < texDict->numEntry; ++i) {
    const NNSG3dResDictTexToMatIdxData *data =
        (const NNSG3dResDictTexToMatIdxData *)NNS_G3dGetResDataByIdx(texDict,
                                                                     i);
...
static const NNSG3dResName *getPlttNameForMatID_(const NNSG3dResMat *mat,
                                                 u32 idx) {
  u32 i, j;
  ...
  for (i = 0; i < plttDict->numEntry; ++i) {
    const NNSG3dResDictTexToMatIdxData *data =
        (const NNSG3dResDictTexToMatIdxData *)NNS_G3dGetResDataByIdx(plttDict,
                                                                     i);
...
void NNS_G3dPrintMatData(const NNSG3dResMat *mat, u32 idx) {
  ...
  tabPlus_();
  {
    ...
    if (matData->flag & NNS_G3D_MATFLAG_DIFFUSE) {
      tabPrint_();
      printRGB_("Diffuse ", (matData->diffAmb & 0x7fff));
    }
    if (matData->flag & NNS_G3D_MATFLAG_AMBIENT) {
      tabPrint_();
      printRGB_("Ambient ", (matData->diffAmb >> 16) & 0x7fff);
    }
    if (matData->flag & NNS_G3D_MATFLAG_SPECULAR) {
      tabPrint_();
      printRGB_("Specular", (matData->specEmi & 0x7fff));
    }
    if (matData->flag & NNS_G3D_MATFLAG_EMISSION) {
      tabPrint_();
      printRGB_("Emission", (matData->specEmi >> 16) & 0x7fff);
    }
  }
  tabMinus_();
}
...
void NNS_G3dPrintNSBMD(const u8 *binFile) {
  ...
  for (i = 0; i < numBlocks; ++i) {
    ...
    default:
      tabPlus_();
      tabPrint_();
      RES_PRINTF("cannot display this block\n");
      tabMinus_();
      break;
    }
  }
...
void printFx32_(fx32 v) { RES_PRINTF("%f", v / 4096.f); }

f32 fx32Tof32_(fx32 v) { return (f32)(v / 4096.f); }
