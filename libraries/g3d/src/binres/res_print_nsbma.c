...
void NNS_G3dPrintMatColAnm(const NNSG3dResMatCAnm *pMatColAnm) {
  ...
  for (j = 0; j < pMatColAnm->dict.numEntry; j++) {
    ...
    RES_PRINTF("\n");
    pAnmData = (const NNSG3dResDictMatCAnmData *)NNS_G3dGetResDataByIdx(
        &pMatColAnm->dict, j);

    tabPlus_();
    tabPrint_();
    RES_PRINTF("diffuse");
    printColData_(pAnmData->diffuse);
    tabPrint_();
    RES_PRINTF("ambient");
    printColData_(pAnmData->ambient);
    tabPrint_();
    RES_PRINTF("specular");
    printColData_(pAnmData->specular);
    tabPrint_();
    RES_PRINTF("emission");
    printColData_(pAnmData->emission);
    tabPrint_();
    RES_PRINTF("alpha");
    printAlphaData_(pAnmData->polygon_alpha);
    tabMinus_();
  }
  tabMinus_();
}
...
void NNS_G3dPrintMatColAnmSet(const NNSG3dResMatCAnmSet *pAnmSet) {
  ...
  for (i = 0; i < pAnmSet->dict.numEntry; i++) {
    ...
    tabPrint_();
    RES_PRINTF("Material color animation, ");
    NNS_G3dPrintResName(name);
    RES_PRINTF("\n");
