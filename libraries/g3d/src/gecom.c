...
BOOL NNS_G3dGeIsSendDLBusy(void) { return NNS_G3dFlagGXDmaAsync; }

BOOL NNS_G3dGeIsBufferExist(void) { return (NNS_G3dGeBuffer != NULL); }
...
void NNS_G3dGeFlushBuffer(void) {
  ...
  if (NNS_G3dGeBuffer && NNS_G3dGeBuffer->idx > 0) {
    sendNB(&NNS_G3dGeBuffer->data[0], (void *)&reg_G3X_GXFIFO,
...
static void simpleUnlock_(void *arg) { *((volatile int *)arg) = 0; }
...
void NNS_G3dGeUseFastDma(BOOL cond) { NNS_G3dFlagUseFastDma = (cond); }
...
void NNS_G3dGeSendDL(const void *src, u32 szByte) {
  ...
  if (szByte < 256 || GX_DMAID == GX_DMA_NOT_USE) {
    NNS_G3dGeBufferOP_N(*(const u32 *)src, (const u32 *)src + 1,
                        (szByte >> 2) - 1);
  } else {
    ...
    if (NNS_G3dFlagUseFastDma) {
      if (OS_IsRunOnTwl()) {
        ...
        if (dmaNo == GX_DMA_NOT_USE) {
          ...
          MI_SendNDmaGXCommandAsyncFast(dmaNo, src, szByte, &simpleUnlock_,
                                        (void *)&NNS_G3dFlagGXDmaAsync);
        } else {
          MI_SendGXCommandAsyncFast(dmaNo, src, szByte, &simpleUnlock_,
                                    (void *)&NNS_G3dFlagGXDmaAsync);
        }
#endif
      } else {
      ...
      if (OS_IsRunOnTwl()) {
        ...
        if (dmaNo == GX_DMA_NOT_USE) {
          dmaNo = GX_GetDefaultNDMA();
          MI_SendNDmaGXCommandAsync(dmaNo, src, szByte, &simpleUnlock_,
                                    (void *)&NNS_G3dFlagGXDmaAsync);
        } else {
          MI_SendGXCommandAsync(dmaNo, src, szByte, &simpleUnlock_,
                                (void *)&NNS_G3dFlagGXDmaAsync);
        }
#endif
      } else {
  ...
  if (NNS_G3dGeBuffer) {
    if (NNS_G3dFlagGXDmaAsync) {
      ...
      if (NNS_G3dGeBuffer->idx + 1 + num <= NNS_G3D_SIZE_COMBUFFER) {
        ...
        if (num > 0) {
          MI_CpuCopyFast(args, &NNS_G3dGeBuffer->data[NNS_G3dGeBuffer->idx],
                         num << 2);
