...
static void texmtxCalc_flag_(MtxFx44 *m, const NNSG3dMatAnmResult *anm) {
  ...
  tmp = ((fx64)anm->transS * anm->cosR + (fx64)anm->transT * anm->sinR) >>
        FX32_SHIFT;
  p = (fx32)((tmp * anm->scaleS) >> FX32_SHIFT);
  tmp = ((fx64)anm->transS * anm->sinR - (fx64)anm->transT * anm->cosR) >>
        FX32_SHIFT;
...
static void texmtxCalc_flagS_(MtxFx44 *m, const NNSG3dMatAnmResult *anm) {
  ...
  p = (fx32)(((fx64)anm->transS * anm->cosR + (fx64)anm->transT * anm->sinR) >>
             FX32_SHIFT);
  q = (fx32)(((fx64)anm->transS * anm->sinR - (fx64)anm->transT * anm->cosR) >>
             FX32_SHIFT);
