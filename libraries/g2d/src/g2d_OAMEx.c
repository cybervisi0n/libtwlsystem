...
IsOamEntryFuncsValid_(const NNSG2dOamManagerInstanceEx *pMan,
                      const NNSG2dOamExEntryFunctions *pF) {
  const BOOL bValid = (BOOL)((pF != NULL) && (pF->getOamCapacity != NULL) &&
                             (pF->entryNewOam != NULL));

  if (pMan->pAffineBuffer != NULL || pMan->lengthAffineBuffer != 0) {

    return (BOOL)(bValid && (pF->getAffineCapacity != NULL) &&
...
static NNS_G2D_INLINE NNSG2dOamChunk *
GetNewOamChunk_(NNSG2dOamManagerInstanceEx *pMan, const GXOamAttr *pOam) {
...
static NNS_G2D_INLINE void AddFront_(NNSG2dOamChunkList *pOamList,
...
static NNS_G2D_INLINE void AddBack_(NNSG2dOamChunkList *pOamList,
                                    NNSG2dOamChunk *pChunk) {
  pChunk->pNext = NULL;
...
EntryOamToToBaseModule_(NNSG2dOamManagerInstanceEx *pMan, const GXOamAttr *pOam,
                        u16 totalOam) {
  (void)(*pMan->oamEntryFuncs.entryNewOam)(pOam, totalOam);
}

static NNS_G2D_INLINE BOOL
IsAffineProxyValid_(NNSG2dOamManagerInstanceEx *pMan) {
  return (pMan->pAffineBuffer != NULL && pMan->lengthAffineBuffer != 0) ? TRUE
                                                                        : FALSE;
}

static NNS_G2D_INLINE BOOL
HasEnoughCapacity_(NNSG2dOamManagerInstanceEx *pMan) {
  NNS_G2D_NULL_ASSERT(pMan);
  return (pMan->numAffineBufferUsed < pMan->lengthAffineBuffer) ? TRUE : FALSE;
}

static NNS_G2D_INLINE u16 ConvertAffineIndex_(NNSG2dOamManagerInstanceEx *pMan,
                                              u16 affineProxyIdx) {
...
static NNS_G2D_INLINE void
ReindexOamChunkList_(NNSG2dOamManagerInstanceEx *pMan,
...
static NNS_G2D_INLINE void
ReindexAffinedOams_(NNSG2dOamManagerInstanceEx *pMan) {
  NNS_G2D_NULL_ASSERT(pMan);
...
static NNS_G2D_INLINE BOOL IsPriorityValid_(NNSG2dOamManagerInstanceEx *pMan,
                                            u8 priority) {
  return (priority < pMan->lengthOfOrderingTbl) ? TRUE : FALSE;
}

static NNS_G2D_INLINE BOOL
EntryNewOamWithAffine_(NNSG2dOamChunkList *pOamList, NNSG2dOamChunk *pOamChunk,
                       u16 affineIdx, NNSG2dOamExDrawOrder drawOrderType) {
...
static NNS_G2D_INLINE NNSG2dOamChunk *AdvancePointer_(NNSG2dOamChunk *pChunk,
                                                      u16 num) {
...
static NNS_G2D_INLINE u16 DrawOamChunks_(NNSG2dOamManagerInstanceEx *pMan,
                                         NNSG2dOamChunkList *pOamList,
                                         NNSG2dOamChunk *pChunk, u16 numOam,
                                         u16 capacityOfHW,
...
static NNS_G2D_INLINE void
LoadAffineProxyToBaseModule_(NNSG2dOamManagerInstanceEx *pMan) {
  NNS_G2D_NULL_ASSERT(pMan);
  NNS_G2D_ASSERT(pMan->numAffineBufferUsed != 0);
  ...
  while (numTotalOamDrawn < pMan->numUsedOam) {
    ...
    if (pOamList->numChunks != 0) {
      ...
      if (pOamList->numChunks <= currentCapacity) {
        ...
        if ((pOamList->numDrawn + pOamList->numLastFrameDrawn) /
                pOamList->numChunks >
            0) {
          pMan->lastRenderedOrderingTblIdx = (u16)(i + 1);
        }

        pOamList->numLastFrameDrawn =
            (u16)((pOamList->numDrawn + pOamList->numLastFrameDrawn) %
                  pOamList->numChunks);

        pOamList->numDrawn =
            currentCapacity; // render only what can be rendered
...
BOOL NNS_G2dGetOamManExInstance(NNSG2dOamManagerInstanceEx *pOam,
  ...
  MI_CpuClear32(pOamOrderingTbl,
                lengthOfOrderingTbl * sizeof(NNSG2dOamChunkList));
...
BOOL NNS_G2dEntryOamManExOam(NNSG2dOamManagerInstanceEx *pMan,
...

  {
    ...
    return EntryNewOamWithAffine_(
        pOamList, pOamChunk, NNS_G2D_OAM_AFFINE_IDX_NONE, pMan->drawOrderType);
  }
}
...
BOOL NNS_G2dEntryOamManExOamWithAffineIdx(NNSG2dOamManagerInstanceEx *pMan,
...

  {
    ...
    return EntryNewOamWithAffine_(pOamList, pOamChunk, affineIdx,
                                  pMan->drawOrderType);
  }
}
...

      {
        ...
        for (i = 0; i < pMan->lengthOfOrderingTbl; i++) {
          ...
          if (pOamList->bDrawn) {
            ...
            pChunk =
                AdvancePointer_(pOamList->pChunks, pOamList->numLastFrameDrawn);

            numTotalOamDrawn =
                DrawOamChunks_(pMan, pOamList, pChunk, pOamList->numDrawn,
                               capacityOfHW, numTotalOamDrawn);
          }
        }
      }
    }
