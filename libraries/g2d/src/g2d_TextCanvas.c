...
void NNSi_G2dTextCanvasDrawString(const NNSG2dTextCanvas *pTxn, int x, int y,
  ...
  int charSpace;   // space between characters (the amount of space between the
                 // right edge of the 1st character and the left edge of the 2nd
                 // character)
  ...
  while ((c = getNextChar((const void **)&pos)) != 0) {
    ...
    {
      const int w =
          NNS_G2dCharCanvasDrawChar(pTxn->pCanvas, pFont, x, y, cl, c) +
          charSpace;
#ifdef NNS_G2D_FONT_ENABLE_DIRECTION_SUPPORT
      x += w * d.x;
      y += w * d.y;
...
void NNSi_G2dTextCanvasDrawTextAlign(const NNSG2dTextCanvas *pTxn, int x, int y,
  ...
  int linefeed; // line spacing (the amount of space between the upper edge of
                // the 1st line and the upper edge of the 2nd line)
#ifdef NNS_G2D_FONT_ENABLE_DIRECTION_SUPPORT
  int linefeedx;
  int linefeedy;
  int line;
