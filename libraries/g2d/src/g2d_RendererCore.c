...
#define NNS_G2D_RND_BETWEEN_BEGINEND_ASSERT()                                  \
  NNS_G2D_ASSERTMSG(pTheInstance_ != NULL,                                     \
                    "Please call this method between Begin - End Rendering")

#define NNS_G2D_RND_OUTSIDE_BEGINEND_ASSERT()                                  \
  NNS_G2D_ASSERTMSG(pTheInstance_ == NULL,                                     \
                    "Please call this method outside Begin - End Rendering")

static NNS_G2D_INLINE
    u16 GetMtxCacheAffineIndex_(const NNSG2dRndCore2DMtxCache *pMtxCache,
                                NNSG2dSurfaceType surfaceType,
                                OAM_FLIP flipType) {
  return pMtxCache
      ->affineIndex[flipType][surfaceType - NNS_G2D_SURFACETYPE_MAIN2D];
}

static NNS_G2D_INLINE void
SetMtxCacheAffineIndex_(NNSG2dRndCore2DMtxCache *pMtxCache,
                        NNSG2dSurfaceType surfaceType, OAM_FLIP flipType,
                        u16 affineIndex) {
  pMtxCache->affineIndex[flipType][surfaceType - NNS_G2D_SURFACETYPE_MAIN2D] =
      affineIndex;
}

static NNS_G2D_INLINE BOOL
IsMtxCacheRegisteredAsAffineParams_(const u16 affineIdx) {
...
static NNS_G2D_INLINE BOOL
IsRndCore2DAffineTransformed_(const NNSG2dRndCoreInstance *pRnd) {
  return (BOOL)(pRnd->flipFlag == NNS_G2D_RENDERERFLIP_NONE &&
                pRnd->pCurrentMtxCacheFor2D != NULL);
}

static NNS_G2D_INLINE int
GetShiftToConvByteTo2DChar_(GXOBJVRamModeChar mappingType) {
  int shift =
      (REG_GX_DISPCNT_EXOBJ_MASK & mappingType) >> REG_GX_DISPCNT_EXOBJ_SHIFT;

  return shift;
}

static NNS_G2D_INLINE BOOL IsAlignedShiftValueBoundary_(u32 sizeByte,
                                                        int shiftBit) {
  const int mask = (0x1 << shiftBit) - 1;
  return (BOOL)((mask & sizeByte) == 0);
}

static NNS_G2D_INLINE u32 GetNum2DCharacter_(u32 sizeByte,
                                             GXOBJVRamModeChar mappingType) {

  const int shiftBit =
      5 + GetShiftToConvByteTo2DChar_(mappingType); // 5 means  /= 32
...
static NNS_G2D_INLINE u32
GetPltBaseAddr3D_(const NNSG2dImagePaletteProxy *pPltProxy) {
...
SurfaceTypeToVramType_(NNSG2dSurfaceType surfaceType) {
  return (NNS_G2D_VRAM_TYPE)surfaceType;
}

static NNS_G2D_INLINE u32 GetCharacterBase2D_(const NNSG2dImageProxy *pImgProxy,
...
static NNS_G2D_INLINE void
HandleCellCallBackFunc_(NNSG2dRndCoreDrawCellCallBack pFunc,
...
static NNS_G2D_INLINE void
HandleCellOamBackFunc_(NNSG2dRndCoreDrawOamCallBack pFunc,
                       const NNSG2dCellData *pCell, u16 oamIdx) {
...
static NNS_G2D_INLINE void MakeFlipMtx_(const MtxFx22 *pMtxSrc,
                                        MtxFx22 *pMtxDst, OAM_FLIP type) {
...
static NNS_G2D_INLINE u16 LoadMtxCacheAsAffineParams_(
    NNSG2dRndCore2DMtxCache *pMtxCache, const NNSG2dRndCoreSurface *pSurface,
    OAM_FLIP type) {
...
static NNS_G2D_INLINE u16 GetFlipedOBJPosX_(GXOamAttr *pOam,
                                            const GXOamShape shape) {
  if (NNS_G2dIsRndCoreFlipH(pTheInstance_)) {
    return (u16)(-NNS_G2dRepeatXinCellSpace((s16)pOam->x) -
                 NNS_G2dGetOamSizeX(&shape));
  }

  return (u16)(NNS_G2dRepeatXinCellSpace((s16)pOam->x));
}

static NNS_G2D_INLINE u16 GetFlipedOBJPosY_(GXOamAttr *pOam,
                                            const GXOamShape shape) {
  if (NNS_G2dIsRndCoreFlipV(pTheInstance_)) {
    return (u16)(-NNS_G2dRepeatYinCellSpace((s16)pOam->y) -
                 NNS_G2dGetOamSizeY(&shape));
  }
...
static void DoAffineTransforme_(const MtxFx22 *pMtxSR, GXOamAttr *pOam,
  ...
  if (pTheInstance_->affineOverwriteMode != NNS_G2D_RND_AFFINE_OVERWRITE_NONE) {
    if (pTheInstance_->affineOverwriteMode ==
...

  {
    ...
    const BOOL bShouldAdjust =
        G2_GetOBJEffect(pOam) == GX_OAM_EFFECT_AFFINE_DOUBLE;

    NNSi_G2dAdjustDifferenceOfRotateOrientation(pOam, pMtxSR, &objTrans,
                                                bShouldAdjust);
  }
...
static NNS_G2D_INLINE void DoFlipTransforme_(GXOamAttr *pOam,
                                             NNSG2dFVec2 *pBaseTrans) {
  ...
  NNS_G2D_NULL_ASSERT(pRnd);
  {
    ...
    if (pRnd->pCurrentTargetSurface == NULL) {
      ...
      if (!pRnd->pCurrentTargetSurface->bActive) {
        OS_Warning(
            "RendererCore:The current tragetSurface isn't active. Is it OK?");
      }
    }

    if (pRnd->pCurrentTargetSurface->type == NNS_G2D_SURFACETYPE_MAIN2D ||
        pRnd->pCurrentTargetSurface->type == NNS_G2D_SURFACETYPE_SUB2D) {
      if ((pRnd->pFuncOamAffineRegister == NULL &&
           pRnd->pFuncOamRegister == NULL)) {
        OS_Warning("RendererCore:OAM-RegisterFunction For 2D Graphics Engine "
                   "rendering isn't ready.");
        return FALSE;
      }
    }
  }
...
static void DrawCellToSurface2D_(const NNSG2dRndCoreSurface *pSurface,
                                 const NNSG2dCellData *pCell) {
...

  {
    ...
    for (i = 0; i < pCell->numOAMAttrs; i++) {
      ...
      if (bAffined) {
        ...
        oamAffinIdx = LoadMtxCacheAsAffineParams_(
            pTheInstance_->pCurrentMtxCacheFor2D, pSurface,
      ...
      if (NNSi_G2D_RNDCORE_OAMENTORY_SUCCESSFUL !=
          (*pTheInstance_->pFuncOamRegister)(pTempOam, oamAffinIdx,
...
static void DrawOamToSurface3D_(GXOamAttr *pOam) {
  ...
  if (pTheInstance_->flipFlag != NNS_G2D_RENDERERFLIP_NONE) {
    ...
    NNS_G2dDrawOneOam3DDirectWithPosFast(
        posX, posY, posZ, pOam, &pTheInstance_->pImgProxy->attr,
        pTheInstance_->baseTexAddr3D, pTheInstance_->basePltAddr3D);
  } else {
...
static void DrawCellToSurface3D_(const NNSG2dRndCoreSurface *pSurface,
                                 const NNSG2dCellData *pCell) {
  u16 i = 0;
...
static NNS_G2D_INLINE void DrawCellImpl_(const NNSG2dCellData *pCell,
  ...
  if (pTheInstance_->bDrawEnable) {
    ...
    if (cellVramTransferHandle != NNS_G2D_INVALID_CELL_TRANSFER_STATE_HANDLE) {
      NNSi_G2dSetVramTransferCellDrawnFlag(
          cellVramTransferHandle, ConvertSurfaceTypeToVramType_(pSurface->type),
          TRUE);
    }
...
void NNS_G2dSetRndCoreImageProxy(NNSG2dRndCoreInstance *pRnd,
...
void NNS_G2dSetRndCoreOamRegisterFunc(
    NNSG2dRndCoreInstance *pRnd, NNSG2dOamRegisterFunction pFuncOamRegister,
...
void NNS_G2dSetRndCoreAffineOverwriteMode(
    NNSG2dRndCoreInstance *pRnd, NNSG2dRendererAffineTypeOverwiteMode mode) {
  NNS_G2D_NULL_ASSERT(pRnd);
...
void NNS_G2dSetRndCoreCurrentMtx3D(const MtxFx32 *pCurrentMxt) {
  ...
  NNS_G2D_WARNING(pTheInstance_->pCurrentTargetSurface->type ==
...
static BOOL
CheckMtx2DParamsValid(const MtxFx32 *pMtx,
                      NNSG2dRndCore2DMtxCache *pCurrentMtxCacheFor2D) {
  if (!(pMtx->_00 == FX32_ONE && pMtx->_01 == 0 && pMtx->_10 == 0 &&
        pMtx->_11 == FX32_ONE)) {
    if (pCurrentMtxCacheFor2D == NULL) {
      NNS_G2D_WARNING(
          FALSE, "Make sure that you have to specified the affine-mtx for the "
                 "2D graphics engine when you use affine transformation.");
      return FALSE;
    }
  } else {
  }
...
void NNS_G2dSetRndCoreCurrentMtx2D(
    const MtxFx32 *pMtx, NNSG2dRndCore2DMtxCache *pCurrentMtxCacheFor2D) {
  NNS_G2D_RND_BETWEEN_BEGINEND_ASSERT();
...
void NNS_G2dSetRndCoreSurface(NNSG2dRndCoreInstance *pRnd,
...
void NNS_G2dSetRndCoreFlipMode(NNSG2dRndCoreInstance *pRnd, BOOL bFlipH,
                               BOOL bFlipV) {
...
void NNS_G2dRndCoreBeginRendering(NNSG2dRndCoreInstance *pRnd) {
  ...
  if (pRnd->pCurrentTargetSurface->type == NNS_G2D_SURFACETYPE_MAIN3D) {
    ...
    pTheInstance_->base2DCharOffset = GetCharacterBase2D_(
        pTheInstance_->pImgProxy, pTheInstance_->pCurrentTargetSurface->type);
  }
}
...
void NNS_G2dRndCoreEndRendering(void) {
  ...
  if (pTheInstance_->pCurrentTargetSurface->type ==
      NNS_G2D_SURFACETYPE_MAIN3D) {
...
void NNS_G2dRndCoreDrawCellVramTransfer(const NNSG2dCellData *pCell,
                                        u32 cellVramTransferHandle) {
  NNS_G2D_RND_BETWEEN_BEGINEND_ASSERT();
  NNS_G2D_NULL_ASSERT(pCell);

  NNS_G2D_ASSERT(cellVramTransferHandle !=
...
void NNS_G2dRndCoreDrawCellFast2D(const NNSG2dCellData *pCell) {
...

  {
    ...
    for (i = 0; i < pCell->numOAMAttrs; i++) {
      ...
      NNS_G2D_NULL_ASSERT(pTheInstance_->pFuncOamRegister);
      if (FALSE == (*pTheInstance_->pFuncOamRegister)(
                       pTempOam, NNS_G2D_OAM_AFFINE_IDX_NONE, FALSE)) {
...
void NNS_G2dSetRndCoreCellCloneSource3D(const NNSG2dCellData *pCell) {
...

  {
    ...
    NNS_G2dSetOamSoftEmuSpriteParamCache(pOam, &pTheInstance_->pImgProxy->attr,
