...
static const u8 sBGTextModeTable[4][8] = {
    ...
    },
    {
        ...
        /* 7 -> */ (GXBGMode)(GX_BGMODE_0 +
                              BG_MODE_WARNING), // No configuration permitted
    },
    {

        /* 2 -> */ (GXBGMode)(GX_BGMODE_0 +
                              BG_MODE_WARNING), // BG2: affine      -> text

        /* 4 -> */ (GXBGMode)(GX_BGMODE_0 + BG_MODE_WARNING), // BG2: affine ->
                                                              // text
        /* 5 -> */ (GXBGMode)(GX_BGMODE_0 + BG_MODE_WARNING), // BG2: affine
                                                              // extension ->
                                                              // text
        /* 6 -> */ (GXBGMode)(GX_BGMODE_0 + BG_MODE_WARNING), // BG2: large
                                                              // screen BMP ->
                                                              // text
        /* 7 -> */ (GXBGMode)(GX_BGMODE_0 + BG_MODE_WARNING), // No
                                                              // configuration
                                                              // permitted
    }};
...
static const u8 sBGAffineModeTable[2][8] = {
    {
        /* 0 -> */ (GXBGMode)(GX_BGMODE_2 +
                              BG_MODE_WARNING), // BG2: text  -> affine

        /* 7 -> */ (GXBGMode)(GX_BGMODE_2 + BG_MODE_WARNING), // No
                                                              // configuration
                                                              // permitted
    },
    {
        ...
        /* 5 -> */ (
            GXBGMode)(GX_BGMODE_1 +
                      BG_MODE_WARNING), // BG2: affine extension     -> text
        /* 6 -> */ (GXBGMode)(GX_BGMODE_1 + BG_MODE_WARNING), // BG2: large
                                                              // screen   ->
                                                              // text
        /* 7 -> */ (GXBGMode)(GX_BGMODE_1 + BG_MODE_WARNING), // No
                                                              // configuration
                                                              // permitted
    }};
static const u8 sBG256x16PlttModeTable[2][8] = {
    {
        /* 0 -> */ (GXBGMode)(GX_BGMODE_5 +
                              BG_MODE_WARNING), // BG3: text -> affine extended
        /* 1 -> */ (GXBGMode)(GX_BGMODE_5 + BG_MODE_WARNING), // BG3: affine ->
                                                              // affine extended
        /* 2 -> */ (GXBGMode)(GX_BGMODE_5 + BG_MODE_WARNING), // BG3: affine ->
                                                              // affine extended

        /* 7 -> */ (GXBGMode)(GX_BGMODE_5 + BG_MODE_WARNING), // No
                                                              // configuration
                                                              // permitted
    },
    {

        /* 6 -> */ (GXBGMode)(GX_BGMODE_3 +
                              BG_MODE_WARNING), // BG2: large screen   -> text
        /* 7 -> */ (GXBGMode)(GX_BGMODE_3 + BG_MODE_WARNING), // no
                                                              // configuration
                                                              // permitted
    }};
...
static NNS_G2D_INLINE int CalcTextScreenOffset(int x, int y, int w, int h) {
  ...
  return w * PLANE_HEIGHT * y_blk + PLANE_WIDTH * blk_h * x_blk +
         blk_w * y_char + x_char;
}

static NNS_G2D_INLINE u32 GetCompressedPlttOriginalIndex(
    const NNSG2dPaletteCompressInfo *pCmpInfo, int idx) {
...
static const ScreenSizeMap *SelectScnSize(const ScreenSizeMap tbl[4], int w,
                                          int h) {
  int i;
  SDK_NULL_ASSERT(tbl);
...
static void ChangeBGModeByTableMain(const u8 modeTable[]) {
  ...
  if (mode >= BG_MODE_WARNING) {
    ...
    OS_Warning("Dangerous Main BG mode change: %d => %d", GetBGModeMain(),
               mode);
  }
...
static void
LoadBGPlttToExtendedPltt(NNSG2dBGExtPlttSlot slot,
...
static void LoadBGPlttToNormalPltt(NNSG2dBGSelect bg,
                                   const NNSG2dPaletteData *pPltData,
                                   const NNSG2dPaletteCompressInfo *pCmpInfo) {
  u8 *pDst;
  SDK_NULL_ASSERT(pPltData);
  ...
  if (pCmpInfo != NULL) {
    ...
    NNSi_G2dDmaCopy16(NNS_G2D_DMA_NO, pPltData->pRawData, pDst,
                      pPltData->szByte);
  }
}
...
static void SetBGnControlToText(NNSG2dBGSelect n, GXBGScrSizeText size,
                                GXBGColorMode cmode, GXBGScrBase scnBase,
                                GXBGCharBase chrBase) {
  const int bgNo = GetBGNo(n);
  GXBGExtPltt extPltt = GX_BG_EXTPLTT_01;
  if (IsMainBG(n)) {
    if (!IsMainBGExtPltt01Available())
      extPltt = GX_BG_EXTPLTT_23;
...
static void SetBGnControlToAffine(NNSG2dBGSelect n, GXBGScrSizeAffine size,
                                  GXBGAreaOver areaOver, GXBGScrBase scnBase,
                                  GXBGCharBase chrBase) {
  if (IsMainBG(n)) {
...
static void SetBGnControlTo256x16Pltt(NNSG2dBGSelect n,
...
static void SetBGControlText(NNSG2dBGSelect bg, GXBGColorMode colorMode,
                             int screenWidth, int screenHeight,
                             GXBGScrBase scnBase, GXBGCharBase chrBase) {
  ...
  SetBGnControlToText(bg, (GXBGScrSizeText)pSizeMap->scnSize, colorMode,
                      scnBase, chrBase);
}

static void SetBGControlAffine(NNSG2dBGSelect bg, int screenWidth,
                               int screenHeight, GXBGScrBase scnBase,
                               GXBGCharBase chrBase) {
  const ScreenSizeMap *pSizeMap;

  NNS_G2D_BG_ASSERT(bg);
  SDK_ASSERTMSG((bg % 4) >= 2, "You can not show affine BG on %s BG %d",
  ...
  SetBGnControlToAffine(bg, (GXBGScrSizeAffine)pSizeMap->scnSize,
                        NNSi_G2dBGAreaOver, scnBase, chrBase);
}

static void SetBGControl256x16Pltt(NNSG2dBGSelect bg, int screenWidth,
                                   int screenHeight, GXBGScrBase scnBase,
                                   GXBGCharBase chrBase) {
  const ScreenSizeMap *pSizeMap;
  ...
  SetBGnControlTo256x16Pltt(bg, (GXBGScrSize256x16Pltt)pSizeMap->scnSize,
                            NNSi_G2dBGAreaOver, scnBase, chrBase);
}
...
static void LoadBGPaletteSelect(NNSG2dBGSelect bg, BOOL bToExtPltt,
                                const NNSG2dPaletteData *pPltData,
                                const NNSG2dPaletteCompressInfo *pCmpInfo) {
  NNS_G2D_BG_ASSERT(bg);
  SDK_NULL_ASSERT(pPltData);
...
static void LoadBGPalette(NNSG2dBGSelect bg, const NNSG2dPaletteData *pPltData,
...
static void LoadBGScreen(NNSG2dBGSelect bg, const NNSG2dScreenData *pScnData) {
  ...
  NNS_G2dBGLoadScreenRect(pDstBase, pScnData, 0, 0, 0, 0, plane_cwidth,
                          plane_cheight, load_cwidth, load_cheight);
}

static void SetBGControlAuto(NNSG2dBGSelect bg, NNSG2dScreenFormat screenFormat,
                             GXBGColorMode colorMode, int screenWidth,
                             int screenHeight, GXBGScrBase scnBase,
                             GXBGCharBase chrBase) {

  switch (screenFormat) {
  case NNS_G2D_SCREENFORMAT_TEXT:
    SetBGControlText(bg, colorMode, screenWidth, screenHeight, scnBase,
                     chrBase);
    break;
...
static void LoadScreenPartText(void *pScreenDst,
                               const NNSG2dScreenData *pScnData, int srcX,
                               int srcY, int dstX, int dstY, int dstW, int dstH,
...

  {
    ...
    for (sy = srcY, dy = dstY; sy < src_y_end; sy++, dy++) {
      for (sx = srcX, dx = dstX; sx < src_x_end; sx++, dx++) {
        const GXScrFmtText *pSrc =
            pSrcBase + CalcTextScreenOffset(sx, sy, srcW, srcH);
        GXScrFmtText *pDst =
            pDstBase + CalcTextScreenOffset(dx, dy, dstW, dstH);
...
static void LoadScreenPartAffine(void *pScreenDst,
                                 const NNSG2dScreenData *pScnData, int srcX,
                                 int srcY, int dstX, int dstY, int dstW,
                                 int width, int height) {
  NNS_G2D_POINTER_ASSERT(pScreenDst);
  NNS_G2D_POINTER_ASSERT(pScnData);
...
static void LoadScreenPart256x16Pltt(void *pScreenDst,
                                     const NNSG2dScreenData *pScnData, int srcX,
                                     int srcY, int dstX, int dstY, int dstW,
                                     int width, int height) {
  NNS_G2D_POINTER_ASSERT(pScreenDst);
...
void NNS_G2dBGLoadElementsEx(NNSG2dBGSelect bg,
                             const NNSG2dScreenData *pScnData,
                             const NNSG2dCharacterData *pChrData,
                             const NNSG2dPaletteData *pPltData,
                             const NNSG2dCharacterPosInfo *pPosInfo,
  ...
  NNS_G2D_ASSERTMSG(pPltData == NULL || ((pPltData->fmt == GX_TEXFMT_PLTT16) ==
                                         (pScnData->colorMode ==
...
void NNS_G2dBGSetupEx(NNSG2dBGSelect bg, const NNSG2dScreenData *pScnData,
                      const NNSG2dCharacterData *pChrData,
                      const NNSG2dPaletteData *pPltData,
                      const NNSG2dCharacterPosInfo *pPosInfo,
                      const NNSG2dPaletteCompressInfo *pCmpInfo,
  ...
  SetBGControlAuto(bg, NNSi_G2dBGGetScreenFormat(pScnData),
                   NNSi_G2dBGGetScreenColorMode(pScnData),
                   pScnData->screenWidth, pScnData->screenHeight, scnBase,
                   chrBase);
  NNS_G2dBGLoadElementsEx(bg, pScnData, pChrData, pPltData, pPosInfo, pCmpInfo);
}

void NNS_G2dBGLoadScreenRect(void *pScreenDst, const NNSG2dScreenData *pScnData,
                             int srcX, int srcY, int dstX, int dstY, int dstW,
                             int dstH, int width, int height) {
  NNS_G2D_POINTER_ASSERT(pScreenDst);
  NNS_G2D_POINTER_ASSERT(pScnData);
  ...
  case NNS_G2D_SCREENFORMAT_TEXT:
    LoadScreenPartText(pScreenDst, pScnData, srcX, srcY, dstX, dstY, dstW, dstH,
                       width, height);
    break;

  case NNS_G2D_SCREENFORMAT_AFFINE:
    LoadScreenPartAffine(pScreenDst, pScnData, srcX, srcY, dstX, dstY, dstW,
                         width, height);
    break;

  case NNS_G2D_SCREENFORMAT_AFFINEEXT:
    LoadScreenPart256x16Pltt(pScreenDst, pScnData, srcX, srcY, dstX, dstY, dstW,
                             width, height);
    break;

  default:
    NNS_G2D_ASSERTMSG(FALSE, "Unknown screen format(=%d)",
