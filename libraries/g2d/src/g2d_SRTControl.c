...
#define ASSERT_SRTControlType_VALID(val)                                       \
  NNS_G2D_MINMAX_ASSERT((val), NNS_G2D_SRTCONTROLTYPE_SRT,                     \
                        NNS_G2D_SRTCONTROLTYPE_MTX3D)

#define ASSERT_SRT_MODE(val)                                                   \
  NNS_G2D_ASSERTMSG((val) == NNS_G2D_SRTCONTROLTYPE_SRT,                       \
                    "You can use the function only if its "                    \
                    "NNSG2dSRTControlType is NNS_G2D_SRTCONTROLTYPE_SRT");
...
#pragma unused(pDst)
  ...
  MTX_Rot22(pDst, FX_SinIdx(pCtrl->srtData.rotZ),
            FX_CosIdx(pCtrl->srtData.rotZ));
