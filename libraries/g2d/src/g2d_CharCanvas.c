...
typedef union OBJ1DParam {
  u32 packed;
  struct {
    unsigned baseWidthShift
        : 8; // size of the OBJ in the upper left corner of CharCanvas
    unsigned baseHeightShift
        : 8; // size of the OBJ in the upper left corner of CharCanvas
  };
} OBJ1DParam;
...
static void DrawGlyphLine(const NNSG2dCharCanvas *pCC, const NNSG2dFont *pFont,
                          int x, int y, int cl, const NNSG2dGlyph *pGlyph);
static void DrawGlyph1D(const NNSG2dCharCanvas *pCC, const NNSG2dFont *pFont,
                        int x, int y, int cl, const NNSG2dGlyph *pGlyph);
static void ClearContinuous(const NNSG2dCharCanvas *pCC, int cl);
static void ClearLine(const NNSG2dCharCanvas *pCC, int cl);
static void ClearAreaLine(const NNSG2dCharCanvas *pCC, int cl, int x, int y,
                          int w, int h);
static void ClearArea1D(const NNSG2dCharCanvas *pCC, int cl, int x, int y,
                        int w, int h);
...
static u32 GetCharIndex1D(u32 cx, u32 cy, u32 areaWidth, u32 areaHeight,
                          u32 objWidth, u32 objHeight) {
...
static GXOamShape OBJSizeToShape(const ObjectSize *pSize) {
  const static GXOamShape shape[4][4] = {
      {GX_OAM_SHAPE_8x8, GX_OAM_SHAPE_16x8, GX_OAM_SHAPE_32x8,
       (GXOamShape)NULL},
      {GX_OAM_SHAPE_8x16, GX_OAM_SHAPE_16x16, GX_OAM_SHAPE_32x16,
       (GXOamShape)NULL},
      {GX_OAM_SHAPE_8x32, GX_OAM_SHAPE_16x32, GX_OAM_SHAPE_32x32,
       GX_OAM_SHAPE_64x32},
      {(GXOamShape)NULL, (GXOamShape)NULL, GX_OAM_SHAPE_32x64,
       GX_OAM_SHAPE_64x64},
  };
...
static void ClearChar(void *pChar, int x, int y, int w, int h, u32 cl8,
                      int bpp) {
      ...
      for (; pDst < pDstEnd; pDst += 2) {
        ...
        for (x = x_st; x < x_ed; x += 8) {
          ...
          if (bits != 0) {
            if (x < 32) {
              out_line_0 =
                  (out_line_0 & ~((u32)0xFF << x)) | ((u32)(cl + bits) << x);
            } else {
              const u32 x_32 = x - 32;
              out_line_1 = (out_line_1 & ~((u32)0xFF << x_32)) |
                           ((u32)(cl + bits) << x_32);
            }
          }
        }
...
static void DrawGlyphLine(const NNSG2dCharCanvas *pCC, const NNSG2dFont *pFont,
  ...
  NNS_G2D_MINMAX_ASSERT(
      cl, 0, (1 << pCC->dstBpp) - (1 << NNS_G2dFontGetBpp(pFont)) + 1);
...

  {
    ...
    chara_x_last =
        (u32)(x + glyphWidth + (CHARACTER_WIDTH - 1)) / CHARACTER_WIDTH;
    if (chara_x_last >= areaWidth) {
      chara_x_last = areaWidth;
    }
    chara_y_last =
        (u32)(y + charHeight + (CHARACTER_HEIGHT - 1)) / CHARACTER_HEIGHT;
...

  {
...

    {
      ...
      for (; ofs_y > ofs_y_end; ofs_y -= CHARACTER_HEIGHT) {
        ...
        cx = cx_base;
        for (ofs_x = ofs_x_base; ofs_x > ofs_x_end; ofs_x -= CHARACTER_WIDTH) {
          const unsigned int iChar =
              GetCharIndex1D((u32)cx, (u32)cy, areaWidth, areaHeight,
                             baseWidthShift, baseHeightShift);
...
static void ClearContinuous(const NNSG2dCharCanvas *pCC, int cl) {
  ...
  MI_CpuFillFast(pCC->charBase, data,
...
static void ClearAreaLine(const NNSG2dCharCanvas *pCC, int cl, int x, int y,
...

  {
    ...
    pCharBase = pCC->charBase + ((top / CHARACTER_HEIGHT) * pCC->param +
...
static void ClearArea1D(const NNSG2dCharCanvas *pCC, int cl, int x, int y,
...

  {
    ...
    for (iy = top; iy < bottom; iy += CHARACTER_HEIGHT) {
      ...
      for (ix = left; ix < right; ix += CHARACTER_WIDTH) {
        const u32 iChar =
            GetCharIndex1D((u32)cx, (u32)cy, (u32)areaWidth, (u32)areaHeight,
...
static void InitCharCanvas(NNSG2dCharCanvas *pCC, void *charBase, int areaWidth,
                           int areaHeight, NNSG2dCharaColorMode colorMode,
...
static void SetCharCanvasOBJAttrs(GXOamAttr *pOam, u16 numObj, int priority,
                                  GXOamMode mode, BOOL mosaic,
                                  GXOamEffect effect, int cParam) {
  int i;
...
static void MakeCell(NNSG2dCellData *pCell, GXOamAttr *pOam, u16 numObj, int x,
                     int y, int areaPWidth, int areaPHeight, GXOamEffect effect,
                     BOOL makeBR) {
  ...
  attr |=
      NNSi_G2dSetCellAttrFlipFlag((effect == GX_OAM_EFFECT_FLIP_H) ? 1 : 0,
  ...
  {
    const u8 bsr =
        (u8)(ISqrt(areaPWidth * areaPWidth + areaPHeight * areaPHeight) / 2);
    NNSi_G2dSetCellBoundingSphereR(pCell, bsr);
  }
}

int NNS_G2dCharCanvasDrawChar(const NNSG2dCharCanvas *pCC,
                              const NNSG2dFont *pFont, int x, int y, int cl,
                              u16 ccode) {
  NNSG2dGlyph glyph;
...
void NNS_G2dCharCanvasInitForBG(NNSG2dCharCanvas *pCC, void *charBase,
  ...
  InitCharCanvas(pCC, charBase, areaWidth, areaHeight, colorMode, &VTABLE_BG,
                 (unsigned int)areaWidth);
}

void NNS_G2dCharCanvasInitForOBJ1D(NNSG2dCharCanvas *pCC, void *charBase,
                                   int areaWidth, int areaHeight,
                                   NNSG2dCharaColorMode colorMode) {
  OBJ1DParam p;
  ...
  InitCharCanvas(pCC, charBase, areaWidth, areaHeight, colorMode, &VTABLE_OBJ1D,
                 p.packed);
}

void NNS_G2dCharCanvasInitForOBJ2DRect(NNSG2dCharCanvas *pCC, void *charBase,
                                       int areaWidth, int areaHeight,
                                       NNSG2dCharaColorMode colorMode) {
  ...
  InitCharCanvas(pCC, charBase, areaWidth, areaHeight, colorMode,
                 &VTABLE_OBJ2DRECT, (unsigned int)lineWidth);
}

void NNS_G2dMapScrToCharText(void *scnBase, int areaWidth, int areaHeight,
                             int areaLeft, int areaTop,
                             NNSG2dTextBGWidth scnWidth, int charNo, int cplt) {
  ...
  if (scnWidth <= 32) {
    ...
    NNS_G2dMapScrToChar256x16Pltt(areaBase, areaWidth, areaHeight,
                                  (NNSG2d256x16PlttBGWidth)scnWidth, charNo,
                                  cplt);
  } else {
...
void NNS_G2dMapScrToCharAffine(void *areaBase, int areaWidth, int areaHeight,
                               NNSG2dAffineBGWidth scnWidth, int charNo) {
...
void NNS_G2dMapScrToChar256x16Pltt(void *areaBase, int areaWidth,
                                   int areaHeight,
                                   NNSG2d256x16PlttBGWidth scnWidth, int charNo,
                                   int cplt) {
  u16 *pScrBase;
  int x, y;
  const u16 cplt_sft = (u16)(cplt << 12);
  ...
  NNS_G2D_ASSERT(areaWidth <= scnWidth);
  NNS_G2D_ASSERT(0 <= charNo);
  NNS_G2D_ASSERT(charNo + (areaHeight * areaWidth) <=
                 MAX_256x16PLTT_BG_CHARACTER);
  NNS_G2D_MINMAX_ASSERT(cplt, 0, 15);
...
int NNS_G2dArrangeOBJ1D(GXOamAttr *oam, int areaWidth, int areaHeight, int x,
                        int y, GXOamColorMode color, int charName,
...

  {
    ...
    const int charNameSize =
        ((charNameUnit << objWidthShift) << objHeightShift) >> vramMode;
    int ox, oy;
  ...
  if (areaAWidth < areaWidth) {
    ...
    const int areaBObjs =
        NNS_G2dArrangeOBJ1D(oam, (int)areaBWidth, (int)areaBHeight, px, py,
                            color, charName, vramMode);
  ...
  if (areaAHeight < areaHeight) {
    ...
    const int areaCObjs =
        NNS_G2dArrangeOBJ1D(oam, (int)areaCWidth, (int)areaCHeight, px, py,
                            color, charName, vramMode);
  ...
  if (areaAWidth < areaWidth && areaAHeight < areaHeight) {
    ...
    const int areaDObjs =
        NNS_G2dArrangeOBJ1D(oam, (int)areaDWidth, (int)areaDHeight, px, py,
                            color, charName, vramMode);
...
int NNS_G2dArrangeOBJ2DRect(GXOamAttr *oam, int areaWidth, int areaHeight,
                            int x, int y, GXOamColorMode color, int charName) {
  const u32 fullbits = (u32)(~0);
  const ObjectSize *pSize = GetMaxObjectSize(areaWidth, areaHeight);
  const int objWidthShift = pSize->widthShift;
  const int objHeightShift = pSize->heightShift;
  ...
  {
    const GXOamShape shape = OBJSizeToShape(pSize);
    const unsigned int charNameSize = (u32)(charNameUnit << objWidthShift)
                                      << objHeightShift;
    const int xNum = areaWidth >> objWidthShift;
    const int yNum = areaHeight >> objHeightShift;
    int ox, oy;
  ...
  if (areaAWidth < areaWidth) {
    ...
    const int areaBObjs = NNS_G2dArrangeOBJ2DRect(
        oam, (int)areaBWidth, (int)areaBHeight, px, py, color, (int)cName);
  ...
  if (areaAHeight < areaHeight) {
    ...
    const int areaCObjs = NNS_G2dArrangeOBJ2DRect(
        oam, (int)areaCWidth, (int)areaCHeight, px, py, color, (int)cName);
  ...
  if (areaAWidth < areaWidth && areaAHeight < areaHeight) {
    ...
    const int px = x + (int)areaAWidth * CHARACTER_WIDTH;
    const int py = y + (int)areaAHeight * CHARACTER_HEIGHT;
    const unsigned int cName =
        charName + areaAHeight * 32 + areaAWidth * charNameUnit;

    const int areaDObjs = NNS_G2dArrangeOBJ2DRect(
        oam, (int)areaDWidth, (int)areaDHeight, px, py, color, (int)cName);
...
void NNS_G2dCharCanvasMakeCell1D(NNSG2dCellData *pCell,
                                 const NNSG2dCharCanvas *pCC, int x, int y,
                                 int priority, GXOamMode mode, BOOL mosaic,
                                 GXOamEffect effect, GXOamColorMode color,
                                 int charName, int cParam,
                                 NNSG2dOBJVramMode vramMode, BOOL makeBR) {
  NNS_G2D_POINTER_ASSERT(pCell);
  NNS_G2D_CHARCANVAS_ASSERT(pCC);

  {
    ...
    SetCharCanvasOBJAttrs(pTmpBuffer, numObj, priority, mode, mosaic, effect,
                          cParam);

    (void)NNS_G2dArrangeOBJ1D(pTmpBuffer, areaCWidth, areaCHeight, -x, -y,
                              color, charName, vramMode);

    MakeCell(pCell, pTmpBuffer, numObj, x, y, areaCWidth * CHARACTER_WIDTH,
             areaCHeight * CHARACTER_HEIGHT, effect, makeBR);
  }
}

void NNS_G2dCharCanvasMakeCell2DRect(NNSG2dCellData *pCell,
                                     const NNSG2dCharCanvas *pCC, int x, int y,
                                     int priority, GXOamMode mode, BOOL mosaic,
                                     GXOamEffect effect, GXOamColorMode color,
                                     int charName, int cParam, BOOL makeBR) {
  NNS_G2D_POINTER_ASSERT(pCell);
  NNS_G2D_CHARCANVAS_ASSERT(pCC);

  {
    ...
    (void)NNS_G2dArrangeOBJ2DRect(pTmpBuffer, areaCWidth, areaCHeight, -x, -y,
                                  color, charName);
