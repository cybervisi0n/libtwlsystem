...
IsManageAreaValid_(const NNSG2dOAMManageArea *pArea) {
...
static NNS_G2D_INLINE GXOamAttr *GetOamBufferPtr_(NNSG2dOamType type,
                                                  u16 index) {
  ASSERT_OAMTYPE_VALID(type);
  {
    OamCache *pOamCache = GetOamCachePtr_(type);
...
static NNS_G2D_INLINE GXOamAffine *GetAffineBufferPtr_(NNSG2dOamType type,
                                                       u16 index) {
  ASSERT_OAMTYPE_VALID(type);
...
static NNS_G2D_INLINE void GetAffineParams_(NNSG2dOamType type, u16 idx,
                                            MtxFx22 *pMtx) {
  ASSERT_OAMTYPE_VALID(type);
...
static NNS_G2D_INLINE u16 GetNewGUID_() { return numRegisterdInstance_++; }
...
static NNS_G2D_INLINE void DoReserveArea_(u16 *pResvTblHead, u16 from, u16 to,
  ...
  MI_CpuFill16(pResvTblHead + from, ownerGUID,
               sizeof(u16) * (u32)(to - from + 1));
}
...
static NNS_G2D_INLINE void DoRestoreArea_(u16 *pResvTblHead, u16 from, u16 to) {
  ...
  MI_CpuFill16(pResvTblHead + from, OAM_NOT_USED,
               sizeof(u16) * (u32)(to - from + 1));
}
...
static NNS_G2D_INLINE BOOL HasEnoughCapacity_(const NNSG2dOAMManageArea *pArea,
                                              u16 num) {
  NNS_G2D_NULL_ASSERT(pArea);
...
static NNS_G2D_INLINE GXOamAttr *
GetOamCurrentPtr_(NNSG2dOamManagerInstance *pMan) {
  ...
  NNS_G2D_ASSERT(pMan->managedAttrArea.toIdx < NUM_HW_OAM_ATTR);
  {
    ...
    GXOamAttr *pret =
        GetOamBufferPtr_(pMan->type, pMan->managedAttrArea.currentIdx);
...
static NNS_G2D_INLINE GXOamAttr *
GetOamFromPtr_(NNSG2dOamManagerInstance *pMan) {
  NNS_G2D_NULL_ASSERT(pMan);
  {
    GXOamAttr *pFrom =
        GetOamBufferPtr_(pMan->type, pMan->managedAttrArea.fromIdx);
...
GetSizeOfManageArea_(const NNSG2dOAMManageArea *pArea) {
  NNS_G2D_NULL_ASSERT(pArea);
...
static NNS_G2D_INLINE void SetAffineParams_(NNSG2dOamType type,
                                            const MtxFx22 *mtx, u16 idx) {
  ASSERT_OAMTYPE_VALID(type);
...
static void DrawBy3DGraphicsEngine_(const GXOamAttr *pOam, u16 numOam,
                                    const NNSG2dImageAttr *pTexImageAttr,
                                    u32 texBaseAddr, u32 pltBaseAddr) {
  MtxFx22 mtx;
  u16 affineIdx;
  s16 posX;
  s16 posY;
  ...
  G3_PushMtx();
  {
    ...
    for (i = 0; i < numOam; i++) {
      ...
      if (NNSi_G2dIsRSEnable(&pOam[i])) {
        ...
        NNS_G2dDrawOneOam3DDirectWithPosAffineFast(posX, posY, posZ, &pOam[i],
                                                   pTexImageAttr, texBaseAddr,
                                                   pltBaseAddr, &mtx);
      } else {
        NNS_G2dDrawOneOam3DDirectWithPosFast(posX, posY, posZ, &pOam[i],
                                             pTexImageAttr, texBaseAddr,
                                             pltBaseAddr);
      }
    }
...
static NNS_G2D_INLINE void ClearOamByDefaultValue_(NNSG2dOamType type) {
  ...
  MI_CpuFill16(GetOamBufferPtr_(type, 0), OAM_SETTING_INVISIBLE,
...
static NNS_G2D_INLINE void CpuLoadOAMMain_(const void *pSrc, u32 offset,
                                           u32 szByte) {
...
static NNS_G2D_INLINE void CpuLoadOAMSub_(const void *pSrc, u32 offset,
                                          u32 szByte) {
  NNS_G2D_NULL_ASSERT(pSrc);
  NNS_G2D_ASSERT(offset + szByte <= HW_OAM_SIZE);
...
static NNS_G2D_INLINE void LoadOneAffine_(const GXOamAffine *pAff, u32 offset,
                                          OBJLoadFunction *pOBJLoadFunc) {
  offset += OAMATTR_SIZE;
...
static NNS_G2D_INLINE void LoadOamAndAffineFast_(NNSG2dOamType type,
                                                 u16 fromIdx, u16 toIdx) {
...
static NNS_G2D_INLINE void LoadOam_(NNSG2dOamType type, u16 fromIdx,
                                    u16 toIdx) {
  GXOamAttr *pFrom = GetOamBufferPtr_(type, fromIdx);
...
static NNS_G2D_INLINE void LoadAffine_(NNSG2dOamType type, u16 fromIdx,
                                       u16 toIdx) {
...
static NNS_G2D_INLINE void ResetOam_(NNSG2dOamType type, u16 fromIdx,
                                     u16 toIdx) {
  GXOamAttr *pFrom = GetOamBufferPtr_(type, fromIdx);
  const u16 numArea = (u16)(toIdx - fromIdx + 1);
  u16 i;
...
static NNS_G2D_INLINE void ResetAffine_(NNSG2dOamType type, u16 fromIdx,
                                        u16 toIdx) {
...
BOOL NNS_G2dGetNewManagerInstance(NNSG2dOamManagerInstance *pMan, u16 from,
                                  u16 to, NNSG2dOamType type) {
  ASSERT_OAMTYPE_VALID(type);
  NNS_G2D_ASSERT(from <= to); // MUST BE!
...

  {
    ...
    if (IsOamNotUsed_(pReserveTbl, from, to)) {
      ...
      OS_Warning("Failure in NNS_G2dGetNewManagerInstance().\n The manageArea "
                 "that you specified has been used by someone.");
      return FALSE;
    }
  }
}

BOOL NNS_G2dInitManagerInstanceAffine(NNSG2dOamManagerInstance *pMan, u16 from,
                                      u16 to) {
  NNS_G2D_NULL_ASSERT(pMan);
...

  {
    ...
    if (IsOamNotUsed_(pReserveTbl, from, to)) {
      ...
      OS_Warning("Failure in NNS_G2dInitManagerInstanceAffine().\n The "
                 "manageArea that you specified has been used by someone.");
      return FALSE;
    }
  }
}

BOOL NNS_G2dGetNewOamManagerInstance(NNSG2dOamManagerInstance *pMan,
                                     u16 fromOBJ, u16 numOBJ, u16 fromAffine,
                                     u16 numAffine, NNSG2dOamType type) {
  ASSERT_OAMTYPE_VALID(type);
...
BOOL NNS_G2dGetNewOamManagerInstanceAsFastTransferMode(
    NNSG2dOamManagerInstance *pMan, u16 fromOBJ, u16 numOBJ,
    NNSG2dOamType type) {
  ASSERT_OAMTYPE_VALID(type);
  NNS_G2D_NON_ZERO_ASSERT(numOBJ); // MUST BE!
  NNS_G2D_ASSERT(numOBJ <= NUM_HW_OAM_ATTR);
...

  {
    ...
    if (IsOamNotUsed_(pReserveTbl, fromOBJ, toOBJ)) {
      ...
      OS_Warning(
          "Failure in NNS_G2dGetNewOamManagerInstanceAsFastTransferMode().\n "
...
BOOL NNS_G2dEntryOamManagerOam(NNSG2dOamManagerInstance *pMan,
                               const GXOamAttr *pOam, u16 num) {
  NNS_G2D_NULL_ASSERT(pMan);
  NNS_G2D_NULL_ASSERT(pOam);
  NNS_G2D_ASSERT(num != 0);
  ...
  if (HasEnoughCapacity_(&pMan->managedAttrArea, num)) {
    ...
    NNSI_G2D_DEBUGMSG0(
        "The OamBuffer has no capacity enough to store new Oam.");
    return FALSE;
  }
}

BOOL NNS_G2dEntryOamManagerOamWithAffineIdx(NNSG2dOamManagerInstance *pMan,
...
void NNS_G2dSetOamManagerAffine(NNSG2dOamType type, const MtxFx22 *mtx,
                                u16 idx) {
  ASSERT_OAMTYPE_VALID(type);
  NNS_G2D_NULL_ASSERT(mtx);
  NNS_G2D_MINMAX_ASSERT(idx, 0, NUM_HW_OAM_AFFINE);
  ...
  SDK_WARNING(IsOamNotUsed_(GetAffineReservationTblPtr_(type), idx, idx),
              "An invalid affine param setting to the managed area is "
              "detected. index = %d",
              idx);
...
u16 NNS_G2dEntryOamManagerAffine(NNSG2dOamManagerInstance *pMan,
                                 const MtxFx22 *mtx) {
  NNS_G2D_NULL_ASSERT(pMan);
  NNS_G2D_NULL_ASSERT(mtx);
  ...
  if (HasEnoughCapacity_(&pMan->managedAffineArea, 1)) {
    ...
    NNSI_G2D_DEBUGMSG0(
        "The OamBuffer has no capacity enough to store new Affine Paramater.");
...
void NNS_G2dApplyOamManagerToHW(NNSG2dOamManagerInstance *pMan) {
  ...
  NNS_G2D_ASSERTMSG(pMan->type != NNS_G2D_OAMTYPE_SOFTWAREEMULATION,
                    " For the NNS_G2D_OAMTYPE_SOFTWAREEMULATION type Manager, "
                    "Use NNS_G2dApplyOamManagerToHWSprite() instead.");
  {

    if (pMan->bFastTransferEnable) {
      LoadOamAndAffineFast_(pMan->type, pMan->managedAttrArea.fromIdx,
                            pMan->managedAttrArea.toIdx);
    } else {

      LoadOam_(pMan->type, pMan->managedAttrArea.fromIdx,
               pMan->managedAttrArea.toIdx);

      if (IsManageAreaValid_(&pMan->managedAffineArea)) {
        LoadAffine_(pMan->type, pMan->managedAffineArea.fromIdx,
...
void NNS_G2dApplyOamManagerToHWSprite(NNSG2dOamManagerInstance *pMan,
                                      const NNSG2dImageAttr *pTexImageAttr,
                                      u32 texBaseAddr, u32 pltBaseAddr) {
  NNS_G2D_NULL_ASSERT(pMan);
  NNS_G2D_NULL_ASSERT(pTexImageAttr);
  ...
  NNS_G2D_ASSERTMSG(pMan->type == NNS_G2D_OAMTYPE_SOFTWAREEMULATION,
                    " For the NNS_G2D_OAMTYPE_MAIN SUB type Manager, Use "
                    "NNS_G2dApplyOamManagerToHW() instead.");
  ...
  if (pMan->spriteZoffsetStep != 0) {
...

    {
      ...
      DrawBy3DGraphicsEngine_(pFrom, numOam, pTexImageAttr, texBaseAddr,
                              pltBaseAddr);
    }
...
void NNS_G2dResetOamManagerBuffer(NNSG2dOamManagerInstance *pMan) {
...

  {
    ...
    if (pMan->bFastTransferEnable) {
      ...
      ResetOam_(pMan->type, pMan->managedAttrArea.fromIdx,
                pMan->managedAttrArea.toIdx);
    }
  }
