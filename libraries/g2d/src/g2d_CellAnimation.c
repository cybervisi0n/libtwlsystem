...
static NNS_G2D_INLINE void InitCellAnimationImpl_(
    ...
    const NNSG2dCellDataBank *pCellDataBank, u32 cellTransferStateHandle) {
  NNS_G2D_NULL_ASSERT(pCellAnim);
...
void NNS_G2dInitCellAnimation(NNSG2dCellAnimation *pCellAnim,
  ...
  InitCellAnimationImpl_(pCellAnim, pAnimSeq, pCellDataBank,
                         NNS_G2D_INVALID_CELL_TRANSFER_STATE_HANDLE);
}

void NNS_G2dInitCellAnimationVramTransfered(NNSG2dCellAnimation *pCellAnim,
                                            const NNSG2dAnimSequence *pAnimSeq,
                                            const NNSG2dCellDataBank *pCellBank,
                                            u32 vramStateHandle, u32 dstAddr3D,
                                            u32 dstAddr2DMain, u32 dstAddr2DSub,
                                            const void *pSrcNCGR,
...
void NNS_G2dSetCellAnimationSequenceNoReset(
...

  {
    ...
    if (!NNS_G2dSetAnimCtrlCurrentFrameNoResetCurrentTime(&pCellAnim->animCtrl,
                                                          frameIdx)) {
    ...
    if (pMtxSR != NULL || pBaseTrans != NULL) {
      ...
      if (pMtxSR != NULL) {
        ...
        NNSi_G2dRemovePositionAdjustmentFromDoubleAffineOBJ(pDstOam, &objTrans);
        {
          ...
          NNSi_G2dAdjustDifferenceOfRotateOrientation(pDstOam, pMtxSR,
                                                      &objTrans, bShouldAdjust);
        }
      }
      ...
      G2_SetOBJPosition(pDstOam, (objTrans.x + 0x800) >> FX32_SHIFT,
