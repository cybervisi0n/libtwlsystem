...
static BOOL IsNotCircularLinked_(const NNSG2dRenderSurface *pList,
...
static NNS_G2D_INLINE void FlipTranslate_(int x, int y) {
  const int x_ =
      NNS_G2dIsRndCoreFlipH(&pCurrentInstance_->rendererCore) ? -x : x;
  const int y_ =
      NNS_G2dIsRndCoreFlipV(&pCurrentInstance_->rendererCore) ? -y : y;

  NNS_G2dTranslate(x_ << FX32_SHIFT, y_ << FX32_SHIFT, 0);
}

static NNS_G2D_INLINE void
SetSrtControlToMtxStack_(const NNSG2dSRTControl *pSrtCtrl) {
  NNS_G2D_RND_BETWEEN_BEGINEND_ASSERT(pCurrentInstance_);
  NNS_G2D_NULL_ASSERT(pSrtCtrl);
  NNS_G2D_ASSERTMSG(pSrtCtrl->type == NNS_G2D_SRTCONTROLTYPE_SRT,
                    "TODO: show msg ");
  ...
  if (NNSi_G2dSrtcIsAffineEnable(pSrtCtrl, NNS_G2D_AFFINEENABLE_ROTATE)) {
    NNS_G2dRotZ(FX_SinIdx(pSrtCtrl->srtData.rotZ),
                FX_CosIdx(pSrtCtrl->srtData.rotZ));
  }

  if (NNSi_G2dSrtcIsAffineEnable(pSrtCtrl, NNS_G2D_AFFINEENABLE_SCALE)) {
    NNS_G2dScale(pSrtCtrl->srtData.scale.x, pSrtCtrl->srtData.scale.y,
                 FX32_ONE);
  }
}

static NNS_G2D_INLINE void
BeginDrawVramTransferedCell_(u32 cellVramTransferHandle) {
  NNS_G2D_ASSERT(currenVramTransferHandle_ ==
                 NNS_G2D_INVALID_CELL_TRANSFER_STATE_HANDLE);
...
static NNS_G2D_INLINE BOOL IsRendererDrawingVramTransferedCell_() {
  return (BOOL)(currenVramTransferHandle_ !=
                NNS_G2D_INVALID_CELL_TRANSFER_STATE_HANDLE);
}
...
static void RndCoreCBFuncBeforeCell_(struct NNSG2dRndCoreInstance *pRend,
  ...
  NNS_G2D_NULL_ASSERT(pCurrentInstance_->pCurrentSurface);
  {
    ...
    if (*pCurrentSurface->pBeforeDrawCellBackFunc) {
      (*pCurrentSurface->pBeforeDrawCellBackFunc)(
          pCurrentInstance_, pCurrentSurface, pCell, NNSi_G2dGetCurrentMtx());
    }
  }
}

static void RndCoreCBFuncAfterCell_(struct NNSG2dRndCoreInstance *pRend,
                                    const NNSG2dCellData *pCell) {
#pragma unused(pRend)
  NNS_G2D_NULL_ASSERT(pCurrentInstance_);
  NNS_G2D_NULL_ASSERT(pCurrentInstance_->pCurrentSurface);
...
static void RndCoreCBFuncBeforeOBJ_(struct NNSG2dRndCoreInstance *pRend,
  ...
  if (pCurrentInstance_->overwriteEnableFlag != NNS_G2D_RND_OVERWRITE_NONE) {
    ...
    if (NNS_G2dIsRendererOverwriteEnable(pCurrentInstance_,
                                         NNS_G2D_RND_OVERWRITE_PRIORITY)) {
      pTempOam->priority = pCurrentInstance_->overwritePriority;
    }

    if (NNS_G2dIsRendererOverwriteEnable(pCurrentInstance_,
                                         NNS_G2D_RND_OVERWRITE_PLTTNO)) {
    ...
    if (NNS_G2dIsRendererOverwriteEnable(pCurrentInstance_,
      ...
      pTempOam->cParam =
          0xF & (pTempOam->cParam + pCurrentInstance_->overwritePlttNoOffset);
    }

    if (NNS_G2dIsRendererOverwriteEnable(pCurrentInstance_,
                                         NNS_G2D_RND_OVERWRITE_MOSAIC)) {
      G2_OBJMosaic(pTempOam, pCurrentInstance_->overwriteMosaicFlag);
    }

    if (NNS_G2dIsRendererOverwriteEnable(pCurrentInstance_,
                                         NNS_G2D_RND_OVERWRITE_OBJMODE)) {
      G2_SetOBJMode(pTempOam, pCurrentInstance_->overwriteObjMode,
                    G2_GetOBJColorParam(pTempOam));
    }
  }

  {
    ...
    if (*pCurrentSurface->pBeforeDrawOamBackFunc) {
      (*pCurrentSurface->pBeforeDrawOamBackFunc)(pCurrentInstance_,
                                                 pCurrentSurface, pCell, oamIdx,
                                                 NNSi_G2dGetCurrentMtx());
    }
  }
}

static void RndCoreCBFuncAfterOBJ_(struct NNSG2dRndCoreInstance *pRend,
                                   const NNSG2dCellData *pCell, u16 oamIdx) {
#pragma unused(pRend)
  NNS_G2D_NULL_ASSERT(pCurrentInstance_);
  NNS_G2D_NULL_ASSERT(pCurrentInstance_->pCurrentSurface);
...
static NNS_G2D_INLINE void
BeginRndCoreRendering2D_(NNSG2dRendererInstance *pRnd,
...
static NNS_G2D_INLINE void
BeginRndCoreRendering3D_(NNSG2dRendererInstance *pRnd,
                         NNSG2dRenderSurface *pSurface) {
  NNS_G2D_NULL_ASSERT(pRnd);
  NNS_G2D_ASSERT(pRnd->pCurrentSurface == NULL);
  NNS_G2D_ASSERT(pSurface->type == NNS_G2D_SURFACETYPE_MAIN3D);
...
static NNS_G2D_INLINE void
DoRenderByRndCore2D_(const NNSG2dCellData *pCell,
...

  {
    ...
    if (NNSi_G2dIsRndCurrentMtxSRTransformed()) {
      ...
      if (mcRenderState_.bDrawMC) {
        ...
        pMtx2D = mcRenderState_
                     .cellAnimMtxCacheTbl[mcRenderState_.currentCellAnimIdx];
        ...
        if (pMtx2D == NULL) {
          ...
          mcRenderState_
              .cellAnimMtxCacheTbl[mcRenderState_.currentCellAnimIdx] = pMtx2D;
        }
      } else {
...
static NNS_G2D_INLINE void
DoRenderByRndCore3D_(const NNSG2dCellData *pCell,
                     NNSG2dRndCoreInstance *pRndCore) {
  NNS_G2D_NULL_ASSERT(pCell);
  NNS_G2D_NULL_ASSERT(pRndCore);
...
static void DrawCellAnimationImpl_(const NNSG2dCellAnimation *pCellAnim) {
  ...
  if (pCellAnim->srtCtrl.srtData.SRT_EnableFlag == NNS_G2D_AFFINEENABLE_NONE) {
    ...
    if (NNSi_G2dIsCellAnimVramTransferHandleValid(pCellAnim)) {
      ...
      BeginDrawVramTransferedCell_(
          NNSi_G2dGetCellAnimVramTransferHandle(pCellAnim));
      DrawCellImpl_(pCell);
...
static void DrawNode_(const NNSG2dNode *pNode) {
  ...
  NNS_G2D_ASSERTMSG(pNode->type == NNS_G2D_NODETYPE_CELL,
                    "NNS_G2D_NODETYPE_CELL is expected.");
...
static NNS_G2D_INLINE void
DrawNode2_(const NNSG2dMultiCellInstance *pMC,
...
void NNS_G2dAddRendererTargetSurface(NNSG2dRendererInstance *pRend,
...
void NNS_G2dBeginRenderingEx(NNSG2dRendererInstance *pRendererInstance,
                             u32 opzHint) {
  NNS_G2D_NULL_ASSERT(pRendererInstance);
...

  {
    ...
    if (opzHint & NNS_G2D_RDR_OPZHINT_LOCK_PARAMS) {
      ...
      NNS_G2D_ASSERTMSG(pSurface->pNextSurface == NULL,
                        "The number of target surface must be ONE. when you "
                        "spesified the NNS_G2D_RDR_OPZHINT_LOCK_PARAMS flag.");
  ...
  if (pMC->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
    ...
    if (IsAutoZoffsetEnable_()) {
      ...
      NNSi_G2dSetOamSoftEmuAutoZOffsetStep(
          pCurrentInstance_->spriteZoffsetStep);
...
#pragma unused(z)
  ...
  if (!NNSi_G2dIsRndCurrentMtxSRTransformed()) {
    NNS_G2D_WARNING(pCurrentInstance_->rendererCore.flipFlag ==
                        NNS_G2D_RENDERERFLIP_NONE,
...
void NNS_G2dSetRendererFlipMode(NNSG2dRendererInstance *pRend, BOOL bFlipH,
                                BOOL bFlipV) {
  NNS_G2D_WARNING(
...
void NNS_G2dSetRendererPaletteTbl(NNSG2dRendererInstance *pRend,
                                  const NNSG2dPaletteSwapTable *pTbl) {
  NNS_G2D_NULL_ASSERT(pRend);
...
void NNS_G2dSetRendererImageProxy(NNSG2dRendererInstance *pRend,
  ...
  NNS_G2D_WARNING(!(pRend->opzHint & NNS_G2D_RDR_OPZHINT_LOCK_PARAMS),
                  "Avoid calling this function, when you specified the "
                  "optimize flag NNS_G2D_RDR_OPZHINT_LOCK_PARAMS.");
