...
SetNodeDataToCellAnim_(NNSG2dCellAnimation *pCellAnim,
                       const NNSG2dMultiCellHierarchyData *pNodeData,
                       const NNSG2dAnimBankData *pAnimBank, u16 mcTotalFrame) {
  ...
  if (NNSi_G2dGetMultiCellNodePlayMode(pNodeData) ==
    ...
    if (NNSi_G2dIsAnimCtrlLoopAnim(
      ...
      const u32 frameToMove =
          (mcTotalFrame >= animSeqLength) ? animSeqLength : mcTotalFrame;
      NNS_G2dTickCellAnimation(pCellAnim, (fx32)(frameToMove << FX32_SHIFT));
    }
  }
}

static void SetMCDataToMCInstanceImpl_(NNSG2dMultiCellInstance *pMCInst,
  ...
  if (pMCInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
    ...
    for (i = 0; i < pMcData->numNodes; i++) {
      ...
      if (!pCellAnimArray[cellAnimIdx].bInited) {
        ...
        SetNodeDataToCellAnim_(pCellAnim, pNodeData, pMCInst->pAnimDataBank,
                               mcTotalFrame);
    ...
    for (i = 0; i < pMcData->numNodes; i++) {
      ...
      NNSi_G2dSrtcSetTrans(&pNodeArray[i].srtCtrl, pNodeData->posX,
                           pNodeData->posY);
    }
  }
}
...
static NNS_G2D_INLINE void
ApplyCurrentAnimResult_(NNSG2dMultiCellAnimation *pMultiCellAnim) {
  ...
  pAnimResult = (const NNSG2dAnimDataSRT *)NNS_G2dGetAnimCtrlCurrentElement(
      &pMultiCellAnim->animCtrl);
  NNS_G2D_NULL_ASSERT(pAnimResult);

  pData = NNS_G2dGetMultiCellDataByIdx(pMultiCellAnim->pMultiCellDataBank,
                                       pAnimResult->index);
  NNS_G2D_NULL_ASSERT(pData);

  {
    const NNSG2dAnimationElement elemType = NNSi_G2dGetAnimSequenceElementType(
        pMultiCellAnim->animCtrl.pAnimSequence->animType);
    NNSi_G2dSrtcInitControl(&pMultiCellAnim->srtCtrl,
    ...
    if (elemType != NNS_G2D_ANIMELEMENT_INDEX) {
      if (elemType == NNS_G2D_ANIMELEMENT_INDEX_T) {
        ...
        NNSi_G2dSrtcSetTrans(&pMultiCellAnim->srtCtrl, pAnmResT->px,
                             pAnmResT->py); // T
      } else {
        NNSi_G2dSrtcSetSRTScale(&pMultiCellAnim->srtCtrl, pAnimResult->sx,
                                pAnimResult->sy); // S
        NNSi_G2dSrtcSetSRTRotZ(&pMultiCellAnim->srtCtrl,
                               pAnimResult->rotZ); // R
        NNSi_G2dSrtcSetTrans(&pMultiCellAnim->srtCtrl, pAnimResult->px,
                             pAnimResult->py); // T
      }
    }
  }

  SetMCDataToMCInstanceImpl_(&pMultiCellAnim->multiCellInstance, pData,
                             pMultiCellAnim->totalVideoFrame);

  NNS_G2D_NULL_ASSERT(pMultiCellAnim->multiCellInstance.pCurrentMultiCell);
}

static NNS_G2D_INLINE void FVecToSVec(const NNSG2dFVec2 *pvSrc,
                                      NNSG2dSVec2 *pvDst) {
  pvDst->x = (s16)(pvSrc->x >> FX32_SHIFT);
  pvDst->y = (s16)(pvSrc->y >> FX32_SHIFT);
}

static NNS_G2D_INLINE void SVecToFVec(const NNSG2dSVec2 *pvSrc,
                                      NNSG2dFVec2 *pvDst) {
...
static NNS_G2D_INLINE void
AddSVec_(const NNSG2dSVec2 *pv1, const NNSG2dSVec2 *pv2, NNSG2dSVec2 *pvDst) {
...
static u16 MakeCellAnimToOams_(GXOamAttr *pDstOams, u16 numDstOams,
                               const NNSG2dCellAnimation *pCellAnim,
                               const NNSG2dSVec2 *pNodeTrans,
                               const MtxFx22 *pMtxSR,
                               const NNSG2dFVec2 *pBaseTrans, u16 affineIndex,
                               BOOL bDoubleAffine) {
  NNSG2dFVec2 vTransF;
  NNSG2dSVec2 vTransS;
  ...
  if (NNSi_G2dSrtcIsAffineEnable_SR(pContentsSRT)) {
    ...
    OS_Warning(
        "invalid affine transformation is found in NNS_G2dInitMCAnimation()");
  }

  return NNS_G2dMakeCellToOams(pDstOams, numDstOams,
                               NNS_G2dGetCellAnimationCurrentCell(pCellAnim),
                               pMtxSR, &vTransF, affineIndex, bDoubleAffine);
}

static NNS_G2D_INLINE u16 MakeSimpleMultiCellToOams_(
    GXOamAttr *pDstOams, u16 numDstOams,
    const NNSG2dMultiCellInstance *pMCellInst, const MtxFx22 *pMtxSR,
    const NNSG2dFVec2 *pBaseTrans, u16 affineIndex, BOOL bDoubleAffine) {
  u16 i = 0;
  u16 numOamUsed = 0;
  ...
  for (i = 0; i < pMCellInst->pCurrentMultiCell->numNodes; i++) {
    ...
    if (numRestOams > 0) {
      ...
      numOamUsed =
          MakeCellAnimToOams_(pDstOams, (u16)numRestOams, pNode[i].pContent,
                              &pNode[i].srtCtrl.srtData.trans, pMtxSR,
                              pBaseTrans, affineIndex, bDoubleAffine);
...
static NNS_G2D_INLINE u16 MakeSimpleMultiCellToOams_ShareCellAnims_(
  ...
  pMCCellAnimArray =
      (const NNSG2dMCCellAnimation *)pMCellInst->pCellAnimInstasnces;
  ...
  for (i = 0; i < pMCellInst->pCurrentMultiCell->numNodes; i++) {
    ...
    if (numRestOams > 0) {
      ...
      numOamUsed = MakeCellAnimToOams_(
          pDstOams, (u16)numRestOams, &pMCCellAnimArray[cellAnimIdx].cellAnim,
          &nodeTrans, pMtxSR, pBaseTrans, affineIndex, bDoubleAffine);
...
static NNS_G2D_INLINE void
InitMCInstance_(NNSG2dMultiCellInstance *pMultiCell, NNSG2dNode *pNodeArray,
                NNSG2dCellAnimation *pCellAnim, u16 numNode,
...

  {
    ...
    for (i = 0; i < numNode; i++) {
      ...
      NNS_G2dInitCellAnimation(&pCellAnim[i],
...
static NNS_G2D_INLINE void
InitMCAnimation_(NNSG2dMultiCellAnimation *pMultiCellAnim,
                 NNSG2dNode *pNodeArray, NNSG2dCellAnimation *pCellAnim,
                 u16 numNode, const NNSG2dCellAnimBankData *pAnimBank,
                 const NNSG2dCellDataBank *pCellDataBank,
                 const NNSG2dMultiCellDataBank *pMultiCellDataBank) {
#pragma unused(pMultiCellDataBank)
  ...
  InitMCInstance_(&pMultiCellAnim->multiCellInstance, pNodeArray, pCellAnim,
                  numNode, pAnimBank, pCellDataBank);
...
static NNS_G2D_INLINE void InitMCAnimation_SharingCellAnim_(
    NNSG2dMultiCellAnimation *pMultiCellAnim, void *pWork,
    const NNSG2dCellAnimBankData *pAnimBank,
    const NNSG2dCellDataBank *pCellDataBank,
    const NNSG2dMultiCellDataBank *pMultiCellDataBank) {
  NNS_G2D_NULL_ASSERT(pMultiCellAnim);
  ...
  pMultiCellAnim->multiCellInstance.mcType = NNS_G2D_MCTYPE_SHARE_CELLANIM;
  {
...

    {
      ...
      for (i = 0; i < numCellAnim; i++) {
        NNS_G2dInitCellAnimation(&pMCCellAnim[i].cellAnim,
                                 NNS_G2dGetAnimSequenceByIdx(pAnimBank, 0),
                                 pCellDataBank);
...
void NNS_G2dInitMCAnimationInstance(
    ...
    const NNSG2dCellAnimBankData *pAnimBank,
    const NNSG2dCellDataBank *pCellDataBank,
    const NNSG2dMultiCellDataBank *pMultiCellDataBank, NNSG2dMCType mcType) {
  NNS_G2D_NULL_ASSERT(pMultiCellAnim);
  NNS_G2D_NULL_ASSERT(pWork);
  NNS_G2D_NULL_ASSERT(pAnimBank);
  NNS_G2D_NULL_ASSERT(pCellDataBank);
  ...
  if (mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
    InitMCAnimation_SharingCellAnim_(pMultiCellAnim, pWork, pAnimBank,
                                     pCellDataBank, pMultiCellDataBank);
  } else {
    ...
    InitMCAnimation_(pMultiCellAnim, pNodeArray, pCellAnimArray, numNode,
                     pAnimBank, pCellDataBank, pMultiCellDataBank);
  }
...
u16 NNS_G2dGetMCNumNodesRequired(
...

  {
    ...
    for (i = 0; i < pMultiCellSeq->numFrames; i++) {
      pAnmFrm =
          (const NNSG2dAnimDataSRT *)pMultiCellSeq->pAnmFrameArray[i].pContent;
...
u32 NNS_G2dGetMCWorkAreaSize(const NNSG2dMultiCellDataBank *pMultiCellDataBank,
                             NNSG2dMCType mcType) {
  if (mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
...
void NNS_G2dTickMCInstance(NNSG2dMultiCellInstance *pMCellInst, fx32 frames) {
  ...
  if (pMCellInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
    ...
    for (i = 0; i < pMCellInst->pCurrentMultiCell->numNodes; i++) {
      ...
      NNS_G2dTickCellAnimation((NNSG2dCellAnimation *)pNodeArray[i].pContent,
                               frames);
    }
  }
}

void NNS_G2dTickMCAnimation(NNSG2dMultiCellAnimation *pMultiCellAnim,
                            fx32 frames) {
  NNS_G2D_NULL_ASSERT(pMultiCellAnim);

  {
    const u16 currentAnimFrameFrames =
        pMultiCellAnim->animCtrl.pCurrent->frames;
...
void NNS_G2dSetMCAnimationCurrentFrame(NNSG2dMultiCellAnimation *pMultiCellAnim,
                                       u16 frameIndex) {
  NNS_G2D_NULL_ASSERT(pMultiCellAnim);
...
void NNS_G2dSetMCAnimationCellAnimFrame(
    NNSG2dMultiCellAnimation *pMultiCellAnim, u16 caFrameIndex) {
  u16 i;
...
void NNS_G2dSetMCAnimationSpeed(NNSG2dMultiCellAnimation *pMultiCellAnim,
                                fx32 speed) {
  NNS_G2D_NULL_ASSERT(pMultiCellAnim);
  {
    ...
    if (pMCellInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
      ...
      u16 i = 0;
      NNSG2dNode *pNode =
          (NNSG2dNode *)pMultiCellAnim->multiCellInstance.pCellAnimInstasnces;
      ...
      for (i = 0; i < pMCellInst->pCurrentMultiCell->numNodes; i++) {
        ...
        NNS_G2dSetCellAnimationSpeed((NNSG2dCellAnimation *)pNode[i].pContent,
                                     speed);
      }
    }
  }
    ...
    if (pMCInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
      ...
      for (i = 0; i < pMCInst->pCurrentMultiCell->numNodes; i++) {
        ...
        NNS_G2dSetCellAnimationCurrentFrame(
            (NNSG2dCellAnimation *)pNode[i].pContent, 0);
      }
    }
  }
}
  ...
  NNS_G2D_NULL_ASSERT(pMCInst);
  {
    if (pMCInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
      ...
      for (i = 0; i < pMCInst->pCurrentMultiCell->numNodes; i++) {
        ...
        NNS_G2D_ASSERT(pNode[i].type == NNS_G2D_NODETYPE_CELL);
        pAnmCtrl = NNS_G2dGetCellAnimationAnimCtrl(
            (NNSG2dCellAnimation *)pNode[i].pContent);
...
void NNS_G2dRestartMCAnimation(NNSG2dMultiCellAnimation *pMultiCellAnim) {
  ...
  NNS_G2dSetAnimSequenceToMCAnimation(
      pMultiCellAnim, NNS_G2dGetAnimCtrlCurrentAnimSequence(pMCAnmCtrl));
...
u16 NNS_G2dMakeSimpleMultiCellToOams(GXOamAttr *pDstOams, u16 numDstOams,
                                     const NNSG2dMultiCellInstance *pMCellInst,
                                     const MtxFx22 *pMtxSR,
                                     const NNSG2dFVec2 *pBaseTrans,
                                     u16 affineIndex, BOOL bDoubleAffine) {
  if (pMCellInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
    return MakeSimpleMultiCellToOams_ShareCellAnims_(
        pDstOams, numDstOams, pMCellInst, pMtxSR, pBaseTrans, affineIndex,
        bDoubleAffine);
  } else {
    return MakeSimpleMultiCellToOams_(pDstOams, numDstOams, pMCellInst, pMtxSR,
                                      pBaseTrans, affineIndex, bDoubleAffine);
  }
}

void NNS_G2dTraverseMCCellAnims(NNSG2dMultiCellInstance *pMCellInst,
  ...
  if (pMCellInst->mcType == NNS_G2D_MCTYPE_SHARE_CELLANIM) {
    ...
    for (i = 0; i < pMCellInst->pCurrentMultiCell->numNodes; i++) {
      ...
      bContinue = (*pCBFunc)(userParamater,
                             (NNSG2dCellAnimation *)pNodeArray[i].pContent, i);
...
void NNS_G2dTraverseMCNodes(NNSG2dMultiCellInstance *pMCellInst,
...
void NNS_G2dInitMCAnimation(NNSG2dMultiCellAnimation *pMultiCellAnim,
                            NNSG2dNode *pNodeArray,
                            NNSG2dCellAnimation *pCellAnim, u16 numNode,
                            const NNSG2dCellAnimBankData *pAnimBank,
                            const NNSG2dCellDataBank *pCellDataBank,
                            const NNSG2dMultiCellDataBank *pMultiCellDataBank) {
  InitMCAnimation_(pMultiCellAnim, pNodeArray, pCellAnim, numNode, pAnimBank,
                   pCellDataBank, pMultiCellDataBank);
}

void NNS_G2dInitMCInstance(NNSG2dMultiCellInstance *pMultiCell,
                           NNSG2dNode *pNodeArray,
                           NNSG2dCellAnimation *pCellAnim, u16 numNode,
                           const NNSG2dCellAnimBankData *pAnimBank,
                           const NNSG2dCellDataBank *pCellDataBank) {
  InitMCInstance_(pMultiCell, pNodeArray, pCellAnim, numNode, pAnimBank,
                  pCellDataBank);
}

BOOL NNS_G2dSetMCDataToMCInstance(NNSG2dMultiCellInstance *pMCInst,
                                  const NNSG2dMultiCellData *pMcData) {
