...
#else
  const wchar_t *resolvedPpChar = *ppChar;
  resolvedPpChar++;
#endif
...
u16 NNSi_G2dSplitCharUTF8(const void **ppChar) {
  ...
  NNS_G2D_POINTER_ASSERT(*ppChar);
  NNS_G2D_ASSERT(
      (*(const u8 *)*ppChar & 0xC0) !=
      0x80); // The 1st UTF-8 byte fulfills the conditions described at left.
  ...
  {
    c = (u16)(((*(pChar + 0) & 0x1F) << 6) | ((*(pChar + 1) & 0x3F) << 0));
    *(u32 *)ppChar += 2;
  } else

  {
    ...
    c = (u16)(((*(pChar + 0) & 0x1F) << 12) | ((*(pChar + 1) & 0x3F) << 6) |
              ((*(pChar + 2) & 0x3F) << 0));
    *(u32 *)ppChar += 3;
  }
