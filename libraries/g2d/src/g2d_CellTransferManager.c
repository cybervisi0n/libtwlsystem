...
static NNS_G2D_INLINE BOOL IsCellTransferStateManagerValid_() {
  return (BOOL)((s_pCellStateArray != NULL) && (s_numCellState != 0) &&
...
ResetCellTransferStateDrawnFlag_(NNSG2dCellTransferState *pState) {
  pState->bDrawn = 0x0;
}

static NNS_G2D_INLINE const void *
GetVramTransferSrc_(const NNSG2dCellTransferState *pState,
...
static NNS_G2D_INLINE BOOL IsCellTransferStateValid_(
  ...
  return (
      BOOL)(NNSi_G2dIsVramLocationReadyToUse(&pState->dstVramLocation, type) &&
            IsVramTransferSrcDataValid_(pState, type));
}

static NNS_G2D_INLINE NNS_GFD_DST_TYPE
ConvertVramType_(NNS_G2D_VRAM_TYPE type) {
  NNS_G2D_MINMAX_ASSERT(type, NNS_G2D_VRAM_TYPE_3DMAIN,
...
static NNS_G2D_INLINE void
ResetCellTransferState_(NNSG2dCellTransferState *pState) {
  NNS_G2D_NULL_ASSERT(pState);
  {
...
void NNSi_G2dInitCellTransferState(u32 handle, u32 dstAddr3D, u32 dstAddr2DMain,
...
void NNS_G2dUpdateCellTransferStateManager() {
  ...
  for (i = 0; i < s_numCellState; i++) {
    ...
    if (pState->bActive) {
      ...
      for (type = NNS_G2D_VRAM_TYPE_3DMAIN; type < NNS_G2D_VRAM_TYPE_MAX;
           type++) {
...
void NNS_G2dSetCellTransferStateRequested(u32 handle, u32 srcOffset,
...

  {
    ...
    pState->bTransferRequested =
        0xFFFFFFFF; // memo: when turning on, everything must be turned on
                    // together
