...
static const char *s_elemTypeStrTbl[] = {
    "NNS_G2D_ANIMELEMENT_INDEX", "NNS_G2D_ANIMELEMENT_INDEX_SRT",
    "NNS_G2D_ANIMELEMENT_INDEX_T", "NNS_G2D_ANIMELEMENT_MAX"};
...
static BOOL GetUnpackedAnimBankImpl_(void *pNanrFile,
                                     NNSG2dAnimBankData **ppAnimBank) {
    ...
    NNSG2dBinaryFileHeader *pBinFile = (NNSG2dBinaryFileHeader *)pNanrFile;
    {
      NNSG2dAnimBankDataBlock *pAnimBankBlk =
          (NNSG2dAnimBankDataBlock *)NNS_G2dFindBinaryBlock(
              pBinFile, NNS_G2D_BLKSIG_ANIMBANK);
      if (pAnimBankBlk) {
...
BOOL NNS_G2dGetUnpackedAnimBank(void *pNanrFile,
                                NNSG2dAnimBankData **ppAnimBank) {
  NNS_G2D_NULL_ASSERT(pNanrFile);
  NNS_G2D_NULL_ASSERT(ppAnimBank);

  NNS_G2D_ASSERTMSG(
      NNSi_G2dIsBinFileSignatureValid(pNanrFile, NNS_G2D_BINFILE_SIG_CELLANIM),
      "Input file signature is invalid for this method.");

  NNS_G2D_ASSERTMSG(
      NNSi_G2dIsBinFileVersionValid(pNanrFile, BIN_FILE_VERSION(NANR)),
      "Input file is obsolete. Please use the new g2dcvtr.exe.");

  return GetUnpackedAnimBankImpl_(pNanrFile, ppAnimBank);
}

BOOL NNS_G2dGetUnpackedMCAnimBank(void *pNanrFile,
                                  NNSG2dAnimBankData **ppAnimBank) {
  NNS_G2D_NULL_ASSERT(pNanrFile);
  NNS_G2D_NULL_ASSERT(ppAnimBank);

  NNS_G2D_ASSERTMSG(NNSi_G2dIsBinFileSignatureValid(
                        pNanrFile, NNS_G2D_BINFILE_SIG_MULTICELLANIM),
                    "Input file signature is invalid for this method.");
...
void NNS_G2dUnpackNAN(NNSG2dAnimBankData *pData) {
  ...
  pData->pSequenceArrayHead =
      NNS_G2D_UNPACK_OFFSET_PTR(pData->pSequenceArrayHead, pData);
  pData->pFrameArrayHead =
      NNS_G2D_UNPACK_OFFSET_PTR(pData->pFrameArrayHead, pData);
...

  {
    ...
    for (i = 0; i < pData->numSequences; i++) {
      pSeq[i].pAnmFrameArray =
          NNS_G2D_UNPACK_OFFSET_PTR(pSeq[i].pAnmFrameArray, pFrameBase);

      for (j = 0; j < pSeq[i].numFrames; j++) {
        pSeq[i].pAnmFrameArray[j].pContent = NNS_G2D_UNPACK_OFFSET_PTR(
            pSeq[i].pAnmFrameArray[j].pContent, pContentsBase);
      }

      NNS_G2D_ASSERTMSG(CheckAnimSequenceValidity_(&pSeq[i]),
                        "An invalid anim-sequence is detected.");
    }
  }

  if (pData->pExtendedData != NULL) {
    pData->pExtendedData =
        NNS_G2D_UNPACK_OFFSET_PTR(pData->pExtendedData, pData);
    {
      u32 i = 0;
      u32 j = 0;
      ...
      pAnmExAttrBank->pAnmSeqAttrArray = NNS_G2D_UNPACK_OFFSET_PTR(
          pAnmExAttrBank->pAnmSeqAttrArray, pAnmExAttrBank);
      ...
      for (i = 0; i < pAnmExAttrBank->numSequences; i++) {
        ...
        pSeqAttr->pAttr =
            NNS_G2D_UNPACK_OFFSET_PTR(pSeqAttr->pAttr, pAnmExAttrBank);
        pSeqAttr->pAnmFrmAttrArray = NNS_G2D_UNPACK_OFFSET_PTR(
            pSeqAttr->pAnmFrmAttrArray, pAnmExAttrBank);
...
NNS_G2dGetAnimSequenceByIdx(const NNSG2dAnimBankData *pAnimBank, u16 idx) {
...
void NNS_G2dPrintAnimContents(const void *pData,
                              NNSG2dAnimationElement animElem) {
  NNS_G2D_NULL_ASSERT(pData);
...
void NNS_G2dPrintAnimFrame(const NNSG2dAnimFrameData *pFrame,
                           NNSG2dAnimationElement animElem) {
...
void NNS_G2dPrintAnimSequence(const NNSG2dAnimSequenceData *pSeq) {
  ...
  NNS_G2D_NULL_ASSERT(pSeq);
  {
    ...
    const NNSG2dAnimationElement animElem =
        NNS_G2dGetAnimSequenceElementType(pSeq);

    NNS_G2D_MINMAX_ASSERT(animType, NNS_G2D_ANIMATIONTYPE_CELL,
                          NNS_G2D_ANIMATIONTYPE_MAX);
    NNS_G2D_MINMAX_ASSERT(animElem, NNS_G2D_ANIMELEMENT_INDEX,
