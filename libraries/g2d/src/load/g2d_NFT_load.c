...
BOOL NNSi_G2dGetUnpackedFont(void *pNftrFile, NNSG2dFontInformation **ppRes) {
...
#ifdef NNS_G2D_FONT_USE_OLD_RESOURCE
  if (!NNS_G2dIsBinFileValid(pNftrFile, NNS_G2D_BINFILE_SIG_FONTDATA,
                             NNS_G2D_NFTR_VER)) {
    if (!NNS_G2dIsBinFileValid(pNftrFile, NNS_G2D_BINFILE_SIG_FONTDATA,
                               NNS_G2D_NFTR_VER_1_1)) {
      if (!NNS_G2dIsBinFileValid(pNftrFile, NNS_G2D_BINFILE_SIG_FONTDATA,
                                 NNS_G2D_NFTR_VER_1_0)) {
        OS_Panic(
            "Input file is invalid or obsolete. Please use latest fontcvtr.");
      }
...
#else
  NNS_G2D_ASSERTMSG(NNS_G2dIsBinFileValid(pNftrFile,
...

  {
    ...
    *ppRes = (NNSG2dFontInformation *)((u8 *)pBinBlock + sizeof(*pBinBlock));
    ...
    NNS_G2D_WARNING(
        (*ppRes)->pGlyph->flags == 0,
        "Loading NFTR requires DIRECTION_SUPPORT, but it disabled.");
#endif
  }
...
void NNSi_G2dUnpackNFT(NNSG2dBinaryFileHeader *pHeader) {
    ...
    while (nBlocks < pHeader->dataBlocks) {
...

      {
        NNSG2dFontGlyph *pGlyph =
            (NNSG2dFontGlyph *)((u8 *)pBlock + sizeof(*pBlock));
  ...
  NNS_G2D_ASSERT(pInfo->alterCharIndex <
                 NNS_G2D_FONT_MAX_GLYPH_INDEX((NNSG2dFont *)&pInfo));
}

#ifndef SDK_FINALROM

static void PrintFontInformation(const NNSG2dFontInformation *pInfo) {
  const static char *fontTypeString[] = {"NNS_G2D_FONTTYPE_GLYPH",
...
static void PrintFontCMapScanList(const NNSG2dCMapInfoScan *scan) {
  ...
  for (i = 0; i < scan->num; i++) {
    OS_Printf("   %4d: 0x%04X -> %d\n", i, scan->entries[i].ccode,
              scan->entries[i].index);
  }
}

static void PrintFontCMap(const NNSG2dFontCodeMap *pCMap) {
  const static char *mappingMethodString[] = {
      "NNS_G2D_MAPMETHOD_DIRECT", "NNS_G2D_MAPMETHOD_TABLE",
      "NNS_G2D_MAPMETHOD_SCAN", "NNS_G2D_NUM_OF_MAPMETHOD"};
