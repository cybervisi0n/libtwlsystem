...
BOOL NNS_G2dGetUnpackedMultiCellBank(void *pNmcrFile,
  ...
  NNS_G2D_ASSERTMSG(
      NNSi_G2dIsBinFileSignatureValid(pNmcrFile, NNS_G2D_BINFILE_SIG_MULTICELL),
      "Input file signature is invalid for this method.");

  NNS_G2D_ASSERTMSG(
      NNSi_G2dIsBinFileVersionValid(pNmcrFile, BIN_FILE_VERSION(NMCR)),
    ...
    NNSG2dBinaryFileHeader *pBinFile = (NNSG2dBinaryFileHeader *)pNmcrFile;
    {
      NNSG2dMultiCellDataBankBlock *pBinBlk =
          (NNSG2dMultiCellDataBankBlock *)NNS_G2dFindBinaryBlock(
              pBinFile, NNS_G2D_BLKSIG_MULTICELLBANK);
      if (pBinBlk) {
...

  {
...

    {
      ...
      for (i = 0; i < pMCellData->numMultiCellData; i++) {
        pCell[i].pHierDataArray = NNS_G2D_UNPACK_OFFSET_PTR(
            pCell[i].pHierDataArray, pMCellData->pHierarchyDataArray);
      }
    }
...
NNS_G2dGetMultiCellDataByIdx(const NNSG2dMultiCellDataBank *pDataBank,
                             u16 idx) {
