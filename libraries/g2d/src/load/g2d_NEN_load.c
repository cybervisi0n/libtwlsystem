...
BOOL NNS_G2dGetUnpackedEntityBank(void *pNenrFile,
  ...
  NNS_G2D_ASSERTMSG(
      NNSi_G2dIsBinFileSignatureValid(pNenrFile, NNS_G2D_BINFILE_SIG_ENTITY),
      "Input file signature is invalid for this method.");

  NNS_G2D_ASSERTMSG(
      NNSi_G2dIsBinFileVersionValid(pNenrFile, BIN_FILE_VERSION(NENR)),
      "Input file is obsolete. Please use the new g2dcvtr.exe.");
  {
    NNSG2dBinaryFileHeader *pBinFile = (NNSG2dBinaryFileHeader *)pNenrFile;
    {
      NNSG2dEntityDataBankBlock *pBinBlk =
          (NNSG2dEntityDataBankBlock *)NNS_G2dFindBinaryBlock(
              pBinFile, NNS_G2D_BLKSIG_ENTITYBANK);
      if (pBinBlk) {
...
void NNS_G2dPrintEntityData(const NNSG2dEntityData *pEntity) {
  ...
  for (i = 0; i < pEntity->animData.numAnimSequence; i++) {
    OS_Printf("anim %d ==> %d \n", i,
              pEntity->animData.pAnimSequenceIdxArray[i]);
  }
}
