...
NNS_G2dFindBinaryBlock(NNSG2dBinaryFileHeader *pBinFileHeader, u32 signature) {
...

  {
    NNSG2dBinaryBlockHeader *pCursor =
        (NNSG2dBinaryBlockHeader *)((u32)pBinFileHeader +
                                    (u32)pBinFileHeader->headerSize);
...
void NNSi_G2dUnpackUserExCellAttrBank(NNSG2dUserExCellAttrBank *pCellAttrBank) {
  ...
  pCellAttrBank->pCellAttrArray =
      NNS_G2D_UNPACK_OFFSET_PTR(pCellAttrBank->pCellAttrArray, pCellAttrBank);
  for (i = 0; i < pCellAttrBank->numCells; i++) {
    pCellAttrBank->pCellAttrArray[i].pAttr = NNS_G2D_UNPACK_OFFSET_PTR(
        pCellAttrBank->pCellAttrArray[i].pAttr, pCellAttrBank);
  }
}

#ifndef SDK_FINALROM

void NNSi_G2dPrintUserExCellAttrBank(
  ...
  for (i = 0; i < pCellAttrBank->numCells; i++) {
    NNSG2dUserExCellAttr *pCellAttr = &pCellAttrBank->pCellAttrArray[i];
