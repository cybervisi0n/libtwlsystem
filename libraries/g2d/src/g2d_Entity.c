...
static NNS_G2D_INLINE void SetCurrentAnimation_(NNSG2dEntity *pEntity) {
  ...
  {
    const NNSG2dAnimSequence *pAnimSeq = NNS_G2dGetAnimSequenceByIdx(
        pEntity->pAnimDataBank, pEntity->currentSequenceIdx);
    if (pAnimSeq) {
      switch (pEntity->pEntityData->type) {
      case NNS_G2D_ENTITYTYPE_CELL:
        NNS_G2dSetCellAnimationSequence(
            (NNSG2dCellAnimation *)pEntity->pDrawStuff, pAnimSeq);
        break;
      case NNS_G2D_ENTITYTYPE_MULTICELL:
        NNS_G2dSetAnimSequenceToMCAnimation(
            (NNSG2dMultiCellAnimation *)pEntity->pDrawStuff, pAnimSeq);
        break;
      default:
...
void NNS_G2dInitEntity(NNSG2dEntity *pEntity, void *pDrawStuff,
...
void NNS_G2dSetEntityCurrentAnimation(NNSG2dEntity *pEntity, u16 idx) {
  ...
  if (pEntity->pEntityData->animData.numAnimSequence > idx) {
    pEntity->currentSequenceIdx =
        pEntity->pEntityData->animData.pAnimSequenceIdxArray[idx];

    NNS_G2D_ASSERT(pEntity->pAnimDataBank->numSequences >
                   pEntity->currentSequenceIdx);

    SetCurrentAnimation_(pEntity);
  } else {
    NNSI_G2D_DEBUGMSG0(FALSE, "Failure of finding animation data in "
                              "NNS_G2dSetEntityCurrentAnimation()");
  }
}

void NNS_G2dSetEntityPaletteTable(NNSG2dEntity *pEntity,
...
void NNS_G2dSetEntityCurrentFrame(NNSG2dEntity *pEntity, u16 frameIndex) {
  ...
  case NNS_G2D_ENTITYTYPE_CELL:
    NNS_G2dSetCellAnimationCurrentFrame(
        (NNSG2dCellAnimation *)pEntity->pDrawStuff, frameIndex);
    break;
  case NNS_G2D_ENTITYTYPE_MULTICELL:
    NNS_G2dSetMCAnimationCurrentFrame(
        (NNSG2dMultiCellAnimation *)pEntity->pDrawStuff, frameIndex);
    break;
  default:
    NNS_G2D_ASSERT(FALSE);
  }
...
BOOL NNS_G2dIsEntityValid(NNSG2dEntity *pEntity) {
  ...
  if ((pEntity->pEntityData != NULL) && (pEntity->pDrawStuff != NULL) &&
...
NNSG2dAnimController *NNS_G2dGetEntityAnimCtrl(NNSG2dEntity *pEntity) {
  ...
  switch (pEntity->pEntityData->type) {
  case NNS_G2D_ENTITYTYPE_CELL:
    return NNS_G2dGetCellAnimationAnimCtrl(
        (NNSG2dCellAnimation *)pEntity->pDrawStuff);
  case NNS_G2D_ENTITYTYPE_MULTICELL:
    return NNS_G2dGetMCAnimAnimCtrl(
