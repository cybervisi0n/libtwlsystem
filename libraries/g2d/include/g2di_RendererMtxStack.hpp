...
NNS_G2D_INLINE void NNSi_G2dSetRndMtxStackSRTransformed() {
  NNS_G2D_ASSERTMSG(firstSRTransformedMtxIdx_ == G2Di_RENDERER_MTX_SR_NONE,
                    "firstSRTransformedMtxIdx_ must be set only in the first "
                    "time of using SR transform.");
...
NNS_G2D_INLINE BOOL NNSi_G2dIsStackEmpty() { return (BOOL)(stackPos_ <= 0); }
...
NNS_G2D_INLINE void NNSi_G2dTranslate(fx32 x, fx32 y, fx32 z) {
  if (bDonotUseSRTransform) {
    ...
    NNSi_G2dMtxConcat32(&temp, &mtxStackFor2DHW_[stackPos_],
                        &mtxStackFor2DHW_[stackPos_]);
...
NNS_G2D_INLINE void NNSi_G2dRotate(fx32 sin, fx32 cos) {
  NNS_G2D_ASSERTMSG(!bDonotUseSRTransform,
                    "Renderer module is the -- DonotUseSRTransform -- Mode.You "
                    "can't use SR Transfomation.");
...
NNS_G2D_INLINE fx32 NNSi_G2dGetCurrentZ() { return fxZStack_[stackPos_]; }

NNS_G2D_INLINE u16 NNSi_G2dGetMtxStackPos() { return (u16)(stackPos_); }
