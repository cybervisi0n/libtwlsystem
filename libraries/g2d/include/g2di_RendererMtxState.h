...
typedef enum MCMRndMtxStateType {
  ...
  MCM_MTX_SR_NOT_CACHELOADED, // SR transformed, and loading to matrix cache not
                              // completed
  MCM_MTX_SR_NOT_CACHELOADED_STACKCHANGED, // SR transformed, and loading to
                                           // matrix cache not completed

  MCM_MTX_SR_CACHELOADED // SR transformed, and loading to matrix cache
                         // completed
...
NNS_G2D_INLINE void SetMtxStateMtxCacheIdx_(MCMMtxState *pMtxState,
                                            u16 cacheIdx) {
...
NNS_G2D_INLINE u16 GetNewGroupID_() { return groupID_++; }

NNS_G2D_INLINE void InitGroupID_() { groupID_ = 0; }
...
NNS_G2D_INLINE NNSG2dRndCore2DMtxCache *NNSi_G2dMCMGetCurrentMtxCache() {
  return NNSi_RMCGetMtxCacheByIdx(
      GetMtxStateMtxCacheIdx_(GetCuurentMtxState_()));
}
