...
typedef NNSGfdTexKey (*NNSGfdFuncAllocTexVram)(u32 szByte, BOOL is4x4comp,
                                               u32 opt);
...
NNS_GFD_INLINE NNSGfdTexKey NNS_GfdAllocTexVram(u32 szByte, BOOL is4x4comp,
                                                u32 opt) {
...
NNS_GFD_INLINE u32 NNSi_GfdGetTexKeyRoundupSize(u32 size) {
  if (size == 0) {
    ...
    return (((u32)(size) + (NNS_GFD_TEXSIZE_MIN - 1)) &
            ~(NNS_GFD_TEXSIZE_MIN - 1));
  }
}

NNS_GFD_INLINE NNSGfdTexKey NNS_GfdMakeTexKey(u32 addr, u32 size,
                                              BOOL b4x4Comp) {
