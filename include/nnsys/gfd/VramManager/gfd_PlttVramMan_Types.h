...
#define NNS_GFD_PLTTSIZE_MAX                                                   \
  (NNS_GFD_PLTTMASK_16BIT << NNS_GFD_PLTTKEY_SIZE_SHIFT)
...
typedef NNSGfdPlttKey (*NNSGfdFuncAllocPlttVram)(u32 szByte, BOOL is4pltt,
                                                 u32 opt);
...
NNS_GFD_INLINE NNSGfdPlttKey NNS_GfdAllocPlttVram(u32 szByte, BOOL is4pltt,
                                                  u32 opt) {
...
NNS_GFD_INLINE u32 NNSi_GfdGetPlttKeyRoundupSize(u32 size) {
  if (size == 0) {
    ...
    return (((u32)(size) + (NNS_GFD_PLTTSIZE_MIN - 1)) &
            ~(NNS_GFD_PLTTSIZE_MIN - 1));
  }
}
...
NNS_GFD_INLINE NNSGfdPlttKey NNS_GfdMakePlttKey(u32 addr, u32 size) {
  ...
  SDK_ASSERT(((size >> NNS_GFD_PLTTKEY_SIZE_SHIFT) & ~NNS_GFD_PLTTMASK_16BIT) ==
             0);
  SDK_ASSERT(((addr >> NNS_GFD_PLTTKEY_ADDR_SHIFT) & ~NNS_GFD_PLTTMASK_16BIT) ==
             0);
