...
#if (NNS_G3D_SIZE_MAT_MAX <= 0 ||                                              \
     NNS_G3D_SIZE_MAT_MAX > NNS_G3D_SIZE_MAT_MAX_MAX)
#error NNS_G3D_SIZE_MAT_MAX range error.
#endif

#if (NNS_G3D_SIZE_JNT_MAX <= 0 ||                                              \
     NNS_G3D_SIZE_JNT_MAX > NNS_G3D_SIZE_JNT_MAX_MAX)
#error NNS_G3D_SIZE_JNT_MAX range error.
#endif

#if (NNS_G3D_SIZE_SHP_MAX <= 0 ||                                              \
     NNS_G3D_SIZE_SHP_MAX > NNS_G3D_SIZE_SHP_MAX_MAX)
...
typedef struct NNSG3dAnmObj_ {
  fx32 frame;
  fx32 ratio;
  void *resAnm;  // The pointer to the animation data block inside the resource
                 // file
  void *funcAnm; // Cast in the function pointer of each animation. Takes the
                 // default, but can be changed.
  struct NNSG3dAnmObj_ *next;
  const NNSG3dResTex *resTex; // When the information about the texture  block
                              // is needed (the texture pattern animation only)
  u8 priority;
  u8 numMapData;
  u16 mapData[1]; // Becomes numMapData array (NNSG3dAnmObjMapData)
} NNSG3dAnmObj;

#define NNS_G3D_ANMOBJ_SIZE_MATANM(pMdl)                                       \
  ((sizeof(NNSG3dAnmObj) + sizeof(u16) * pMdl->info.numMat) & ~3)

#define NNS_G3D_ANMOBJ_SIZE_JNTANM(pMdl)                                       \
  ((sizeof(NNSG3dAnmObj) + sizeof(u16) * pMdl->info.numNode) & ~3)
#define NNS_G3D_ANMOBJ_SIZE_VISANM(pMdl)                                       \
  ((sizeof(NNSG3dAnmObj) + sizeof(u16) * pMdl->info.numNode) & ~3)
...
typedef struct NNSG3dRenderObj_ {
  ...
  u8 cbCmd;    // Designate the stopping position with the command.
               // NNS_G3D_SBC_***** (defined in res_struct.h)
...
NNS_G3D_INLINE void NNS_G3dAnmObjSetBlendRatio(NNSG3dAnmObj *pAnmObj,
                                               fx32 ratio);
...
NNS_G3D_INLINE void NNS_G3dRenderObjSetFlag(NNSG3dRenderObj *pRenderObj,
                                            NNSG3dRenderObjFlag flag);
NNS_G3D_INLINE void NNS_G3dRenderObjResetFlag(NNSG3dRenderObj *pRenderObj,
                                              NNSG3dRenderObjFlag flag);
NNS_G3D_INLINE BOOL NNS_G3dRenderObjTestFlag(const NNSG3dRenderObj *pRenderObj,
                                             NNSG3dRenderObjFlag flag);
NNS_G3D_INLINE u8 *NNS_G3dRenderObjSetUserSbc(NNSG3dRenderObj *pRenderObj,
                                              u8 *sbc);
NNS_G3D_INLINE void
...
NNS_G3D_INLINE void *NNS_G3dRenderObjSetUserPtr(NNSG3dRenderObj *pRenderObj,
                                                void *ptr);
...
void NNS_G3dRenderObjSetCallBack(NNSG3dRenderObj *pRenderObj,
                                 NNSG3dSbcCallBackFunc func, u8 *, u8 cmd,
