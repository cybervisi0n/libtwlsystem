...
NNS_G3dGetResNameByIdx(const NNSG3dResDict *dict, u32 idx) {
  ...
  if (dict != NULL && idx < dict->numEntry) {
    ...
    return (NNSG3dResName *)((u8 *)hdr + hdr->ofsName +
...
NNS_G3D_RES_INLINE void *NNS_G3dGetResDataByIdx(const NNSG3dResDict *dict,
                                                u32 idx) {
  NNSG3dResDictEntryHeader *hdr;
  NNS_G3D_NULL_ASSERT(dict);
...
NNS_G3dGetNodeIdxByName(const NNSG3dResNodeInfo *nodeinfo,
                        const NNSG3dResName *name) {
  NNS_G3D_NULL_ASSERT(nodeinfo);
...
NNS_G3dGetMdlByIdx(const NNSG3dResMdlSet *mdlSet, u32 idx) {
  ...
  if (mdlSet) {
    data =
        (NNSG3dResDictMdlSetData *)NNS_G3dGetResDataByIdx(&mdlSet->dict, idx);
    if (data) {
...
NNS_G3dGetMdlByName(const NNSG3dResMdlSet *mdlSet, const NNSG3dResName *name) {
  ...
  if (mdlSet) {
    data =
        (NNSG3dResDictMdlSetData *)NNS_G3dGetResDataByName(&mdlSet->dict, name);
    if (data) {
      NNS_G3D_ASSERT(data->offset != 0);
      return (NNSG3dResMdl *)((u8 *)mdlSet + data->offset);
    }
