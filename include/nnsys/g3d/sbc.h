...
typedef enum {
  NNS_G3D_RSFLAG_NODE_VISIBLE =
      0x00000001, // Turned on and off by the NODE command
  NNS_G3D_RSFLAG_MAT_TRANSPARENT =
      0x00000002, // Turned on and off by the MAT command
  NNS_G3D_RSFLAG_CURRENT_NODE_VALID =
      0x00000004, // Turned on by the NODE command
  ...
  NNS_G3D_RSFLAG_SKIP = 0x00000040,   // User flag for callbacks (skips to the
                                      // next callback in the command).

  NNS_G3D_RSFLAG_OPT_RECORD =
      0x00000080, // Stores calculation results in NNSG3dRenderObj > recJntAnm,
                  // recMatAnm.
  NNS_G3D_RSFLAG_OPT_NOGECMD = 0x00000100, // Does not send geometry commands.
  NNS_G3D_RSFLAG_OPT_SKIP_SBCDRAW =
      0x00000200, // Skips Draw type SBC commands. Also skips callbacks.
  NNS_G3D_RSFLAG_OPT_SKIP_SBCMTXCALC =
      0x00000400 // Skips MtxCalc type SBC commands. Also skips callbacks.
...
typedef struct NNSG3dRS_ {
  ...
  NNSG3dSbcCallBackFunc
      cbVecFunc[NNS_G3D_SBC_COMMAND_NUM];  // Callback function vector
...
typedef void (*NNSG3dFuncSbc_MatInternal)(NNSG3dRS *, u32,
                                          const NNSG3dResMatData *, u32);
typedef void (*NNSG3dFuncSbc_ShpInternal)(NNSG3dRS *, u32,
                                          const NNSG3dResShpData *, u32);

#define NNS_G3D_MTXSTACK_SYS (30)
#define NNS_G3D_MTXSTACK_USER (29)

NNS_G3D_INLINE void NNS_G3dRSSetCallBack(NNSG3dRS *rs,
                                         NNSG3dSbcCallBackFunc func, u8 cmd,
...
void NNSi_G3dFuncSbc_SHP_InternalDefault(NNSG3dRS *rs, u32 opt,
...
NNS_G3D_INLINE BOOL NNSi_G3dCallBackCheck_A(NNSG3dRS *rs, u8 cmd,
                                            NNSG3dSbcCallBackTiming *pTiming);
NNS_G3D_INLINE BOOL NNSi_G3dCallBackCheck_B(NNSG3dRS *rs, u8 cmd,
                                            NNSG3dSbcCallBackTiming *pTiming);
NNS_G3D_INLINE BOOL NNSi_G3dCallBackCheck_C(NNSG3dRS *rs, u8 cmd,
                                            NNSG3dSbcCallBackTiming timing);
...
extern NNSG3dFuncSbc_MatInternal
    NNS_G3dFuncSbcMatTable[NNS_G3D_SIZE_MAT_VTBL_NUM];
extern NNSG3dFuncSbc_ShpInternal
    NNS_G3dFuncSbcShpTable[NNS_G3D_SIZE_SHP_VTBL_NUM];
