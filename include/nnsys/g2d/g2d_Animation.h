...
#define NNS_G2dSetCallBackFunctorAtAnimFrame                                   \
...
#define NNS_G2dInitAnimControllerCallBackFunctor                               \
...
#define NNS_G2D_ASSERT_ANIMATIONTYPE_VALID(val)                                \
  NNS_G2D_MINMAX_ASSERT(val, NNS_G2D_ANIMATIONTYPE_CELL,                       \
...
typedef enum NNSG2dAnmCallbackType {
  ...
  NNS_G2D_ANMCALLBACKTYPE_LAST_FRM, // Call when last frame of animation
                                    // sequence finishes.
...
typedef struct NNSG2dCallBackFunctor {
  ...
  u16 frameIdx;               // frame number (used when type ==
...
typedef struct NNSG2dAnimController {
  ...
  const NNSG2dAnimFrame *pActiveCurrent; // current animation frame (limited to
                                         // valid display targets)
  ...
  NNSG2dAnimationPlayMode overriddenPlayMode; // Set when the programmer
                                              // overrides the playback mode.
...
fx32 NNS_G2dGetAnimCtrlNormalizedTime(
    NNSG2dAnimController *pAnimCtrl); // Testing
...
static NNSG2dAnimationType
NNS_G2dGetAnimCtrlType(const NNSG2dAnimController *pAnimCtrl);

static void NNS_G2dSetAnimCtrlSpeed(NNSG2dAnimController *pAnimCtrl,
                                    fx32 speed);
...
static void
NNS_G2dSetAnimCtrlPlayModeOverridden(NNSG2dAnimController *pAnimCtrl,
                                     NNSG2dAnimationPlayMode playMode);

static void
NNS_G2dResetAnimCtrlPlayModeOverridden(NNSG2dAnimController *pAnimCtrl);

static fx32
NNS_G2dGetAnimCtrlCurrentTime(const NNSG2dAnimController *pAnimCtrl);

static void NNS_G2dSetAnimCtrlCurrentTime(NNSG2dAnimController *pAnimCtrl,
                                          fx32 time);

static u16
NNS_G2dGetAnimCtrlCurrentElemIdxVal(const NNSG2dAnimController *pAnimCtrl);

static const NNSG2dAnimSequence *
NNS_G2dGetAnimCtrlCurrentAnimSequence(const NNSG2dAnimController *pAnimCtrl);
