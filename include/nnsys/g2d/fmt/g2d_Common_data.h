...
#define NNS_G2D_MINMAX_ASSERT(x, min, max)                                     \
  assert(((x) >= (min)) && ((x) <= (max)))
...
NNS_G2D_INLINE const NNSG2dUserExDataBlock *
NNSi_G2dGetUserExDataBlkByID(const void *pExtendData, u32 blkTypeID) {
  if (pExtendData != NULL) {
    const NNSG2dUserExDataBlock *pBlk =
        (const NNSG2dUserExDataBlock *)pExtendData;
