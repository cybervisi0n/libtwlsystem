...
#define NNS_G2D_OAM_AFFINE_IDX_NONE                                            \
  0xFFFE // The Affine Index value to specify when registering an OAM that does
         // not use Affine in the OAM manager.
...
NNS_G2D_INLINE GXOamShape NNS_G2dGetOAMSize(const GXOamAttr *oamAttr) {
  ...
  const GXOamShape result =
      (GXOamShape)((GX_OAM_ATTR01_SHAPE_MASK | GX_OAM_ATTR01_SIZE_MASK) &
                   oamAttr->attr01);
...
#endif // NNS_FROM_TOOL
  return NNSi_objSizeWTbl[(*oamShape & GX_OAM_ATTR01_SHAPE_MASK) >>
...
NNS_G2D_INLINE int NNS_G2dGetOamSizeY(const GXOamShape *oamShape) {
  ...
  return NNSi_objSizeHTbl[(*oamShape & GX_OAM_ATTR01_SHAPE_MASK) >>
                          GX_OAM_ATTR01_SHAPE_SHIFT]
                         [(*oamShape & GX_OAM_ATTR01_SIZE_MASK) >>
                          GX_OAM_ATTR01_SIZE_SHIFT];
}
