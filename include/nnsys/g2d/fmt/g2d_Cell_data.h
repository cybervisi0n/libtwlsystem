...
#define NNS_G2D_CELLBK_ATTR_CELLWITHBR                                         \
  0x1 // Take & with NNSG2dCellDataBank.cellBankAttr

#define NNS_G2D_NCER_MAJOR_VER 1
#define NNS_G2D_NCER_MINOR_VER 0

#define NNS_G2dGetSizeReqiredVramTransferCellDataBank                          \
...
typedef struct NNSG2dVramTransferData {
  ...
  NNSG2dCellVramTransferData
      *pCellTransferDataArray; // This is the length of the numCells amount of
                               // the maintained NNSG2dCellDataBank.
...
typedef struct NNSG2dCellData {
  ...
  NNSG2dCellOAMAttrData
      *pOamAttrArray; // Pointer to the NNSG2dCellOAMAttrData array
...
typedef struct NNSG2dCellDataBank {
  ...
  NNSG2dCharacterDataMapingType
      mappingMode; // Mapping mode of the referenced character

  NNSG2dVramTransferData
      *pVramTransferData; // Information related to Vram transfer animation
...
NNS_G2D_INLINE void NNSi_G2dSetCellBoundingSphereR(NNSG2dCellData *pCell,
                                                   u8 R) {
...
NNS_G2D_INLINE u8 NNS_G2dGetCellBoundingSphereR(const NNSG2dCellData *pCell) {
  u8 R = (u8)((pCell->cellAttr >> NNS_G2D_CELL_BS_R_SHIFT) &
...
NNS_G2D_INLINE BOOL
NNS_G2dCellDataBankHasVramTransferData(const NNSG2dCellDataBank *pCellBank) {

  return (BOOL)(pCellBank->pVramTransferData != NULL);
}

NNS_G2D_INLINE u32 NNS_G2dGetSizeRequiredVramTransferCellDataBank(
...
NNS_G2dGetUserExCellAttrBankFromCellBank(const NNSG2dCellDataBank *pCellBank) {
  ...
  const NNSG2dUserExDataBlock *pBlk = NNSi_G2dGetUserExDataBlkByID(
      pCellBank->pExtendedData, NNS_G2D_USEREXBLK_CELLATTR);
...
NNS_G2dGetUserExCellAttr(const NNSG2dUserExCellAttrBank *pCellAttrBank,
                         u16 idx) {
...
NNS_G2dGetUserExCellAttrValue(const NNSG2dUserExCellAttr *pCellAttr) {
  return pCellAttr->pAttr[0];
}

NNS_G2D_INLINE void NNS_G2dCopyCellAsOamAttr(const NNSG2dCellData *pCell,
                                             u16 idx, GXOamAttr *pDst) {
