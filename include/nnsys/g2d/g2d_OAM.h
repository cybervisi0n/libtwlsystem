...
#define ASSERT_OAMTYPE(x)                                                      \
  NNS_G2D_ASSERT((x) == NNS_G2D_OAMTYPE_INVALID ||                             \
                 (x) == NNS_G2D_OAMTYPE_MAIN || (x) == NNS_G2D_OAMTYPE_SUB ||  \
                 (x) == NNS_G2D_OAMTYPE_SOFTWAREEMULATION ||                   \
                 (x) == NNS_G2D_OAMTYPE_MAX)

#define ASSERT_OAMTYPE_VALID(x)                                                \
  NNS_G2D_ASSERT((x) == NNS_G2D_OAMTYPE_MAIN || (x) == NNS_G2D_OAMTYPE_SUB ||  \
                 (x) == NNS_G2D_OAMTYPE_SOFTWAREEMULATION)
...
typedef struct NNSG2dOamManagerInstance {
  ...
  NNSG2dOAMManageArea
      managedAttrArea; // Manager management area (Oam Attribute)
  NNSG2dOAMManageArea
      managedAffineArea; // Manager management area (Affine Parameter)

  u16 GUID;                 // ID of NNSG2dOamManagerInstance
  u16 pad16_;               // Padding
  BOOL bFastTransferEnable; // Set during initialization. (User should not
                            // manipulate this.)
  fx32 spriteZoffsetStep;   // The value of the automatic offset used when the
                            // sprite is rendered
...
NNS_G2D_INLINE void
NNS_G2dSetOamManagerSpriteZoffsetStep(NNSG2dOamManagerInstance *pMan,
                                      fx32 zStep) {
  ...
  NNS_G2D_WARNING(pMan->type == NNS_G2D_OAMTYPE_SOFTWAREEMULATION,
                  "This function works only for the manager initialized as "
                  "NNS_G2D_OAMTYPE_SOFTWAREEMULATION type ");
