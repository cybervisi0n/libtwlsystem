...
#define NNS_G2D_TEXTCANVAS_ASSERT(pTxn)                                        \
  NNS_G2D_ASSERTMSG(NNS_G2D_IS_VALID_POINTER(pTxn) &&                          \
...
#define NNS_G2D_CHARENCODING_ASSERT(enc)                                       \
  NNS_G2D_ASSERTMSG((0 <= (enc)) && ((enc) < NNS_G2D_NUM_OF_CHARENCODING),     \
...
void NNSi_G2dTextCanvasDrawTaggedText(const NNSG2dTextCanvas *pTxn, int x,
                                      int y, int cl, const void *txt,
...
NNS_G2D_INLINE void NNS_G2dTextCanvasInit(NNSG2dTextCanvas *pTxn,
                                          const NNSG2dCharCanvas *pCC,
                                          const NNSG2dFont *pFont, int hSpace,
                                          int vSpace) {
...
NNS_G2D_INLINE const NNSG2dCharCanvas *
NNS_G2dTextCanvasGetCharCanvas(const NNSG2dTextCanvas *pTxn) {
  NNS_G2D_TEXTCANVAS_ASSERT(pTxn);
  return pTxn->pCanvas;
}

NNS_G2D_INLINE const NNSG2dFont *
NNS_G2dTextCanvasGetFont(const NNSG2dTextCanvas *pTxn) {
  NNS_G2D_TEXTCANVAS_ASSERT(pTxn);
  return pTxn->pFont;
}
...
NNS_G2D_INLINE void
NNS_G2dTextCanvasSetCharCanvas(NNSG2dTextCanvas *pTxn,
                               const NNSG2dCharCanvas *pCC) {
  NNS_G2D_TEXTCANVAS_ASSERT(pTxn);
  NNS_G2D_CHARCANVAS_ASSERT(pCC);
  pTxn->pCanvas = pCC;
...
NNS_G2D_INLINE void NNS_G2dTextCanvasSetFont(NNSG2dTextCanvas *pTxn,
                                             const NNSG2dFont *pFont) {
  NNS_G2D_TEXTCANVAS_ASSERT(pTxn);
  NNS_G2D_FONT_ASSERT(pFont);
  pTxn->pFont = pFont;
...
NNS_G2D_INLINE void NNS_G2dTextCanvasSetHSpace(NNSG2dTextCanvas *pTxn,
                                               int hSpace) {
  NNS_G2D_TEXTCANVAS_ASSERT(pTxn);
  pTxn->hSpace = hSpace;
}

NNS_G2D_INLINE void NNS_G2dTextCanvasSetVSpace(NNSG2dTextCanvas *pTxn,
                                               int vSpace) {
  NNS_G2D_TEXTCANVAS_ASSERT(pTxn);
  pTxn->vSpace = vSpace;
}

NNS_G2D_INLINE int NNS_G2dTextCanvasGetStringWidth(const NNSG2dTextCanvas *pTxn,
                                                   const NNSG2dChar *str,
                                                   const NNSG2dChar **pPos) {
  return NNS_G2dFontGetStringWidth(pTxn->pFont, pTxn->hSpace, str, pPos);
}

NNS_G2D_INLINE int NNS_G2dTextCanvasGetTextHeight(const NNSG2dTextCanvas *pTxn,
                                                  const NNSG2dChar *txt) {
  return NNS_G2dFontGetTextHeight(pTxn->pFont, pTxn->vSpace, txt);
}

NNS_G2D_INLINE int NNS_G2dTextCanvasGetTextWidth(const NNSG2dTextCanvas *pTxn,
                                                 const NNSG2dChar *txt) {
  return NNS_G2dFontGetTextWidth(pTxn->pFont, pTxn->hSpace, txt);
}

NNS_G2D_INLINE NNSG2dTextRect NNS_G2dTextCanvasGetTextRect(
    const NNSG2dTextCanvas *pTxn, const NNSG2dChar *txt) {
  return NNS_G2dFontGetTextRect(pTxn->pFont, pTxn->hSpace, pTxn->vSpace, txt);
}

NNS_G2D_INLINE NNSiG2dTextDirection
NNSi_G2dGetTextDirection(const NNSG2dFont *pFont) {
...
#ifdef NNS_G2D_FONT_ENABLE_DIRECTION_SUPPORT
NNS_G2D_INLINE void NNS_G2dTextCanvasDrawString(const NNSG2dTextCanvas *pTxn,
                                                int x, int y, int cl,
                                                const NNSG2dChar *str,
                                                const NNSG2dChar **pPos) {
  NNSi_G2dTextCanvasDrawString(
      pTxn, x, y, cl, str, (const void **)pPos,
      NNSi_G2dGetTextDirection(NNS_G2dTextCanvasGetFont(pTxn)));
}
NNS_G2D_INLINE void NNS_G2dTextCanvasDrawText(const NNSG2dTextCanvas *pTxn,
                                              int x, int y, int cl, u32 flags,
                                              const NNSG2dChar *txt) {
  NNSi_G2dTextCanvasDrawText(
      pTxn, x, y, cl, flags, txt,
      NNSi_G2dGetTextDirection(NNS_G2dTextCanvasGetFont(pTxn)));
}
NNS_G2D_INLINE void NNS_G2dTextCanvasDrawTextRect(const NNSG2dTextCanvas *pTxn,
                                                  int x, int y, int w, int h,
                                                  int cl, u32 flags,
                                                  const NNSG2dChar *txt) {
  NNSi_G2dTextCanvasDrawTextRect(
      pTxn, x, y, w, h, cl, flags, txt,
      NNSi_G2dGetTextDirection(NNS_G2dTextCanvasGetFont(pTxn)));
}
NNS_G2D_INLINE void
NNS_G2dTextCanvasDrawTaggedText(const NNSG2dTextCanvas *pTxn, int x, int y,
                                int cl, const NNSG2dChar *txt,
                                NNSG2dTagCallback cbFunc, void *cbParam) {
  NNSi_G2dTextCanvasDrawTaggedText(
      pTxn, x, y, cl, txt, cbFunc, cbParam,
      NNSi_G2dGetTextDirection(NNS_G2dTextCanvasGetFont(pTxn)));
}
#else
NNS_G2D_INLINE void NNS_G2dTextCanvasDrawString(const NNSG2dTextCanvas *pTxn,
