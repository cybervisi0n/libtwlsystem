...
#define NNS_G2D_OAMFLIP_PATTERN_NUM                                            \
  0x04 // ( OAM_FLIP_NONE, OAM_FLIP_H, OAM_FLIP_V, OAM_FLIP_HV )

typedef enum NNSG2dRendererAffineTypeOverwiteMode {
  NNS_G2D_RND_AFFINE_OVERWRITE_NONE,   // Does not overwrite.
  NNS_G2D_RND_AFFINE_OVERWRITE_NORMAL, // Sets to normal affine transformation
                                       // method
...
typedef struct NNSG2dRndCore2DMtxCache {
  MtxFx22 m22;
  u16 affineIndex[NNS_G2D_OAMFLIP_PATTERN_NUM]
                 [NNS_G2D_NUMBER_OF_2DGRAPHICS_ENGINE];
...
typedef BOOL (*NNSG2dOamRegisterFunction)(const GXOamAttr *pOam,
...
typedef void (*NNSG2dRndCoreDrawCellCallBack)(
    struct NNSG2dRndCoreInstance *pRend, const NNSG2dCellData *pCell);
...
typedef struct NNSG2dRndCoreSurface {
  ...
  NNSG2dRndCoreDrawCellCallBack
      pBeforeDrawCellBackFunc; // includes visibility culling function
  NNSG2dRndCoreDrawCellCallBack pAfterDrawCellBackFunc;

  NNSG2dRndCoreDrawOamCallBack
      pBeforeDrawOamBackFunc; // callback for OAM parameter overwrite
...
typedef struct NNSG2dRndCoreInstance {
  ...
  NNSG2dRendererAffineTypeOverwiteMode
      affineOverwriteMode; // (standard value:
  ...
  NNSG2dRndCore2DMtxCache
      *pCurrentMtxCacheFor2D; // current matrix cache (for 2D HW)
...
void NNS_G2dSetRndCoreImageProxy(
    NNSG2dRndCoreInstance *pRnd, const struct NNSG2dImageProxy *pImgProxy,
...
NNS_G2D_INLINE void
NNS_G2dInitRndCore2DMtxCache(NNSG2dRndCore2DMtxCache *pMtxCache) {
  MI_CpuFillFast(pMtxCache->affineIndex, MtxCache_NOT_AVAILABLE_ForMemFill,
                 sizeof(pMtxCache->affineIndex));
}

NNS_G2D_INLINE void
NNS_G2dSetRndCore2DMtxCacheMtxParams(NNSG2dRndCore2DMtxCache *pMtxCache,
                                     MtxFx22 *pM) {
